...
async function list(userUid) {
  ...
  if (trackedPages.length) {
    ...
    words.forEach(word => {
      ...
      metadata.last = ref;
      if (!Number(word.is_ayah_marker) && word.text && metadata.words.length < 4)
...
function reviewOrderSql(settings) {
  const memorization = settings && settings.memorization || {};
  switch (memorization.reviewOrder) {
    case 'hard_easy_good':
      return "CASE status WHEN 'hard' THEN 0 WHEN 'easy' THEN 1 ELSE 2 END, next_review_at, page_number";
    case 'due_first':
      return "CASE status WHEN 'hard' THEN 0 ELSE 1 END, next_review_at, CASE status WHEN 'good' THEN 0 ELSE 1 END, page_number";
    case 'page_order':
      return "CASE status WHEN 'hard' THEN 0 ELSE 1 END, page_number";
    case 'random':
      return "CASE status WHEN 'hard' THEN 0 ELSE 1 END, RAND()";
    default:
      return "CASE status WHEN 'hard' THEN 0 WHEN 'good' THEN 1 ELSE 2 END, next_review_at, page_number";
  }
}

async function nextDue(userUid, excludePages, settings, options) {
  await ensureTable();
  const exclude = Array.from(new Set((Array.isArray(excludePages) ? excludePages : [excludePages])
    .map(validPage)
    .filter(Boolean)));
  const rows = await global.query(`
    ...
    FROM ${TABLE}
    WHERE user_uid=${MySQL.escape(userUid)}
      AND status IN (${options && options.hardOnly ? "'hard'" : "'hard', 'good', 'easy'"})
      ${exclude.length ? `AND page_number NOT IN (${exclude.join(',')})` : ''}
    ORDER BY ${reviewOrderSql(settings)}
    LIMIT 1
  `);
