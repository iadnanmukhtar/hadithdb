...
const DOWNLOAD_SCHEMA_VERSION = 'book-v41';
...
async function sendBookDownload(req, res, book, format, loadRows) {
	...
	const cachedFile = preferredDownloadCacheFile(cacheFile);
	if (!flush && cachedFile && await isCacheFresh(cacheFile, contentLastmod))
	...
	const downloadDocument = bookDownloadDocument(book, rows, document, { format: format });
...
async function isCacheFresh(cacheFile, contentLastmod) {
	...
	try {
		if (!contentLastmod)
			return cached.stat.size > 0;
		return cached.stat.size > 0 && cached.stat.mtimeMs >= contentLastmod.getTime();
	} catch (_err) {
		return false;
	}
}
...
function bookDownloadDocument(book, rows, document, options) {
	...
	if (type === 'trans' && options?.format === 'epub')
		return withTranslationEpubPrefatoryBasmalah(rows, document);
	if (type !== 'tafsir')
		return document;
	return buildTafsirPassageDocument(book, rows, document);
}
