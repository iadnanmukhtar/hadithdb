	...
	static async syncForHadithRows(rows, options) {
		...
		for (var row of rows || []) {
			try {
				...
				console.error(`ERROR: syncing chatbot knowledge for ${row.ref || row.hId || row.id || 'unknown'}: ${err.message}`);
				results.push({
					status: 'error',
...
function normalizeBookFilters(books) {
	...
	return Array.from(new Set(aliases.filter(alias => alias !== 'sahihayn' && alias !== 'sixbooks' && alias !== 'toc')));
}
