	...
	static async updateBulk(indexName, recs, full) {
		...
		for (var i = 0; i < recs.length; i++) {
			...
			if (batchSize >= BULK_INDEX_BATCH_SIZE || Buffer.byteLength(bulk, 'utf8') >= BULK_INDEX_MAX_BYTES) {
				...
				console.info(`indexed ${i + 1}/${recs.length} docs on ${indexName}`);
				bulk = "";
				batchSize = 0;
			}
		}
		if (bulk.length > 0) {
			...
			debug(`${res.status} errors=${res.data.errors}`);
			console.info(`indexed ${recs.length}/${recs.length} docs on ${indexName}`);
			bulk = "";
		}
	}
	...
	if (highlight) {
		highlightParams = {
			...
			'fields': [
				...
				{ 'commentary_name': {} },
				{ 'commentary_name_en': {} },
				{ 'commentary_shortName': {} },
				{ 'commentary_shortName_en': {} },
				{ 'commentary_author': {} },
...
async function postBulkWithRetry(indexURL, data, headers) {
	...
	for (var attempt = 1; attempt <= attempts; attempt++) {
		try {
			var res = await axios.post(indexURL, data, { headers });
			if (res.data.errors) {
