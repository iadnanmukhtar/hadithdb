...
const BULK_INDEX_GZIP = process.env.BULK_INDEX_GZIP === '1';
...
class Index {
	...
	/**
	 * Return the distinct values for an Elasticsearch field among matching documents.
	 *
	 * @param {string} indexName
	 * @param {*} query
	 * @param {string} field
	 ...
	 * @returns {Promise<*[]>}
	 */
	static async distinctTermsFromQuery(indexName, query, field, size) {
		if (!/^[A-Za-z0-9_.-]+$/.test(field || ''))
			throw new TypeError(`Invalid Elasticsearch aggregation field: ${field}`);
		size = Number.isInteger(Number(size)) && Number(size) > 0 ? Number(size) : 100;
		...
		if (queryTimeout)
			data.timeout = `${Math.ceil(queryTimeout / 1000)}s`;
		var response = await _post(indexName, '_search', JSON.stringify(data));
		if (response.status != 200)
			throw createError(502, `${response.status} Error aggregating ${field} in Index ${indexName}: ${response.statusText}`);
		if (response.data?.timed_out)
			logSearchTimeoutError(indexName, JSON.stringify(data), searchQueryTimeoutMs(), 'Elasticsearch reported that the aggregation timed out');
		return (response.data?.aggregations?.values?.buckets || []).map(bucket => bucket.key);
	}
		...
		if (settings.search.reindex || options.force === true) {
			try {
				...
				if (indexName === 'hadiths')
					{
					data = (await HadithTranslationIndexView.attachTranslations([data]))[0] || data;
					await require('./HadithSharhIndex').syncHadiths([data]);
					await require('./HadithNarratorIndex').attach([data]);
				}
				normalizeHadithAvailabilityFields(data);
				decorateSearchFields(data);
				debug(`reindexing hId=${data.hId}, ${data.ref} on ${indexName}`);
				var refresh = options.refresh === true || options.refresh === 'true'
					? 'true'
					: options.refresh === 'wait_for' ? 'wait_for' : '';
				var action = `_update/${data.hId}${refresh ? `?refresh=${refresh}` : ''}`;
				await _post(indexName, action, JSON.stringify({
					doc: data,
					doc_as_upsert: true
				}));
			} catch (err) {
	...
	static async updateBulk(indexName, recs, full) {
		...
		if (indexName === 'hadiths') {
			recs = await HadithTranslationIndexView.attachTranslations(recs);
			await require('./HadithSharhIndex').syncHadiths(recs);
			await require('./HadithNarratorIndex').attach(recs);
		}
	...
	static async updateBulkPartial(indexName, recs) {
		debug(`bulk partial updating ${recs.length} docs...`);
		if (indexName === 'hadiths') {
			recs = await HadithTranslationIndexView.attachTranslations(recs);
			await require('./HadithNarratorIndex').attach(recs);
		}
		...
		for (var i = 0; i < recs.length; i++) {
			...
			if (indexName === 'hadiths')
				normalizeHadithAvailabilityFields(data);
			decorateSearchFields(data);
			var id = data.hId || data.id;
			bulk += `{ "update" : { "_index":"${indexName}","_id":"${id}" } }\n${JSON.stringify({ doc: data, doc_as_upsert: true })}\n`;
			batchSize++;
			if (batchSize >= BULK_INDEX_BATCH_SIZE || Buffer.byteLength(bulk, 'utf8') >= BULK_INDEX_MAX_BYTES) {
				var res = await postBulkWithRetry(indexURL, bulk + '\n', headers);
				debug(`${res.status} errors=${res.data.errors}`);
				debug(`partial updated ${i + 1}/${recs.length} docs on ${indexName}`);
				bulk = '';
				batchSize = 0;
			}
		}
		if (bulk.length > 0) {
			res = await postBulkWithRetry(indexURL, bulk + '\n', headers);
			debug(`${res.status} errors=${res.data.errors}`);
			debug(`partial updated ${recs.length}/${recs.length} docs on ${indexName}`);
		}
	}
	...
	static async refresh(indexName) {
		var t0 = new Date().getTime();
		try {
			await axios.post(`${global.settings.search.domain}/${indexName}/_refresh`, null, SearchHttp.axiosConfig());
		} catch (e) {
	...
	static async delete(indexName, id) {
		var t0 = new Date().getTime();
		try {
			await axios.delete(`${global.settings.search.domain}/${indexName}/_doc/${id}`, SearchHttp.axiosConfig());
		} catch (e) {
	...
	static async deleteByQuery(indexName, query) {
		if (!query || typeof query !== 'object')
			throw new TypeError('deleteByQuery requires an Elasticsearch query');
		return await _post(indexName, '_delete_by_query', JSON.stringify({ query: query }));
	}
		...
		highlightParams = {
			...
			'fields': [
				...
				{ 'h2_title_en': {} },
					...
					{ 'h3_intro': {} },
					{ 'h3_intro_en': {} },
					{ 'text': indexName.split(',').includes('sharhs') ? { require_field_match: true } : {} },
					{ 'text_en': indexName.split(',').includes('sharhs') ? { require_field_match: true } : {} },
					{ 'footnotes': {} },
					{ 'footnotes_en': {} },
...
async function postBulkWithRetry(indexURL, data, headers) {
	const attempts = 4;
	const requestData = BULK_INDEX_GZIP ? zlib.gzipSync(Buffer.from(data, 'utf8')) : data;
	const requestHeaders = BULK_INDEX_GZIP
		? Object.assign({}, headers, { 'Content-Encoding': 'gzip' })
		: headers;
	for (var attempt = 1; attempt <= attempts; attempt++) {
		const t0 = Date.now();
		try {
			var res = await axios.post(indexURL, requestData, SearchHttp.axiosConfig({ headers: requestHeaders, timeout: BULK_INDEX_TIMEOUT_MS }));
			const elapsedMs = Date.now() - t0;
			debug.slow('elasticsearch bulk POST', elapsedMs, `url=${indexURL} bytes=${Buffer.byteLength(requestData)} sourceBytes=${Buffer.byteLength(data, 'utf8')}`);
			if (res.data.errors) {
			...
			if (!isRetryableBulkError(err) || attempt === attempts) {
				const responseDetail = err.response && err.response.data !== undefined
					? `\nresponse=${formatBulkErrorResponse(err.response.data)}`
					: '';
				debug.error(`elasticsearch bulk POST failed attempt=${attempt}/${attempts}: ${err.message}${responseDetail}\n${err.stack || ''}`);
				throw err;
			}
...
function formatBulkErrorResponse(data) {
	if (Buffer.isBuffer(data))
		return data.toString('utf8').slice(0, 2000);
...
function searchBackendReason(e) {
	...
	const reason = error?.root_cause?.[0]?.reason ||
		error?.reason;
	const causedBy = error?.caused_by?.reason;
	if (reason && causedBy && reason !== causedBy)
		return `${reason}: ${causedBy}`;
	return reason || causedBy ||
...
function normalizeHadithAvailabilityFields(data) {
	if (!data || data.available_translation_languages === undefined)
		return;
	if (Array.isArray(data.available_translation_languages)) {
		data.available_translation_languages = normalizedLanguageCodes(data.available_translation_languages);
		return;
	}
	try {
		data.available_translation_languages = normalizedLanguageCodes(JSON.parse(String(data.available_translation_languages || '[]')));
	} catch (_err) {
		data.available_translation_languages = [];
	}
}

function normalizedLanguageCodes(values) {
	const seen = new Set();
	return (Array.isArray(values) ? values : []).map(value => String(value && (value.code || value.targetLanguage || value) || '').trim().toLowerCase()).filter(code => {
