...
function createLogger(namespace) {
	...
	if (!logger.__hadithdbAlwaysMethods) {
		...
		logger.slow = (label, elapsedMs, details) => {
			...
			logAlways(logger, 'INFO', [`slow dependency ${label} ${elapsedMs}ms${suffix}`], { includeRequestUrl: true });
		};
...
function logAlways(logger, level, args, options = {}) {
	const message = util.formatWithOptions(debugFactory.inspectOpts, ...args);
	const requestUrl = (level === 'ERROR' || options.includeRequestUrl) ? currentRequestUrl() : '';
	if (logger.enabled) {
		if (requestUrl)
