...
async function headingsForSurahs(surahNumbers) {
	...
	do {
		page = await Index.docsFromQueryFields(
			Heading.INDEX, query, HEADING_FIELDS, offset, PAGE_SIZE, 'h1, h2, h3, ordinal');
		headings.push(...page);
		offset += page.length;
...
function buildOutlines(surahNumbers, headings) {
	...
	Object.values(outlines).forEach(function (outline) {
		outline.sections.sort(function (a, b) {
			return a.start - b.start || a.end - b.end || a.section - b.section;
		});
		outline.sections.forEach(function (section) {
			section.subsections.sort(function (a, b) {
				return a.start - b.start || a.end - b.end || a.subsection - b.subsection;
			});
		});
	});
