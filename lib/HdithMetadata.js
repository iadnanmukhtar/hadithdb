...
const TYPES_FOR_RENDERED_PAIRS = ['narrator', 'sharh_title', 'attribution', 'chain_classification', 'grader', 'grade'];
...
function withPrimaryGrade(grades, item) {
	...
	if (Number(legacyGradeId) === -1) {
		const preferredGrade = preferredColoredGradeOpinion(rows);
		if (preferredGrade) {
			if (item) {
				item.legacyGradeOverride = preferredGrade;
				item.legacyGradeColor = preferredGrade.grade_color || gradeColorForCategory(preferredGrade.grade_category_id);
			}
			return rows;
		}
	}
...
function resolvedPair(managedPairs, type, valueAr, valueEn) {
	return HadithBilingualPairs.resolve(managedPairs, type, valueAr, valueEn);
}

function resolvedAttributionOptions(managedPairs) {
	return HadithAttributions.ATTRIBUTIONS.map(option => {
		const pair = resolvedPair(managedPairs, 'attribution', option.title, option.title_en);
		return Object.assign({}, option, { title: pair.value_ar, title_en: pair.value_en });
	});
}

function resolvedChainCategoryOptions(managedPairs) {
	return HadithChainCategories.CATEGORIES.map(option => {
		const pair = resolvedPair(managedPairs, 'chain_classification', option.title, option.title_en);
		return Object.assign({}, option, { title: pair.value_ar, title_en: pair.value_en });
	});
}

function classificationFromRow(row, managedPairs) {
	if (!row) return { attribution: null, chainCategories: [] };
	const managedSourceAttribution = (managedPairs || []).find(pair => pair.pair_type === 'attribution'
		&& HadithBilingualPairs.normalize(pair.value_ar) === HadithBilingualPairs.normalize(row.source_attribution));
	const sourceAttributionId = HadithAttributions.idForArabic(row.source_attribution) >= 0
		? HadithAttributions.idForArabic(row.source_attribution)
		: HadithAttributions.idForArabic(managedSourceAttribution?.pair_key);
	let attribution = Number(row.attribution_id) >= 0 ? {
	...
	if (attribution) {
		const pair = resolvedPair(managedPairs, 'attribution', attribution.title, attribution.title_en);
		attribution = Object.assign({}, attribution, { title: pair.value_ar, title_en: pair.value_en });
	}
	return {
		attribution,
		chainCategories: HadithChainCategories.parse(row.chain_type).map(category => {
			const pair = resolvedPair(managedPairs, 'chain_classification', category.title, category.title_en);
			return Object.assign({}, category, { title: pair.value_ar, title_en: pair.value_en });
		})
	};
}
...
async function attachClassifications(items) {
	...
	try {
		const [rows, managedPairs] = await Promise.all([global.query(`SELECT h.id AS hadith_id, h.chain, m.attribution AS source_attribution, m.chain_type, m.narrator, m.narrator_en,
			...
			WHERE h.id IN (${ids.join(',')})`), HadithBilingualPairs.managed(['narrator', 'attribution', 'chain_classification'])]);
		const byHadithId = new Map(rows.map(row => [Number(row.hadith_id), classificationFromRow(row, managedPairs)]));
		...
		list.forEach(item => {
			...
			item.hdithClassificationOptions = {
				attributionOptions: resolvedAttributionOptions(managedPairs),
				chainCategoryOptions: resolvedChainCategoryOptions(managedPairs)
			};
			let narrator = row && row.narrator
				? { name: row.narrator, name_en: row.narrator_en || null }
				: localPrimaryNarrator(row?.chain);
			if (narrator) {
				const pair = resolvedPair(managedPairs, 'narrator', narrator.name, narrator.name_en);
				narrator = { name: pair.value_ar, name_en: pair.value_en };
			}
...
async function forHadith(hadithId) {
	...
	try {
		...
		const [metadataRows, narratorRows, linkRows, sharhRows, gradeRows, managedPairs] = await Promise.all([
			...
			global.query(`SELECT hs.id, hs.source_id, hs.title AS source_title, hs.ordinal, hs.chapter, hs.page_num, hs.text, hs.text_en, hs.source_url,
			...
			HadithBilingualPairs.managed(TYPES_FOR_RENDERED_PAIRS)
		]);
		...
		const classification = classificationFromRow(metadata, managedPairs);
		const narratorPair = resolvedPair(managedPairs, 'narrator', metadata.narrator || fallbackNarrator?.name,
			metadata.narrator_en || fallbackNarrator?.name_en);
		return {
			...
			chainCategoryOptions: resolvedChainCategoryOptions(managedPairs),
			narrator: narratorPair.value_ar || null,
			narratorEn: narratorPair.value_en || null,
			sourceIsnadHtml: metadata.source_isnad_html || null,
			gharib: parseJson(metadata.gharib_json, []),
			narrators: narratorRows.map(row => {
				const narratorPair = resolvedPair(managedPairs, 'narrator', vocalizedNarratorName(row, vocalizedNarratorNames), row.name_ala_lc);
				return Object.assign({}, row, {
					local: String(row.source_slug || '').startsWith('admin-'),
					vocalized_name: narratorPair.value_ar,
					name_ala_lc: narratorPair.value_en,
			...
			sharh: sharhRows.map(row => { const pair = resolvedPair(managedPairs, 'sharh_title', row.title, row.title_en);
				return Object.assign({}, row, { title: pair.value_ar, title_en: pair.value_en, sharh_book: SharhBooks.bookForEntry(row) }); }),
			grades: uniqueGradeGraderPairs(gradeRows.map(row => Object.assign({}, row, {
				grade: resolvedPair(managedPairs, 'grade', row.grade, row.grade_en || translatedSourceGrade(row.grade)).value_ar,
				grade_en: resolvedPair(managedPairs, 'grade', row.grade, row.grade_en || translatedSourceGrade(row.grade)).value_en,
				grader: resolvedPair(managedPairs, 'grader', row.grader, row.grader_en || translatedSourceGrader(row.grader)).value_ar,
				grader_en: resolvedPair(managedPairs, 'grader', row.grader, row.grader_en || translatedSourceGrader(row.grader)).value_en,
