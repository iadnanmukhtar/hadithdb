...
async function primaryNarratorSuggestions(query, limit) {
	...
	if (!primaryNarratorOptionsCache || primaryNarratorOptionsExpiresAt <= Date.now()) {
		if (!primaryNarratorOptionsPromise) {
			primaryNarratorOptionsPromise = global.query(`SELECT COALESCE(NULLIF(n.name_tashkil, ''), n.name) AS narrator,
				n.name_ala_lc AS narrator_en
				FROM hdith_narrators n
				WHERE EXISTS (SELECT 1 FROM hdith_hadith_narrators hn WHERE hn.narrator_id=n.id AND hn.ordinal=1)
				LIMIT 10000`).then(rows => {
...
module.exports = { attachClassifications, classificationFromRow, CUSTOM_SHARH_SOURCE_BOOK_ID, ensureEditableColumns, ensureLocalMetadataRow, forHadith, gradeColorForCategory, HDITH_GRADE_COLOR_OPTIONS, invalidatePrimaryNarratorSuggestionCache, legacyGradeCategoryForId, legacyGradeColorForId, localPrimaryNarrator, normalizePrimaryNarratorSearch, narratorDisplayFullname, preferredColoredGradeOpinion, primaryNarratorSuggestions, resolvedSimilarLinks, sourceNarratorNames, translatedSourceGrade, translatedSourceGrader, uniqueGradeGraderPairs, vocalizedNarratorName, withPreferredColoredGradeFirst, withPrimaryGrade };
