...
async function forHadith(hadithId) {
	...
	try {
		const [metadataRows, narratorRows, linkRows, sharhRows, gradeRows] = await Promise.all([
			...
			global.query(`SELECT link_type, source_book_id, source_book_title, source_entry_id, source_num, label, internal_ref, source_url FROM hdith_hadith_links WHERE hadith_id=${id} ORDER BY link_type, source_book_id, source_num, source_entry_id`),
...
module.exports = { forHadith, sourceNarratorNames, translatedSourceGrade, translatedSourceGrader, vocalizedNarratorName, withPrimaryGrade };
