...
async function forHadith(hadithId) {
	...
	try {
		...
		const [metadataRows, narratorRows, linkRows, sharhRows, gradeRows] = await Promise.all([
			...
			global.query(`SELECT hs.id, hs.chapter, hs.page_num, hs.text, hs.text_en, hs.source_url, ss.title, ss.author FROM hdith_hadith_sharh hs JOIN hdith_sharh_sources ss ON ss.id=hs.source_id WHERE hs.hadith_id=${id} ORDER BY hs.id`),
...
module.exports = { attachClassifications, classificationFromRow, ensureEditableColumns, forHadith, gradeColorForCategory, HDITH_GRADE_COLOR_OPTIONS, legacyGradeCategoryForId, legacyGradeColorForId, narratorDisplayFullname, resolvedSimilarLinks, sourceNarratorNames, translatedSourceGrade, translatedSourceGrader, uniqueGradeGraderPairs, vocalizedNarratorName, withPrimaryGrade };
