...
async function attachClassifications(items) {
	...
	try {
		const rows = await global.query(`SELECT h.id AS hadith_id, m.attribution AS source_attribution, m.chain_type, m.narrator, m.narrator_en,
		...
		list.forEach(item => {
			...
			if (row && row.narrator) item.hdithNarrator = { name: row.narrator, name_en: row.narrator_en || null };
...
async function forHadith(hadithId) {
	...
	try {
		...
		const [metadataRows, narratorRows, linkRows, sharhRows, gradeRows] = await Promise.all([
			global.query(`SELECT m.source_book_slug, m.source_entry_id, m.source_reference, m.attribution AS source_attribution, m.chain_type, m.narrator, m.narrator_en, m.source_isnad_html, m.gharib_json,
				a.id AS attribution_id, a.attribution_en, a.attribution
			...
			global.query(`SELECT hn.ordinal, hn.formula, hn.flags_json, n.source_slug, n.name, n.name_tashkil, n.name_ala_lc, n.fullname, n.reliability, n.generation_name, n.death_text, n.source_url FROM hdith_hadith_narrators hn JOIN hdith_narrators n ON n.id=hn.narrator_id WHERE hn.hadith_id=${id} ORDER BY hn.ordinal`),
		...
		return {
			...
			narratorEn: metadata.narrator_en || null,
