  ...
  tool('lookup_hadith_detail', 'Look up hadith detail',
    'Retrieve the full Arabic and English text, chain, headings, grades, scholarly metadata, and canonical URL for an exact hadith reference.', {
      ...
      properties: {
        ...
        response_profile: { type: 'string', enum: ['compact', 'default', 'full'], default: 'default', description: 'Default includes scholarly metadata. Compact returns the core record without the potentially large metadata object.' }
      },
      required: ['reference'],
      additionalProperties: false
    })
...
function responseProfile(args) {
  return args.response_profile || 'default';
}
...
function normalizeTafsirCommentaryEntry(entry, ayah, language, maxChars) {
  ...
  return {
    ...
    language: text(entry.content_translation_language) || language || null,
    ...bilingual
  };
}
...
async function callTool(name, args = {}, context = {}) {
  ...
  if (name === 'search_quran') {
    ...
    return result({ query: input.query, offset: page.pagination.offset, pagination: page.pagination, results: page.results }, `Found ${page.results.length} Quran results.`);
  }
  ...
  if (name === 'lookup_hadith_detail') {
    ...
    const profile = responseProfile(args);
    const records = response.data.map(item => normalizeScriptureItem(item, urls.hadith, { detail: profile !== 'compact' }));
