  ...
  tool('lookup_tafsir', 'Look up tafsir',
    'Retrieve commentary from a named HadithDB tafsir source for one Quran ayah.', {
      ...
      properties: {
        ...
        max_chars: { type: 'integer', minimum: 500, maximum: MAX_TEXT_LIMIT, default: DEFAULT_TEXT_LIMIT }
      },
...
function normalizeScriptureItem(item, baseUrl, options = {}) {
  ...
  const result = {
    ...
    english: {
      ...
      chain: truncate(item.chain_en),
      body: truncate(item.body_en, options.detail ? 20000 : 5000),
      footnote: truncate(item.footnote_en)
    },
    arabic: {
      title: text(item.title),
      chain: truncate(item.chain),
      body: truncate(item.body, options.detail ? 20000 : 5000),
      footnote: truncate(item.footnote)
    }
  };
...
function normalizeTafsirSearchItem(item, quranBaseUrl) {
  ...
  return {
    ...
    text_english: truncate(htmlToText(item.text_en || item.translation_body_html), 8000),
    text_arabic: truncate(htmlToText(item.text), 8000),
...
async function callTool(name, args = {}, context = {}) {
  ...
  if (name === 'lookup_tafsir') {
    ...
    if (ayah === 0 && surah !== 1)
      throw new Error('Ayah 0 is valid only for Surah 1.');
    const maxChars = integer(args.max_chars, 'max_chars', { min: 500, max: MAX_TEXT_LIMIT, fallback: DEFAULT_TEXT_LIMIT });
    ...
    const commentary = entries.filter(Boolean).map(entry => {
      ...
      return {
        ...
        text: truncate(fullText, maxChars),
