...
async function applySameBookHeadingNavigation(heading) {
	...
	if (level === 2 && Number(heading.h1) > 0 && Number(heading.h2_count) === 0) {
		const interveningChapters = await Index.docsFromQueryString(Heading.INDEX,
			`book_alias:${heading.book_alias} AND level:1 AND ordinal:>${value}`, 0, 100, 'ordinal ASC');
		const firstPopulatedChapter = interveningChapters.find(candidate => Number(candidate.h1_count) > 0);
		if (firstPopulatedChapter && (!nextHeading || Number(firstPopulatedChapter.ordinal) < Number(nextHeading.ordinal)))
			nextHeading = firstPopulatedChapter;
	}
	...
	heading.prev = publicHeading(previousHeading);
	heading.next = publicHeading(nextHeading);
	return heading;
}
...
function publicHeading(heading) {
	if (!heading)
		return null;
	const output = Heading.toLevel(heading);
	if (Number(output.h1) === 0 && !(Number(output.count) > 0))
		output.path = `${output.book_alias}/introduction`;
	return output;
}
