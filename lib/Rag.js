...
class Rag {
	...
	static async retrieveItems(question, options) {
		...
		var results = await Search.a_searchText(question, options.books, 0);
		var items = results.slice(0, topK).map(stripHighlights);
		debug(`retrieve complete hits=${results.length} returned=${items.length} refs=${items.map(item => item.ref).join(',')} elapsed=${elapsed(t0)}s`);
		return items;
	}
...
function fallbackAnswer(sources) {
	...
	return 'Generation is disabled because no OpenAI API key is configured. Retrieved sources: ' +
...
async function generateAnswer(question, sources, model) {
	...
	var input = [
		{
			role: 'developer',
			content: [
				'You answer questions about hadith using only the provided HadithDB sources.',
	...
	try {
		...
		debug(`generate failed model=${model} elapsed=${elapsed(t0)}s error=${err.message}`);
		if (err.response) {
