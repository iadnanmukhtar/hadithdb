...
function isEnabled() {
  return !adminOnlyTranslations() && isFeatureEnabled() && isConfigured();
}

// Admin-only translations bypass billing and remain available without Stripe.
function adminOnlyTranslations() {
  return booleanSetting(contentSettings().adminOnly);
}

function contentTranslationsEnabled() {
  return adminOnlyTranslations() || isEnabled();
}
