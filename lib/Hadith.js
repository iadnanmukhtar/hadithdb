...
class Hadith {
    ...
    static async a_GetHadithByIds(rows, idColnName) {
        ...
        for (var i = 0; i < rows.length; i++)
            ids.push({
                FIELD: 'hId',
                VALUE: rows[i][idColnName]
            });
        var results = await global.searchIdx.QUERY({
            OR: ids
        }, { DOCUMENTS: true });
        for (var n = 0; n < results.RESULT.length; n++) {
            var hadith = results.RESULT[n]._doc;
            hadith.score = results.RESULT[n]._score;
            hadiths.push(new Hadith(hadith));
        }
        return hadiths;
    }
