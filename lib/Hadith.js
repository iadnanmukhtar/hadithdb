...
class Hadith {
    ...
    static async a_dbGetSimilarCandidates(hadith) {
        ...
        var directSimilarSql = `
                SELECT id, 1 AS rating, 0 AS is_candidate, 0 AS is_actual, 1 AS is_part, bookId, ordinal, part FROM hadiths
                WHERE part = "${Utils.escSQL(Utils.sql(hadith.part))}"
                UNION
                SELECT hadithId2 AS id, 1 AS rating, 0 AS is_candidate, 1 AS is_actual, 0 AS is_part, h.bookId, h.ordinal, h.part FROM hadiths_sim, hadiths h
                WHERE hadithId1 = ${hadith.id} AND hadithId2=h.id
                UNION
                SELECT hadithId1 AS id, 1 AS rating, 0 AS is_candidate, 1 AS is_actual, 0 AS is_part, h.bookId, h.ordinal, h.part FROM hadiths_sim, hadiths h
                WHERE hadithId2 = ${hadith.id} AND hadithId1=h.id
                UNION
                SELECT hadithId2 AS id, rating, 1 AS is_candidate, 0 AS is_actual, 0 AS is_part, h.bookId, h.ordinal, h.part FROM hadiths_sim_candidates, hadiths h
                WHERE hadithId1 = ${hadith.id} AND rating >= 0.6 AND hadithId2=h.id
                UNION
                SELECT hadithId1 AS id, rating, 1 AS is_candidate, 0 AS is_actual, 0 AS is_part, h.bookId, h.ordinal, h.part FROM hadiths_sim_candidates, hadiths h
                WHERE hadithId2 = ${hadith.id} AND rating >= 0.6 AND hadithId1=h.id`;
        var rows = await global.query(`
            SELECT DISTINCT * FROM (
                ${directSimilarSql}
                UNION
                SELECT c.hadithId2 AS id, c.rating, 1 AS is_candidate, 0 AS is_actual, 0 AS is_part, h.bookId, h.ordinal, h.part
                FROM hadiths_sim_candidates c, hadiths h, (${directSimilarSql}) direct
                WHERE c.hadithId1 = direct.id
                  AND c.rating >= 0.6
                  AND c.hadithId2 = h.id
                UNION
                SELECT c.hadithId1 AS id, c.rating, 1 AS is_candidate, 0 AS is_actual, 0 AS is_part, h.bookId, h.ordinal, h.part
                FROM hadiths_sim_candidates c, hadiths h, (${directSimilarSql}) direct
