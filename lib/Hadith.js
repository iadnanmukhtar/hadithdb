...
class Hadith {
    ...
    static async a_dbHadithInit(hadith) {
        ...
        // if (!hadith.tags)
        //     hadith.tags = await a_dbGetHadithTags(hadith);
    ...
    static async a_ReindexHadith(id) {
        ...
        if (settings.reindex || settings.findSimilar) {
            ...
            console.log(`post processing ${id}`);
...
async function a_dbGetHadithTags(hadith) {
    console.log('getting tag: ' + hadith.id);
    var rows = await global.query(`SELECT t.text_en
