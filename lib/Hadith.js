...
class Hadith {
    ...
    static async a_GetHadithByIds(rows, idColnName) {
        ...
        var _hadiths = await axios.post(
            global.searchURLBase + '/hadiths/_search',
            equery,
    ...
    static async a_GetAllHadithsWithTag(tagId, offset) {
        ...
        return await Hadith.a_GetHadithByIds(rows, 'hadithId');
    }
    ...
    static async a_dbGetSimilarCandidates(hadith) {
        ...
        var rows = await global.query(`
            SELECT DISTINCT * FROM (
                ...
                WHERE hadithId1 = ${hadith.id} AND rating >= 0.6
                UNION
                SELECT hadithId1 AS id, rating FROM hadiths_sim_candidates
                WHERE hadithId2 = ${hadith.id} AND rating >= 0.6
            ) x`);
        results = await Hadith.a_GetHadithByIds(rows, 'id');
        return results;
    }
    ...
    static async a_ReindexHadith(id) {
        var rows = await global.query(`SELECT * FROM v_hadiths WHERE hId=${id}`);
        console.log(`post processing hadith=${id} (${rows.length}) docs`);
        await Hadith.a_ReindexRows("hadiths", rows);
    }

    static async a_ReindexTOC(id) {
        var rows = await global.query(`SELECT * FROM v_toc WHERE (h1_id=${id} OR h2_id=${id})`);
        console.log(`post processing toc=${id} (${rows.length}) docs`);
        await Hadith.a_ReindexRows("toc", rows);
    }

    static async a_ReindexRows(indexName, rows) {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                if (settings.reindex) {
                    try {
                        ...
                        for (var k in rows[i])
                            data[k] = rows[i][k];
                        var endPoint = 'http://search.quranunlocked.com/' + indexName + '/_update/' + rows[i].hId;
                        console.log(`reindexing hId=${rows[i].hId}, ${data.ref} ${endPoint}`);
