...
class Hadith {
    ...
    static async a_recordSimilarMatches(deHadith, deDB) {
        ...
        for (var i = 0; i < deDB.length; i++) {
            ...
            try {
                var source = (deHadith.body.length >= deDB[i].body.length) ? deDB[i].body : deHadith.body;
                var target = (deHadith.body.length >= deDB[i].body.length) ? deHadith.body : deDB[i].body;
    ...
    static disemvoweledHadith(hadith) {
        ...
        hadith.body = hadith.body.replace(/[ؤئ]/g, 'ء');
        return hadith;
    }
