...
class Hadith {
    ...
    static async a_parseNarrators(hadith) {
        ...
        var translatedChain = Hadith.transliteratedNarratorChain(hadith.chain);
        var chains_en = translatedChain.narrators;
        ...
        var trans = translatedChain.chain_en;
        if (trans.length > 0)
    ...
    static transliteratedNarratorChain(chain_ar) {
        var chains_en = Hadith.translateNarrators(chain_ar).narrators;
        var trans = '';
        for (var j = 0; j < chains_en.length; j++) {
            if (j > 0)
                trans += ' ';
            if (chains_en.length > 1)
                trans += `[Chain ${j + 1}] `;
            var narrs_en = [].concat(chains_en[j]);
            for (var k = 0; k < narrs_en.length; k++) {
                if (k > 0) trans += ' > ';
                trans += narrs_en[k];
            }
        }
        return { chain_en: trans.trim(), narrators: chains_en };
    }

    static async a_dbGetSimilarCandidates(hadith) {
        await Hadith.a_ensureSimilarSuppressionTable();
        var results = [];
        var hasPart = Utils.isTruthy(hadith.part) && String(hadith.part).trim().toLowerCase() !== 'null';
        var directSimilarParts = [];
        if (hasPart)
            directSimilarParts.push(`
                SELECT id, 1 AS rating, 0 AS is_candidate, 0 AS is_actual, 1 AS is_part, 1 AS is_direct, NULL AS remove_id1, NULL AS remove_id2, bookId, ordinal, part
                FROM hadiths WHERE part = "${Utils.escSQL(Utils.sql(hadith.part))}"`);
        directSimilarParts.push(`
        ...
        var directSimilarSql = directSimilarParts.join('\nUNION\n');
        // Similarity is reciprocal, but not transitive: only load direct links.
        var rows = await global.query(`
            WITH direct AS (
                ${directSimilarSql}
            )
            SELECT * FROM direct x
            WHERE id != ${hadith.id}
              AND (
                x.is_actual=1
                OR (x.is_candidate=1 AND x.is_direct=1)
                OR NOT EXISTS (
                    SELECT 1 FROM hadiths_sim_suppressed s
                    WHERE (s.hadithId1 = ${hadith.id} AND s.hadithId2 = x.id)
                       OR (s.hadithId2 = ${hadith.id} AND s.hadithId1 = x.id)
                )
              )`);
        var rowMap = new Map();
        rows.forEach(row => {
        ...
        // Similar relationships are not paginated. Fetch every ID in bounded
        // batches instead of inheriting the search results page-size limit.
        for (var offset = 0; offset < rows.length; offset += 500) {
            var batch = rows.slice(offset, offset + 500);
            results.push(...await Index.docsFromObjectArray('hadiths', batch, 'id', 0, batch.length));
        }
    ...
    static async a_reinit() {
        ...
        debug('rebuilding in-memory caches...');
        ...
        await Promise.all([
            ...
            Search.rebuildInMemoryCaches(),
            PaymentConfig.loadLanguages({ force: true }),
            Tafsir.withFirstPassages(tafsirs)
        ]);
    ...
    static parseNarrators(chain_ar) {
        ...
        for (var i = 0; i < chains.length; i++) {
            ...
            ch = ch.replace(/(^| )([وف]\u064e)?أَخْبَرَتْنَاهُ/g, ' {} ');
            ch = ch.replace(/(^| )([وف]\u064e)?أَخْبَرَتْنَا/g, ' {} ');
            ch = ch.replace(/(^| )([وف]\u064e)?أَخْبَرَتْنِي/g, ' {} ');
            ch = ch.replace(/(^| )([وف]\u064e)?أَخْبَرَتْهَا/g, ' {} ');
            ch = ch.replace(/(^| )([وف]\u064e)?أَخْبَتْرَهُ/g, ' {} ');
    ...
    static splitHadithText(hadithText) {
        try {
            ...
            textMarked = Utils.replaceRA(textMarked).replace(/ؓ/g, '');
            ...
            hadithText.chain = Utils.replaceRA(Utils.replacePBUH(hadithText.chain || '')).replace(/\s+/g, ' ').trim();
            hadithText.body = Utils.replaceRA(Utils.replacePBUH(hadithText.body || '')).replace(/\s+/g, ' ').trim();
        } catch (err) {
