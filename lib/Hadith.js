...
class Hadith {
    ...
    static async a_dbGetTranslationRequests() {
        ...
        var rows = await global.query(
            ...
            WHERE h.requested > 0
              AND h.id = vh.hId
            ORDER BY h.requested DESC, h.lastmod DESC
    ...
    static async a_dbGetSimilarCandidates(hadith) {
        ...
        var rows = await global.query(`
            SELECT DISTINCT * FROM (
                SELECT id, 1 AS rating, bookId FROM hadiths
                WHERE part = "${Utils.escSQL(Utils.sql(hadith.part))}"
                UNION
                SELECT hadithId2 AS id, 1 AS rating, h.bookId AS bookId FROM hadiths_sim, hadiths h
                WHERE hadithId1 = ${hadith.id} AND hadithId2=h.id
                UNION
                SELECT hadithId1 AS id, 1 AS rating, h.bookId AS bookId FROM hadiths_sim, hadiths h
                WHERE hadithId2 = ${hadith.id} AND hadithId1=h.id
                UNION
                SELECT hadithId2 AS id, rating, h.bookId AS bookId FROM hadiths_sim_candidates, hadiths h
                WHERE hadithId1 = ${hadith.id} AND rating >= 0.6 AND hadithId2=h.id
                UNION
                SELECT hadithId1 AS id, rating, h.bookId AS bookId FROM hadiths_sim_candidates, hadiths h
                WHERE hadithId2 = ${hadith.id} AND rating >= 0.6 AND hadithId1=h.id
            ) x
            WHERE id != ${hadith.id}
            ORDER BY rating, bookId`);
