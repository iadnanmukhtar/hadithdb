...
function buildViewSql() {
	const selectColumns = [
		...
		'h.id AS hId',
		't.tId AS tId',
		"CASE WHEN b.type IN ('sirah','history') THEN 'sirah' ELSE 'hadith' END AS doctype",
		'b.type AS book_type',
		't.path AS path',
...
async function ensureView(options) {
	...
	const missingBaseColumns = ['available_translation_languages', 'body_indopak', 'body_warsh', 'book_type'].filter(column => !columns.has(column));
