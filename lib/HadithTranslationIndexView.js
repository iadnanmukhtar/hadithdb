...
function translationColumns(codes) {
	const columns = [];
	codes.forEach(code => {
		TRANSLATION_FIELDS.forEach(field => {
			columns.push(`ht.${columnName(field, code)} AS ${columnName(field, code)}`);
		});
	});
	return columns;
}
...
function translationAggregateColumns(codes) {
	...
	codes.forEach(code => {
		TRANSLATION_FIELDS.forEach(field => {
			columns.push(`MAX(CASE WHEN target_language=${sql(code)} THEN JSON_UNQUOTE(JSON_EXTRACT(content_json, '$.${field}')) END) AS ${columnName(field, code)}`);
		});
	});
	return columns;
}

function translationJoinSql(codes) {
	if (!codes.length)
		return '';
	return `
LEFT JOIN (
	...
	WHERE row_num=1
	GROUP BY hadith_id
) ht ON ht.hadith_id=h.id`;
}

function buildViewSql(codes) {
	codes = translationLanguageCodes(codes);
	const translatedColumns = translationColumns(codes);
	const selectColumns = [
		'h.id AS id',
		'h.id AS hId',
		't.tId AS tId',
...
async function ensureView(options) {
	...
	const expectedColumns = expectedTranslationColumnNames(codes);
	...
	const stale = generatedTranslationColumnNames(columns).filter(column => !expectedColumnSet.has(column));
	if (!options.force && missing.length < 1 && stale.length < 1)
		return { updated: false, languages: codes, columns: expectedColumns };
	const sqlText = buildViewSql(codes);
	debug(`updating v_hadiths translation columns languages=${codes.join(',')}`);
	await global.query(sqlText);
	return { updated: true, languages: codes, columns: expectedColumns };
}
