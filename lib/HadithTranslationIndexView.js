...
async function ensureView(options) {
	...
	const needsAvailability = !columns.has('available_translation_languages');
	if (!options.force && stale.length < 1 && !needsAvailability)
		return { updated: false, columns: [] };
	const sqlText = buildViewSql();
	debug(`updating v_hadiths base view; removing generated translation columns=${stale.join(',')} availability=${needsAvailability}`);
...
function hadithRowId(row) {
	const id = parseInt(row && (row.hId || row.id), 10);
