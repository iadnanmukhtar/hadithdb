...
let quranSubsectionRangesBySurahCache = null;
...
async function quranSectionRangesBySurah() {
	...
	const rows = await global.query(`
		SELECT h1 AS surah, h2 AS section,
	...
	for (var i = 0; i < rows.length; i++) {
		...
		if (section > 0 && range.surah > 0) {
			...
			rangesBySurah[range.surah].push({
				section: section,
				title_en: (row.h2_title_en || '').toString().trim(),
				title: (row.h2_title || '').toString().trim(),
				start: range.ayah,
...
async function quranSubsectionRangesBySurah() {
	if (quranSubsectionRangesBySurahCache)
		return quranSubsectionRangesBySurahCache;
	const rows = await global.query(`
		SELECT h1 AS surah, h2 AS section, h3 AS subsection,
			h3_title_en, h3_title, h3_start, h3_count
		FROM v_toc
		WHERE book_alias='quran' AND level=3 AND h3_start IS NOT NULL AND h3_count IS NOT NULL
		ORDER BY h1, h2, h3`);
	const rangesBySurah = {};
	for (const row of rows) {
		const range = parseStartParts(row.h3_start);
		const section = Number(row.section);
		const subsection = Number(row.subsection);
		const count = parseInt(row.h3_count, 10);
		if (!Number.isInteger(section) || !Number.isInteger(subsection) || !Number.isInteger(range.surah) || !Number.isInteger(range.ayah) || !Number.isInteger(count) || count <= 0)
			continue;
		rangesBySurah[range.surah] = rangesBySurah[range.surah] || [];
		rangesBySurah[range.surah].push({
			section: section,
			subsection: subsection,
			title_en: (row.h3_title_en || '').toString().trim(),
			title: (row.h3_title || '').toString().trim(),
			start: range.ayah,
			end: range.ayah + count - 1
		});
	}
	Object.keys(rangesBySurah).forEach(function (surahNum) {
		rangesBySurah[surahNum].sort(function (a, b) {
			return a.start - b.start || a.section - b.section || a.subsection - b.subsection;
		});
	});
	quranSubsectionRangesBySurahCache = rangesBySurah;
	return quranSubsectionRangesBySurahCache;
}
