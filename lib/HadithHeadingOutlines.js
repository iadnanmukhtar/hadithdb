...
function h1NavigationItem(heading, bookAlias) {
	...
	return {
		...
		path: text(heading.path) || `${bookAlias}/${number}`
	};
}
...
function buildOutline(chapter, headings) {
	...
	(headings || []).forEach(function (heading) {
		...
		const section = {
			...
			title: title.text,
			titleLang: title.lang,
			path: text(heading.path) || `${bookAlias}/${chapterNumber}/${sectionNumber}`,
			subsections: []
		};
	...
	(headings || []).forEach(function (heading) {
		...
		section.subsections.push({
			...
			title: title.text,
			titleLang: title.lang,
			path: text(heading.path) || `${bookAlias}/${chapterNumber}/${sectionNumber}/${subsectionNumber}`
		});
	});
...
function buildFlatOutline(chapter, headings) {
	...
	(headings || []).forEach(function (heading) {
		...
		const title = headingTitle(heading, 1);
		const headingPath = text(heading.path) || `${bookAlias}/${chapterNumber}`;
	...
	const currentIndex = sections.findIndex(function (section) {
		return text(section.chapter) === text(chapter.h1) || (text(chapter.path) && section.path === text(chapter.path));
	});
	return {
		key: `${bookAlias}:flat`,
		flat: true,
...
async function headingsFromQuery(query) {
	const headings = [];
	let offset = 0;
	let page;
	do {
		page = await Index.docsFromQueryFields(Heading.INDEX, query, HEADING_FIELDS, offset, PAGE_SIZE, 'ordinal');
		headings.push(...page);
		offset += page.length;
...
async function forChapter(chapter) {
	...
	const headings = await headingsFromQuery(query);
