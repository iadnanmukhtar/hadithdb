...
const DEFAULT_TAFSIR_ALIAS = 'mokhtasar';
...
function visibleTafsirsSync() {
  ...
  global.tafsirCarouselBooks = expandBilingualLocalCommentaries((global.commentaries || [])
    .filter(row => Number(row.hidden) === 0 && commentaryCatalogRoles(row).includes('tafsir'))
    .map(({ hidden, ...row }) => commentaryCatalogBook(row, 'tafsir')))
...
function visibleTranslationsSync() {
  return (global.commentaries || [])
    .filter(row => Number(row.hidden) === 0 && commentaryCatalogRoles(row).includes('trans'))
    .map(({ hidden, ...row }) => commentaryCatalogBook(row, 'trans'));
}
...
function invalidateMemoryCaches(alias) {
  global.tafsirCarouselBooks = null;
  if (!alias || !(global.tafsirFirstPassages instanceof Map)) {
    global.tafsirFirstPassages = null;
    return;
  }
  const prefix = `${alias}:`;
  for (const key of global.tafsirFirstPassages.keys()) {
    if (key.startsWith(prefix))
      global.tafsirFirstPassages.delete(key);
  }
}
...
function commentaryCatalogRoles(row) {
  const storedType = commentaryType(row);
  const roles = new Set(storedType === 'trans' || storedType === 'tafsir' ? [storedType] : []);
  const properties = commentaryProperties(row && row.properties);
  const displayAs = properties.quran && Array.isArray(properties.quran.display_as)
    ? properties.quran.display_as
    : [];
  displayAs.forEach(role => {
    role = (role || '').toString().toLowerCase();
    if (role === 'translation')
      role = 'trans';
    if (role === 'trans' || role === 'tafsir')
      roles.add(role);
  });
  return Array.from(roles);
}

function commentaryProperties(value) {
...
function commentaryCatalogBook(row, role) {
  const storedType = commentaryType(row);
  return {
    ...row,
    type: role,
    ...(storedType !== role ? {
      ...
      ...(role === 'trans' ? { quranBookSlug: row.alias } : {})
    } : {})
  };
}
...
function expandBilingualLocalCommentaries(rows) {
  const expanded = [];
  rows.forEach(row => {
    const languages = commentaryLanguages(row);
    if (row.source === 'local' && languages.length > 1) {
      ['en', 'ar'].concat(languages).filter((lang, index, values) => languages.includes(lang) && values.indexOf(lang) === index)
        .forEach(lang => expanded.push({ ...row, languages: row.languages || row.lang, lang: lang }));
      return;
    }
    expanded.push(row);
...
function commentaryLanguages(book) {
  return Array.from(new Set((book && (book.languages || book.lang) || '').toString().toLowerCase().split('-')
    .map(value => value.trim())
    .filter(value => /^[a-z][a-z0-9]{1,15}$/.test(value))));
}

function uniqueCarouselTafsirs(tafsirs) {
  const unique = [];
  const indexes = new Map();
  (Array.isArray(tafsirs) ? tafsirs : []).forEach(tafsir => {
    const alias = (tafsir && tafsir.alias || '').toString();
    if (!alias || !indexes.has(alias)) {
      indexes.set(alias, unique.length);
      unique.push(tafsir);
      return;
    }
    const index = indexes.get(alias);
    if (carouselLanguageRank(tafsir && tafsir.lang) < carouselLanguageRank(unique[index] && unique[index].lang))
      unique[index] = tafsir;
  });
  return unique.sort((a, b) => Number(!isDefaultTafsir(a)) - Number(!isDefaultTafsir(b)));
}

function isDefaultTafsir(book) {
  return (book && book.alias || '').toString() === DEFAULT_TAFSIR_ALIAS;
}

function carouselLanguageRank(lang) {
  if (lang === 'en')
    return 0;
  if (lang === 'ar')
    return 1;
...
function isBilingualTafsir(book) {
  if (!book || book.source !== 'local')
    return false;
  if (commentaryLanguages(book).length > 1)
    return true;
  return hasCommentaryLanguageFormat(book.format, 'en') && hasCommentaryLanguageFormat(book.format, 'ar');
}

function carouselLanguage(book) {
  const languages = commentaryLanguages(book);
  const bilingual = isBilingualTafsir(book) || languages.includes('ar') && languages.includes('en');
  return book && book.lang === 'ar' && !bilingual ? 'ar' : 'en';
}
...
async function localTranslationEntry(book, surah, ayah, options = {}) {
  if (!book || book.type !== 'trans' || book.source !== 'local')
    return null;
  const rows = await localCommentaryRowsFromIndex(book.alias, surah, ayah, ayah, {
    dbFallbackOnEmpty: false,
    dbFallbackOnUnavailable: options.dbFallbackOnUnavailable !== false,
    logStart: false
  });
  const storedRow = rows.find(row => commentaryRowHasLanguage(row, 'en'));
  const row = splitTranslationRowByAyah(storedRow, book).find(entry => Number(entry.ayahFrom) === Number(ayah));
  if (!row)
    return null;
...
function splitTranslationRowByAyah(row, book) {
  if (!row)
    return [];
  if (book && book.storedType === 'tafsir')
    row = { ...row, text: '', footnotes: '' };
  const properties = commentaryProperties(book && book.properties);
  const quranProperties = properties.quran || {};
  if (quranProperties.translation_extract === 'first_paragraph') {
    const firstParagraph = Utils.emptyIfNull(row.text_en).toString().trim().split(/\n\s*\n+/)[0] || '';
    row = { ...row, text_en: firstParagraph, footnotes_en: '' };
  }
  const ayahFrom = Number(row.ayahFrom);
  const ayahTo = Number(row.ayahTo);
  const ayahCount = ayahTo - ayahFrom + 1;
  if (!Number.isInteger(ayahFrom) || !Number.isInteger(ayahTo) || ayahCount <= 1)
    return [row];
  let paragraphs = Utils.emptyIfNull(row.text_en).toString().trim().split(/\n\s*\n+/).filter(Boolean);
  if (paragraphs.length !== ayahCount) {
    const splitMode = quranProperties.translation_split;
    if (splitMode !== 'paragraphs' || paragraphs.length < ayahCount)
      return [row];
    const leadingParagraphCount = paragraphs.length - ayahCount + 1;
    paragraphs = [paragraphs.slice(0, leadingParagraphCount).join('\n\n')].concat(paragraphs.slice(leadingParagraphCount));
  }
  return paragraphs.map((text, index) => ({
    ...row,
    ayahFrom: ayahFrom + index,
    ayahTo: ayahFrom + index,
    text_en: text
  }));
}
...
async function localTafsirEntries(book, surah, ayah, options = {}) {
  ...
  const rows = await localCommentaryRowsFromIndex(book.alias, surah, ayah, ayah, {
    dbFallbackOnEmpty: options.editMode === true,
    logStart: options.editMode === true
  });
  ...
  return rows.map(row => {
    ...
    return {
      ...
      translationHtml: rendered.translationHtml
    };
...
async function adjacentPassage(book, entries, direction, currentRef) {
  if ((!entries || entries.length < 1) && currentRef)
...
function invocationBoundary(entries, surah, ayah, direction, introductionHref) {
  ...
  if (direction < 0 && introductionHref && current.surah === 1 && current.startAyah === 0)
...
async function firstPassage(book) {
  ...
  try {
    ...
    }, ['surah', 'ayahFrom', 'ayahTo'], 0, 1, `${surahSortDirection(book, 1)}, ayahFrom ASC, ayahTo ASC`, false);
  } catch (err) {
  ...
  return {
    ...
    endAyah: Number(row.ayahTo)
  };
}

async function firstPassageInSurah(book, surah, options = {}) {
  if (!book || !Number.isInteger(Number(surah)))
    return null;
  surah = Number(surah);
  const minimumAyah = options.includeZero === true && surah === 1 ? 0 : 1;
  if (book.source !== 'local')
    ...
    rows = await Index.docsFromQueryFields('commentaries', {
      bool: {
        filter: [
          ...
          { term: { source: 'local' } },
          { term: { surah: surah } },
          { range: { ayahFrom: { gte: minimumAyah } } },
          { range: { ayahTo: { gte: minimumAyah } } }
        ]
      }
    }, ['surah', 'ayahFrom', 'ayahTo'], 0, 1, 'ayahFrom ASC, ayahTo ASC', false);
  } catch (err) {
    if (!isSearchBackendUnavailable(err) || typeof global.query !== 'function')
      throw err;
    rows = await localFirstPassageInSurahRowsFromDb(book, surah, minimumAyah);
  }
  const row = rows[0];
  if (!row)
    return null;
...
async function localAdjacentPassage(book, current, direction) {
  ...
  const wrapsBackwardFromInvocation = direction < 0 && current.surah === 1 && current.startAyah === 0;
  const query = wrapsBackwardFromInvocation
    ? {
      bool: {
        filter: [
          { term: { doctype: 'commentary' } },
...
async function remoteAdjacentPassage(book, current, direction) {
  if (direction > 0) {
    ...
    if (next && isNextPassage(book, next, current))
      return { surah: next.surah, ayah: next.startAyah, endAyah: next.endAyah };
    return { ...candidate, endAyah: candidate.ayah };
  }

  const wrapsBackwardFromInvocation = current.surah === 1 && current.startAyah === 0;
  let candidate = wrapsBackwardFromInvocation
    ? lastQuranAyah()
    : previousQuranAyah(book, current.surah, current.startAyah);
  while (candidate) {
    const entries = await remoteTafsirEntries(book, candidate.surah, candidate.ayah);
    const previous = currentPassageRange(entries);
    if (!previous)
      return { ...candidate, endAyah: candidate.ayah };
    if (wrapsBackwardFromInvocation)
      return { surah: previous.surah, ayah: previous.startAyah, endAyah: previous.endAyah };
...
function lastQuranAyah() {
  const surah = quranSurah(114);
  return surah ? { surah: 114, ayah: Number(surah.ayahs) } : null;
}

function quranSurah(surahNum) {
  return Surahs.find(surahNum)
    || (global.surahs || []).find(item => Number(item.num) === Number(surahNum))
    || null;
}
...
function quranAyahFromMemory(ref) {
  ...
  if (!Number.isInteger(surahNum) || !Number.isInteger(ayahNum) || ayahNum < (surahNum === 1 ? 0 : 1))
    return null;
  const surah = quranSurah(surahNum);
...
async function localCommentaryRowsFromIndex(src, surah, ayahFrom, ayahTo, options = {}) {
  ...
  try {
    ...
    if (!isSearchBackendUnavailable(err) || typeof global.query !== 'function' || options.dbFallbackOnUnavailable === false)
      throw err;
  ...
  if (!rows.length && options.dbFallbackOnEmpty === true && typeof global.query === 'function')
    rows = await localCommentaryRowsFromDb(src, surah, ayahFrom, ayahTo, size);
  return rows.map(row => ({
    ...row,
    format: row.format,
...
async function localFirstPassageInSurahRowsFromDb(book, surah, minimumAyah = 1) {
  const alias = global.dbPool.escape(book.alias);
  const commentaryJoin = await Books.commentaryJoin('bc', 'hc');
  return global.query(`
    ...
    WHERE bc.source='local'
      ...
      AND hc.ayahFrom>=${Number(minimumAyah)}
      AND hc.ayahTo>=${Number(minimumAyah)}
...
async function localSitemapPassageRowsFromDb(book) {
  ...
  return global.query(`
    ...
    WHERE bc.source='local'
      ...
      AND hc.ayahFrom>=1
      AND hc.ayahTo>=1
      AND (TRIM(COALESCE(hc.text, ''))<>'' OR TRIM(COALESCE(hc.text_en, ''))<>'')
    ORDER BY hc.surah ${isReverseSurahOrder(book) ? 'DESC' : 'ASC'}, hc.ayahFrom ASC, hc.ayahTo ASC
    LIMIT 10000`);
}
...
async function localAdjacentPassageRowsFromDb(book, current, direction) {
  ...
  const adjacencyClause = direction < 0 && Number(current.surah) === 1 && Number(current.startAyah) === 0
    ? '1=1'
    : `(
...
function renderLocalCommentaryResult(row, editMode, lang, src) {
  if (editMode) {
    const arabicHtml = renderEditableCommentaryLanguage(row, 'ar', src);
    const translationHtml = renderEditableCommentaryLanguage(row, 'en', src);
    return {
      html: renderLocalCommentaryPair(arabicHtml, translationHtml, localCommentaryLanguageMetadata('en')),
      bilingual: true,
...
function renderCommentaryText(text, footnotes, format, options = {}) {
  if (!text)
    return '';
  const renderedFootnotes = format === 'md' ? prepareMarkdownFootnotesForRendering(footnotes) : footnotes;
  let content = inlineFootnoteLinksToMarkdown([text, renderedFootnotes].filter(Boolean).map(stripPageMarkers).join('\n\n'));
  if (options.englishQuotationMarks)
...
function inlineFootnoteLinksToMarkdown(text) {
  const link = String.raw`\[[^\]\n]+\]\([^\s)]+(?:\s+["'][^"'\n]*["'])?\)`;
  const linkedInlineFootnote = new RegExp(String.raw`\[\^((?:${link})(?:\s*[;,؛،]\s*${link})*)\]`, 'g');
  return Utils.emptyIfNull(text).toString().replace(linkedInlineFootnote, '^[$1]');
}

function renderEnglishCommentaryMarkdown(markdown) {
  return renderCommentaryText(markdown, '', 'md', { englishQuotationMarks: true, quranBackticks: true });
}

function prepareMarkdownFootnotesForRendering(footnotes) {
  let inDefinition = false;
  return Utils.emptyIfNull(footnotes).toString().replace(/\r\n?/g, '\n').split('\n').map(line => {
    if (/^\[\^[^\]\n]+\]:/.test(line)) {
      inDefinition = true;
      return line;
    }
    if (!inDefinition || !line || /^(?: {4}|\t)/.test(line))
      return line;
    return `    ${line}`;
  }).join('\n');
}
...
function passageUrl(book, surah = 1, startAyah = 1, endAyah = startAyah, tafsirs) {
  const slug = book.slug || tafsirSlug(book.alias);
  const range = Number(endAyah) > Number(startAyah) ? `${Number(startAyah)}-${Number(endAyah)}` : `${Number(startAyah)}`;
  return `/quran/tafsir/${encodeURIComponent(slug)}/quran:${Number(surah)}:${range}`;
}

function browseUrl(book, surah = 1, ayah = 1, tafsirs) {
  return passageUrl(book, surah, ayah, ayah, tafsirs);
}

function renumberMarkdownFootnotes(text, footnotes) {
  text = Utils.emptyIfNull(text).toString().replace(/\r\n?/g, '\n');
  footnotes = Utils.emptyIfNull(footnotes).toString().replace(/\r\n?/g, '\n');
  const embedded = splitEmbeddedMarkdownFootnotes(text);
  const definitions = embedded.hasDefinitions ? embedded.definitions : markdownFootnoteDefinitions(footnotes);
  const textBody = embedded.hasDefinitions ? embedded.text : text;
  let referenceNumber = 0;
  const renumberedText = textBody.replace(/(?<!\\)\[\^([^\[\]\n]+)\]/g, function (marker, label) {
    return `[^${++referenceNumber}]`;
  });
  const renumberedFootnotes = definitions.map((definition, index) => {
    return [`[^${index + 1}]:${definition.lines[0] ? ` ${definition.lines[0]}` : ''}`, ...definition.lines.slice(1)].join('\n').trimEnd();
  }).join('\n');
  if (embedded.hasDefinitions) {
    return {
      text: [renumberedText.trimEnd(), renumberedFootnotes].filter(Boolean).join('\n\n'),
      footnotes: ''
    };
  }
  return { text: renumberedText, footnotes: renumberedFootnotes };
}

function splitEmbeddedMarkdownFootnotes(text) {
  const definitions = [];
  const body = [];
  let definition = null;
  for (const line of text.split('\n')) {
    const match = /^\[\^([^\[\]\n]+)\]:[ \t]?(.*)$/.exec(line);
    if (match) {
      definition = { label: match[1].trim(), lines: [match[2]] };
      definitions.push(definition);
    } else if (definition !== null && (!line.trim() || /^[ \t]+/.test(line))) {
  ...
  for (const item of definitions)
    item.lines = item.lines.slice(0, lastNonemptyLine(item.lines) + 1);
  return {
    definitions,
...
function markdownFootnoteDefinitions(footnotes) {
  const definitions = [];
  let definition = null;
...
module.exports = {
  DEFAULT_TAFSIR_ALIAS,
  browseUrl,
  carouselLanguage,
  commentaryCatalogRoles,
  commentaryLanguages,
  passageUrl,
  adjacentPassage,
  invocationBoundary,
  ...
  languageLabel,
  rawShortName,
  inlineFootnoteLinksToMarkdown,
  prepareMarkdownFootnotesForRendering,
  renumberMarkdownFootnotes,
  renderCommentaryText,
  renderEnglishCommentaryMarkdown,
  renderLocalCommentaryLanguage,
  resolveTafsir,
  localTranslationEntry,
  sitemapPassages,
  sourceLabel,
  splitTranslationRowByAyah,
  stripPageMarkers,
  tafsirEntries,
  tafsirSlug,
  uniqueCarouselTafsirs,
  visibleTafsirsSync,
  visibleTafsirs,
