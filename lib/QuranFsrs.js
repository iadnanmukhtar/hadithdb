...
// FSRS difficulty is a 1-10 recall-difficulty score. Above 6, keep the
// user-facing lifecycle in Weak while FSRS continues scheduling the card.
const WEAK_DIFFICULTY_THRESHOLD = 6;
const CORE_STABILITY_THRESHOLD_DAYS = 730;
const CORE_DIFFICULTY_MAXIMUM = 2;
...
function intervalModifier(settings) {
  return fsrs(parameters(settings)).intervalModifier;
}

function intervalForStability(stabilityValue, settings) {
  const stability = Math.max(0.001, Number(stabilityValue) || 0.001);
  return Math.min(36500, Math.max(1, Math.round(stability * intervalModifier(settings))));
}
...
function lifecycleAfter(existing, grade, card) {
  const coreEligible = ['review', 'weak', 'core'].includes(existing.lifecycle_state)
    && ['good', 'easy'].includes(grade)
    && Number(card.stability) > CORE_STABILITY_THRESHOLD_DAYS
    && Number(card.difficulty) < CORE_DIFFICULTY_MAXIMUM;
  if (coreEligible) return 'core';
  if (existing.lifecycle_state === 'weak') {
    return Number(card.difficulty) <= WEAK_DIFFICULTY_THRESHOLD
      && (grade === 'easy' || (grade === 'good' && Number(existing.consecutive_successes || 0) >= 1))
      ? 'review'
      : 'weak';
  }
  ...
  if (Number(card.difficulty) > WEAK_DIFFICULTY_THRESHOLD) return 'weak';
...
function schedule(existing, grade, settings, nowValue) {
  ...
  const scheduled = lifecycleState !== 'core';
  ...
  return {
    ...
    due: scheduled ? card.due : null,
    interval: scheduled ? Math.max(0, Math.round(dueMs / 86400000)) : 0,
    scheduled_days: scheduled ? Math.max(0, Number(card.scheduled_days) || Math.round(dueMs / 86400000)) : 0,
...
function initialAssessment(lifecycleState, settings, nowValue) {
  ...
  if (!grade) throw new TypeError('Weak, Good, or Easy is required for an initial FSRS assessment.');
...
module.exports = {
  CORE_DIFFICULTY_MAXIMUM,
  CORE_STABILITY_THRESHOLD_DAYS,
  FSRS_VERSION,
  WEAK_DIFFICULTY_THRESHOLD,
  effectiveWeights,
  initialAssessment,
