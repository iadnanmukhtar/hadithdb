...
let hdithBookIdColumnEnsured = false;
...
const REFERENCE_VIEWS = new Set(['item', 'section']);
const LEGACY_BOOK_ALIASES = new Map([
	['islamweb-history', 'history']
]);

function findByAlias(alias, books = global.books || []) {
	alias = LEGACY_BOOK_ALIASES.get(alias) || alias;
	const key = value => (value || '').toString().toLowerCase().replace(/[\p{P}\p{S}\s]+/gu, '');
	const normalized = key(alias);
	if (!normalized)
		return null;
	return books.find(book => book && book.alias === alias)
		|| books.find(book => book && key(book.alias) === normalized)
		|| null;
}
...
function normalizeBook(row, sourceTable) {
	...
	book.book_model = book.alias === 'quran' ? 'quran' : (book.type === 'sirah' ? 'sirah' : 'hadith');
	book.book_type = book.book_model;
...
function referenceView(book) {
	var properties = normalizeBookProperties(book && book.properties);
	var view = properties.reader && properties.reader.referenceView;
	return REFERENCE_VIEWS.has(view) ? view : 'item';
}
...
function isCommentaryBook(row) {
	...
	return type === 'tafsir' || type === 'trans' || type === 'sharh';
}
...
async function ensureHdithBookIdColumn() {
	if (hdithBookIdColumnEnsured)
		return;
	var rows = await global.query(`SHOW COLUMNS FROM books LIKE 'hdith_book_id'`);
	if (!rows.length)
		await global.query(`ALTER TABLE books ADD COLUMN hdith_book_id INT NULL AFTER id`);
	var indexes = await global.query(`SHOW INDEX FROM books WHERE Key_name='books_hdith_book_id'`);
	if (!indexes.length)
		await global.query(`ALTER TABLE books ADD UNIQUE KEY books_hdith_book_id (hdith_book_id)`);
	var mappings = await global.query(`SHOW TABLES LIKE 'hdith_book_mappings'`);
	if (mappings.length)
		await global.query(`UPDATE books b
