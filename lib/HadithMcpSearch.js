...
async function searchPage(options, context = {}) {
  ...
  let rawTotal = null;
  let exhausted = false;
  while (!exhausted && results.length < targetCount && rawOffset < MAX_SEARCH_RESULTS) {
    const rawResults = await search(options.query, options.filters || [], rawOffset, {
      ...(options.searchOptions || {}),
      resultSize: Math.min(MAX_RAW_RESULTS, MAX_SEARCH_RESULTS - rawOffset),
    ...
    if (Number.isFinite(rawResults && rawResults.total))
      rawTotal = Math.min(Number(rawResults.total), MAX_SEARCH_RESULTS);
