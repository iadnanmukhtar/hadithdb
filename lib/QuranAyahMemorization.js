...
function orderReviewItemsBySurah(items) {
  ...
  (items || []).forEach(item => {
    const stage = item.lifecycle_state || '';
    if (!stageGroups.has(stage)) stageGroups.set(stage, []);
    stageGroups.get(stage).push(item);
  });
  return Array.from(stageGroups.values()).flatMap(group => group.sort((left, right) =>
...
async function startReviewSession(userUid, settings, options) {
  ...
  const snapshot = await transaction(async function (q) {
    ...
    if (request.reviewUnit === 'ayah') {
      items = orderReviewItemsBySurah(await includeEnrolledAyatForSelectedPassages(
