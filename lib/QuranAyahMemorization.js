...
function assertUserStateTransition(currentState, nextState) {
  ...
  const err = new Error('An enrolled ayah can only be marked Easy, Paused, or Later. Mudhakkir grades manage its other learning stages.');
  err.status = 409;
  throw err;
}
...
async function selectReviewSessionItems(q, userUid, scheduledLimit, categoryLimitsValue, orderValue, refsValue) {
  ...
  // Once the due queue is complete, Review begins another deliberate
...
async function pauseReviewSession(userUid, sessionId) {
  ...
  return transaction(async function (q) {
    ...
    if (!session) {
      const err = new Error('Mudhakkir session not found.');
      err.status = 404;
      throw err;
    }
...
async function nextSessionItem(userUid, sessionId, dayStartValue) {
  ...
  const result = await transaction(async function (q) {
    ...
    // session. A custom surah review deliberately includes every state except
