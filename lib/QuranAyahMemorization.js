...
function reviewSessionRequest(options) {
  ...
  if (!['regular', 'surah', 'page', 'passage'].includes(rawMode)) {
    const err = new Error('Review mode must be regular, surah, page, or passage.');
    err.status = 400;
    throw err;
  }
  if (rawMode === 'regular') return { mode: 'regular', reviewUnit: 'ayah', surahNumber: null, pageLimit: null, pageNumber: null, startRef: null, continueForward: false };
  if (rawMode === 'passage') {
    const startRef = parseRefString(options && options.startRef);
    if (!startRef) {
      const err = new Error('A valid Quran ayah is required for a Passage Review.');
      err.status = 400;
      throw err;
    }
    return { mode: 'passage', reviewUnit: 'ayah', surahNumber: startRef.surah, pageLimit: null, pageNumber: null, startRef: `${startRef.surah}:${startRef.ayah}`, continueForward: options && options.continueForward === true };
  }
  if (rawMode === 'page') {
    ...
    return { mode: 'page', reviewUnit: 'ayah', surahNumber: null, pageLimit: null, pageNumber, startRef: null, continueForward: options && options.continueForward === true };
  }
  ...
  const reviewUnit = (options && options.reviewUnit || 'ayah').toString().trim().toLowerCase();
  if (!['ayah', 'passage'].includes(reviewUnit)) {
    const err = new Error('Choose āyah-by-āyah or passage-by-passage review.');
    err.status = 400;
    throw err;
  }
  return { mode: 'surah', reviewUnit, surahNumber: ref.surah, reviewType, pageLimit: reviewType === 'all' ? null : 0, pageNumber: null, startRef: null, continueForward: false };
}
...
function passageKeysForItems(surahNumber, items, sectionsBySurah, subsectionsBySurah) {
...
async function startReviewSession(userUid, settings, options) {
  ...
  const passageScope = request.mode === 'passage'
    ...
    : null;
  const passageCatalog = request.reviewUnit === 'passage' ? await Promise.all([
    QuranTocSubdivisions.quranSectionRangesBySurah(),
    QuranTocSubdivisions.quranSubsectionRangesBySurah()
  ]) : null;
  ...
  const snapshot = await transaction(async function (q) {
    ...
    if (request.reviewUnit === 'passage') passageKeys = passageKeysForItems(request.surahNumber, items, passageCatalog[0], passageCatalog[1]);
    await q(`
      ...
      VALUES (${sql(sessionId)},${sql(userUid)},${scheduledLimit},${limits.learning},${limits.relearning},${limits.weak},${limits.memorized},${timeBudget},
        ...
        ${request.pageNumber === null ? 'NULL' : request.pageNumber},${request.continueForward ? 1 : 0},${request.pageNumber === null ? 'NULL' : request.pageNumber},${sql(passageScope && passageScope.end_ref)})
    `);
    if (items.length) {
...
async function extendForwardReviewSession(q, userUid, session) {
  if (Number(session.continue_forward) !== 1 || !['page', 'passage'].includes(session.session_mode)) return false;
  let scope;
  let nextPage = null;
  ...
  const values = items.map((row, index) => `(${sql(session.session_id)},${sql(userUid)},${Number(row.surah_number)},${Number(row.ayah_number)},${sql(row.lifecycle_state)},${firstPosition + index})`);
  ...
  const result = await transaction(async function (q) {
    ...
    if (session.review_unit === 'passage' && item.unit_key) {
      unitItems = await q(`
        ...
        ORDER BY queue_position FOR UPDATE
      `);
    }
