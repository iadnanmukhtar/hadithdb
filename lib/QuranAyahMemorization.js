...
function assertUserStateTransition(currentState, nextState) {
  ...
  const err = new Error('An enrolled ayah can only be marked Core, Paused, or Later. Review grades manage its other learning stages.');
  err.status = 409;
  throw err;
}
  ...
  const changedCount = await transaction(async function (q) {
    ...
    refs.forEach(ref => {
      ...
      if (!bulkStateTransitionAllowed(current.lifecycle_state, state)) {
        const err = new Error('An enrolled page or surah can only be marked Core, Paused, or Later.');
        err.status = 409;
        throw err;
      }
...
async function setProgressGroupState(userUid, groupKeyValue, stateValue) {
  ...
  if (!ENROLLED_BULK_USER_SELECTABLE_LIFECYCLE_STATES.has(state)) {
    const err = new Error('A page or short surah can only be marked Core, Paused, or Later from Memorization Progress.');
    err.status = 400;
    throw err;
  }
...
async function enrollReviewScope(q, userUid, refsValue) {
  ...
  const existing = await q(`
    ...
    FOR UPDATE
  `) || [];
  const enrolledCount = refs.length - existing.filter(row => row.lifecycle_state !== 'later').length;
  ...
  `);
  return Math.max(0, enrolledCount);
}
...
async function startReviewSession(userUid, settings, options) {
  ...
  const snapshot = await transaction(async function (q) {
    ...
    const enrolledCount = request.mode === 'regular'
      ? 0
