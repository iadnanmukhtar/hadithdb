...
const LIFECYCLE_STATES = new Set(['later', 'learning', 'weak', 'review', 'core', 'relearning', 'suspended']);
const USER_SELECTABLE_LIFECYCLE_STATES = new Set(['later', 'learning', 'weak', 'review', 'core', 'suspended']);
...
const SURAH_REVIEW_PAGE_CHOICES = Object.freeze([1, 2, 3, 4, 5, 10, 20]);
...
// Canonical Hafs ayah counts. Keeping this immutable list here makes reference
// validation strict even during early startup, before Globals has loaded surah
// metadata or a database connection is available.
...
function sql(value) {
  return value === null || value === undefined ? 'NULL' : MySQL.escape(value);
}
...
function parseRef(surahValue, ayahValue) {
  ...
  if (!surah || !ayah || surah > CANONICAL_AYAH_COUNTS.length || ayah > CANONICAL_AYAH_COUNTS[surah - 1])
    return null;
  return { surah, ayah };
}
    ...
    ensurePromise = (async function () {
      ...
      await global.query(`
        CREATE TABLE IF NOT EXISTS ${SESSION_TABLE} (
          ...
          session_mode ENUM('regular','surah','page') NOT NULL DEFAULT 'regular',
...
function decorate(row) {
  if (!row) return row;
  row.lifecycle_label = STATE_LABELS[row.lifecycle_state] || row.lifecycle_state;
  const scheduledDue = row.next_review_at && new Date(row.next_review_at).getTime() <= Date.now();
  ...
  row.due = initialized && REVIEWABLE_LIFECYCLE_STATES.has(row.lifecycle_state) && scheduledDue ? 1 : 0;
  return row;
}
...
function stateUpdateValues(existing, nextState, fsrsSettings) {
  ...
  if (nextState === 'learning') {
    ...
    values.learning_started_at = existing.learning_started_at || new Date();
    ...
    const assessment = QuranFsrs.initialAssessment('weak', fsrsSettings);
    values.learning_progress = existing.learning_progress || 'nearly_memorized';
    values.learning_started_at = existing.learning_started_at || new Date();
    values.learning_last_worked_at = new Date();
    values.fully_memorized_at = existing.fully_memorized_at || new Date();
...
function shouldQueueReviewRetry(grade, attempts) {
  return grade === 'again' && Number(attempts) === 0;
}
...
async function setProgressGroupState(userUid, groupKeyValue, stateValue) {
  ...
  const result = await setRefsState(userUid, group.member_refs, stateValue);
...
function buildProgressGroups(definitions, rows, nowValue) {
  ...
  (rows || []).forEach(row => {
    const state = (row.lifecycle_state || '').toString();
    if (!['learning', 'weak', 'review', 'core', 'relearning', 'suspended'].includes(state)) return;
    ...
    if (REVIEWABLE_LIFECYCLE_STATES.has(state) && Number(row.fsrs_state) === 0) group.new_count += 1;
    if (REVIEWABLE_LIFECYCLE_STATES.has(state) && [1, 2, 3].includes(Number(row.fsrs_state)) && row.next_review_at) {
      ...
      if (Number.isFinite(reviewTime)) {
        ...
        if (Number.isFinite(now) && reviewTime <= now) group.due_count += 1;
      }
    }
  });
...
async function progressGroups(userUid) {
  ...
  const [definitions, rows] = await Promise.all([
    ...
    global.query(`
      SELECT surah_number, ayah_number, lifecycle_state, next_review_at, review_count, fsrs_state
...
async function progress(userUid) {
    ...
    SELECT lifecycle_state, COUNT(*) AS count,
      SUM(CASE
        WHEN lifecycle_state IN ('learning','weak','review','relearning')
          AND fsrs_state IN (1,2,3) AND next_review_at<=NOW() THEN 1
        ELSE 0
      END) AS due_count,
  ...
  let explicit = 0;
  rows.forEach(row => {
    counts[row.lifecycle_state] = Number(row.count) || 0;
    explicit += Number(row.count) || 0;
...
function reviewSessionLimits(settings) {
  ...
  return {
    ...
    relearning: boundedSessionLimit(memorization.relearningLimit, REVIEW_SESSION_DEFAULTS.relearning, 50),
    weak: boundedSessionLimit(memorization.weakLimit, REVIEW_SESSION_DEFAULTS.weak, 50),
...
function normalizeSelectionLimits(value, total) {
  ...
  return {
    ...
    relearning: boundedSessionLimit(source.relearning, REVIEW_SESSION_DEFAULTS.relearning, 50),
    weak: boundedSessionLimit(source.weak, REVIEW_SESSION_DEFAULTS.weak, 50),
...
async function reviewCategoryCandidates(q, userUid, category, limit, order, future) {
  if (limit < 1) return [];
  ...
  const ordering = future
    ? `CASE WHEN last_reviewed_at>=CURDATE() THEN 1 ELSE 0 END, ${order}`
    : state === 'weak' || state === 'review'
      ? `CASE WHEN next_review_at<CURDATE() THEN 0 ELSE 1 END, ${order}`
      : order;
  return await q(`
    ...
    ORDER BY CASE WHEN fsrs_state=0 THEN 0 ELSE 1 END, ${ordering}
    LIMIT ${limit} FOR UPDATE
  `) || [];
}

async function selectReviewSessionItems(q, userUid, scheduledLimit, categoryLimitsValue, orderValue) {
  ...
  for (const category of categoryOrder) {
    candidates[category] = await reviewCategoryCandidates(q, userUid, category, limits[category], order, false);
  }
  ...
  if (!dueCount) {
    ...
    for (const category of freshOrder) {
      futureCandidates[category] = await reviewCategoryCandidates(q, userUid, category, limits[category], order, true);
    }
...
async function surahReviewScope(surahNumber, pageLimit) {
  const definitions = await progressGroupDefinitions();
  ...
  const availablePageChoices = SURAH_REVIEW_PAGE_CHOICES.filter(count => count <= totalPages);
  if (pageLimit !== null && !availablePageChoices.includes(pageLimit)) {
    const pageChoiceText = availablePageChoices.map(count => `${count} page${count === 1 ? '' : 's'}`).join(', ');
    const err = new Error(`Choose ${pageChoiceText}, or all memorized āyāt, for this Surah Review.`);
    err.status = 400;
    throw err;
  }
  const selectedPages = pageLimit === null
    ? new Set(groups.map(group => Number(group.page_number)))
    : new Set(Array.from(new Set(groups.map(group => Number(group.page_number)))).slice(0, pageLimit));
  return {
    refs: groups.filter(group => selectedPages.has(Number(group.page_number)))
      .flatMap(group => group.member_refs || []),
...
function reviewSessionRequest(options) {
  const rawMode = !options || options.mode === undefined
    ? 'regular'
    : (options.mode || '').toString().trim().toLowerCase();
  if (!['regular', 'surah', 'page'].includes(rawMode)) {
    const err = new Error('Review mode must be regular, surah, or page.');
    err.status = 400;
    throw err;
  }
  if (rawMode === 'regular') return { mode: 'regular', surahNumber: null, pageLimit: null, pageNumber: null };
  if (rawMode === 'page') {
    const pageNumber = strictInteger(options && options.pageNumber);
    if (!pageNumber || pageNumber > 604) {
      const err = new Error('A valid Mushaf page from 1 through 604 is required for a Page Review.');
      err.status = 400;
      throw err;
    }
    return { mode: 'page', surahNumber: null, pageLimit: null, pageNumber };
  }
  const ref = parseRef(options && options.surahNumber, 1);
  if (!ref) {
    const err = new Error('A valid surah is required for a Surah Review.');
    err.status = 400;
    throw err;
  }
  const rawPageLimit = options && options.pageLimit;
  const pageLimit = rawPageLimit === undefined || rawPageLimit === null || rawPageLimit === ''
    ? null
    : strictInteger(rawPageLimit);
  if (rawPageLimit !== undefined && rawPageLimit !== null && rawPageLimit !== '' && !pageLimit) {
    const err = new Error('The number of Surah Review pages must be a positive whole number.');
    err.status = 400;
    throw err;
  }
  return { mode: 'surah', surahNumber: ref.surah, pageLimit, pageNumber: null };
}
...
async function startReviewSession(userUid, settings, options) {
  ...
  const surahScope = request.mode === 'surah'
    ? await surahReviewScope(request.surahNumber, request.pageLimit)
    : null;
  ...
  const snapshot = await transaction(async function (q) {
    // Serialize session replacement per user. A deliberate new session
    // supersedes any abandoned active session, while its immutable item list
    // keeps limits stable for the life of the session.
    ...
    if (active.length) {
      await q(`
        UPDATE ${SESSION_TABLE} SET completed_at=NOW()
        WHERE user_uid=${sql(userUid)} AND completed_at IS NULL AND paused_at IS NULL
      `);
    }
    const selected = request.mode === 'surah'
      ...
      : request.mode === 'page'
        ? await selectSurahReviewSessionItems(q, userUid, null, pageScope.refs)
        : await selectReviewSessionItems(q, userUid, scheduledLimit, limits, order);
    const items = selected.items || selected.learning.concat(selected.relearning, selected.weak, selected.memorized);
    if (request.mode !== 'regular' && !items.length) {
      const err = new Error(request.mode === 'page'
        ? 'This Mushaf page has no memorized āyāt to review.'
        : 'This surah has no memorized āyāt to review.');
      err.status = 409;
      throw err;
    }
    if (request.mode !== 'regular') scheduledLimit = items.length;
    await q(`
      INSERT INTO ${SESSION_TABLE}
        (session_id,user_uid,scheduled_limit,learning_limit,relearning_limit,weak_limit,memorized_limit,time_budget_minutes,
         session_mode,review_surah_number,review_page_limit,review_page_number)
      VALUES (${sql(sessionId)},${sql(userUid)},${scheduledLimit},${limits.learning},${limits.relearning},${limits.weak},${limits.memorized},${timeBudget},
        ${sql(request.mode)},${request.surahNumber === null ? 'NULL' : request.surahNumber},${request.pageLimit === null ? 'NULL' : request.pageLimit},
        ${request.pageNumber === null ? 'NULL' : request.pageNumber})
    `);
    if (items.length) {
      const values = items.map((row, index) => `(${sql(sessionId)},${sql(userUid)},${Number(row.surah_number)},${Number(row.ayah_number)},${sql(row.lifecycle_state)},${index + 1})`);
      await q(`INSERT INTO ${SESSION_ITEM_TABLE} (session_id,user_uid,surah_number,ayah_number,source_state,queue_position) VALUES ${values.join(',')}`);
    }
    return Object.assign({}, selected, { fresh: selected.fresh, superseded: active.length });
  });
...
async function activeReviewSession(userUid) {
  ...
  const sessions = await global.query(`
    SELECT session_id, scheduled_limit, learning_limit, relearning_limit, weak_limit, memorized_limit,
      ...
      weak_reviewed, memorized_reviewed, session_mode, review_surah_number, review_page_limit, review_page_number, started_at
    FROM ${SESSION_TABLE}
    WHERE user_uid=${sql(userUid)} AND completed_at IS NULL AND paused_at IS NULL
  ...
  `) || [];
  const counts = new Map(rows.map(row => [row.source_state, Math.max(0, Number(row.queued) || 0)]));
  session.learning_queued = counts.get('learning') || 0;
  session.relearning_queued = counts.get('relearning') || 0;
  session.weak_queued = counts.get('weak') || 0;
  session.memorized_queued = counts.get('review') || 0;
  session.core_queued = counts.get('core') || 0;
  session.paused_queued = counts.get('suspended') || 0;
...
async function resumeReviewSession(userUid, sessionId) {
  ...
  return transaction(async function (q) {
    ...
    if (!session.paused_at) return { session_id: normalizedId, resumed: true };
    const active = await q(`
      SELECT session_id FROM ${SESSION_TABLE}
      WHERE user_uid=${sql(userUid)} AND completed_at IS NULL AND paused_at IS NULL
    ...
    `) || [];
    if (active.length) {
      const err = new Error('Pause or end your current review session before resuming this one.');
      err.status = 409;
      throw err;
    }
    await q(`
    ...
    `);
    return { session_id: normalizedId, resumed: true };
  });
}
...
async function deletePausedReviewSession(userUid, sessionId) {
  await ensureTables();
  const normalizedId = (sessionId || '').toString().trim();
  return transaction(async function (q) {
    const rows = await q(`SELECT * FROM ${SESSION_TABLE} WHERE session_id=${sql(normalizedId)} AND user_uid=${sql(userUid)} LIMIT 1 FOR UPDATE`);
    const session = rows[0];
    if (!session) {
      const err = new Error('Paused review session not found.');
      err.status = 404;
      throw err;
    }
    if (session.completed_at || !session.paused_at) {
      const err = new Error('Only a paused review session can be deleted.');
      err.status = 409;
      throw err;
    }
    await q(`DELETE FROM ${SESSION_ITEM_TABLE} WHERE session_id=${sql(normalizedId)} AND user_uid=${sql(userUid)}`);
    await q(`DELETE FROM ${SESSION_TABLE} WHERE session_id=${sql(normalizedId)} AND user_uid=${sql(userUid)}`);
    return { session_id: normalizedId, deleted: true };
  });
}

async function nextSessionItem(userUid, sessionId) {
  await ensureTables();
  const result = await transaction(async function (q) {
    ...
    const timedOut = Number(session.time_budget_minutes) > 0
      && Date.now() - new Date(session.started_at).getTime() >= Number(session.time_budget_minutes) * 60000;
    if (timedOut) {
    ...
    if (!item.current_token) {
      item.current_token = crypto.randomUUID();
      await q(`UPDATE ${SESSION_ITEM_TABLE} SET current_token=${sql(item.current_token)}, presented_at=NOW() WHERE id=${Number(item.id)}`);
    }
    return { session, item, complete: false };
  });
  if (!result.item) return result;
  result.reviewed_refs = await reviewedRefsForSession(userUid, sessionId);
  const stateRows = await getMany(userUid, [`${result.item.surah_number}:${result.item.ayah_number}`]);
  const ayah = (await metadataForRows(stateRows))[0];
  Object.assign(ayah, {
  ...
  return { session: result.session, ayah, complete: false };
}

async function reviewedRefsForSession(userUid, sessionId) {
  const rows = await global.query(`
    SELECT DISTINCT memory.surah_number, memory.ayah_number
    FROM ${TABLE} memory
    WHERE memory.user_uid=${sql(userUid)} AND memory.last_reviewed_at>=CURDATE()
      AND NOT EXISTS (
...
async function activeSessionReviewedRefs(userUid, surahValue, ayahValue) {
  await ensureTables();
  const ref = await assertCanonicalRef(surahValue, ayahValue);
  const sessions = await global.query(`
  ...
  return reviewedRefsForSession(userUid, sessions[0].session_id);
}

async function undoableSessionReview(userUid, sessionId) {
  const rows = await global.query(`
    SELECT history.attempt_token, history.surah_number, history.ayah_number, history.grade
    FROM ${SESSION_TABLE} session
...
async function activeSessionReviewState(userUid, surahValue, ayahValue) {
  await ensureTables();
  const ref = await assertCanonicalRef(surahValue, ayahValue);
  const rows = await global.query(`
    SELECT session.session_id, current_item.current_token AS attempt_token
    FROM ${SESSION_TABLE} session
    JOIN ${SESSION_ITEM_TABLE} current_item ON current_item.session_id=session.session_id
      AND current_item.user_uid=session.user_uid
    WHERE session.user_uid=${sql(userUid)} AND session.completed_at IS NULL AND session.paused_at IS NULL
  ...
  return {
    ...
    attempt_token: rows[0].attempt_token,
    reviewed_refs: await reviewedRefsForSession(userUid, rows[0].session_id),
...
async function recordSessionReview(userUid, sessionId, surahValue, ayahValue, gradeValue, options) {
  ...
  return transaction(async function (q) {
    ...
    if (!item || !['queued', 'retry_pending'].includes(item.item_state)
      || !attemptToken || item.current_token !== attemptToken) {
      const err = new Error('This review attempt has already been recorded or is no longer current.');
      err.status = 409;
      throw err;
    }
    const firstAttempt = Number(item.attempts) === 0;
    if (grade === 'skip') {
      await q(`
      ...
      `);
      return { skipped: true, ayah: null, already_recorded: false };
    }
    const rows = await q(`
      SELECT * FROM ${TABLE}
      WHERE user_uid=${sql(userUid)} AND surah_number=${ref.surah} AND ayah_number=${ref.ayah}
    ...
    if (!existing || !allowedStates.has(existing.lifecycle_state)) {
      // The ayah may have been moved after this attempt was presented. Consume
      // the stale snapshot item without writing review history, then let the
      // client advance normally to the next eligible ayah.
      await q(`
        UPDATE ${SESSION_ITEM_TABLE}
        ...
        WHERE id=${Number(item.id)}
      `);
      return { skipped: true, stale: true, ayah: null, already_recorded: false };
    }
    const elapsedDays = elapsedDaysSince(existing.last_reviewed_at || existing.fully_memorized_at);
    const schedule = reviewSchedule(existing, grade, options && options.fsrs);
    const undoSnapshot = reviewUndoSnapshot(existing, item);
    ...
    await q(`
      ...
      VALUES (${sql(userUid)},${ref.surah},${ref.ayah},${sql(grade)},${sql(existing.lifecycle_state)},${sql(schedule.lifecycle_state)},
    ...
    `);
    const retry = shouldQueueReviewRetry(grade, item.attempts);
    await q(`
      UPDATE ${SESSION_ITEM_TABLE}
      SET item_state=${sql(retry ? 'retry_pending' : 'reviewed')}, attempts=attempts+1,
        current_token=NULL, presented_at=NULL, last_attempt_token=${sql(attemptToken)},
        last_attempt_grade=${sql(grade)}, last_retry_queued=${retry ? 1 : 0}
      WHERE id=${Number(item.id)}
    `);
    ...
    if (firstAttempt) {
      ...
      sessionUpdates.push(...counters.map(counter => `${counter}=${counter}+1`));
    }
    ...
    return {
      ...
      undoable_review: { attempt_token: attemptToken, reference: `${ref.surah}:${ref.ayah}`, grade }
    };
  }).then(async function (result) {
    if (!result.skipped) result.ayah = await get(userUid, ref.surah, ref.ayah);
    return result;
  });
}
...
async function optimizeFsrsParameters(userUid) {
  await ensureTables();
  const rows = await global.query(`
    SELECT surah_number, ayah_number, reviewed_at, grade
    FROM ${HISTORY_TABLE}
    WHERE user_uid=${sql(userUid)}
...
async function nextReview(userUid, excludedRefs, settings, counters) {
  ...
  if (totalReviewed < scheduledLimit && relearningReviewed < limits.relearning) allowed.push("'relearning'");
  if (totalReviewed < scheduledLimit && weakReviewed < limits.weak) allowed.push("'weak'");
  ...
  const rows = await global.query(`
    ...
    ORDER BY
      ...
      CASE WHEN lifecycle_state='review' AND next_review_at<CURDATE() THEN 0 ELSE 1 END,
