...
const INITIAL_USER_SELECTABLE_LIFECYCLE_STATES = new Set(['learning', 'weak', 'review', 'core']);
const ENROLLED_USER_SELECTABLE_LIFECYCLE_STATES = new Set(['later', 'suspended']);
...
function userStateTransitionAllowed(currentStateValue, nextStateValue) {
  ...
  return currentState === 'later'
    ...
    : ENROLLED_USER_SELECTABLE_LIFECYCLE_STATES.has(nextState);
}

function assertUserStateTransition(currentState, nextState) {
  if (userStateTransitionAllowed(currentState, nextState)) return;
  const err = new Error('An enrolled ayah can only be Paused or moved to Later. Review grades manage its learning stage.');
  err.status = 409;
  throw err;
}
...
async function setRefsState(userUid, refsValue, stateValue) {
  ...
  const changedCount = await transaction(async function (q) {
    ...
    refs.forEach(ref => {
      ...
      assertUserStateTransition(current.lifecycle_state, state);
...
async function setProgressGroupState(userUid, groupKeyValue, stateValue) {
  ...
  const state = (stateValue || '').toString().trim().toLowerCase();
  if (!ENROLLED_USER_SELECTABLE_LIFECYCLE_STATES.has(state)) {
    const err = new Error('A page or short surah can only be Paused or moved to Later from Memorization Progress.');
    err.status = 400;
    throw err;
  }
...
function buildProgressGroups(definitions, rows, nowValue) {
  ...
  (rows || []).forEach(row => {
    ...
    if (REVIEWABLE_LIFECYCLE_STATES.has(lifecycleState) && Number(row.fsrs_state) === 0) group.new_count += 1;
...
function fillUnusedCapacityWithDueCards(selected, candidates, totalLimit, categoryOrder) {
...
async function selectReviewSessionItems(q, userUid, scheduledLimit, categoryLimitsValue, orderValue) {
  ...
  for (const category of categoryOrder) {
    candidates[category] = await reviewCategoryCandidates(q, userUid, category, totalLimit, order, false);
  }
  let selected = allocateReviewCandidates(candidates, limits, totalLimit, categoryOrder);
  selected = fillUnusedCapacityWithDueCards(selected, candidates, totalLimit, categoryOrder);
  let fresh = false;
...
async function surahReviewScope(surahNumber, pageLimit) {
  ...
  if (pageLimit !== null && !availablePageChoices.includes(pageLimit)) {
    ...
    const err = new Error(`Choose ${pageChoiceText}, or all āyāt, for this Surah Review.`);
    err.status = 400;
    throw err;
  }
...
async function enrollReviewScope(q, userUid, refsValue) {
  ...
  const values = refs.map(ref => `(${sql(userUid)},${ref.surah},${ref.ayah},'learning','started',NOW(),NOW(),${QuranFsrs.FSRS_VERSION})`);
  const result = await q(`
    INSERT INTO ${TABLE}
      (user_uid,surah_number,ayah_number,lifecycle_state,learning_progress,
...
async function startReviewSession(userUid, settings, options) {
  ...
  const snapshot = await transaction(async function (q) {
    ...
    if (request.mode !== 'regular')
      await enrollReviewScope(q, userUid, request.mode === 'surah' ? surahScope.refs : pageScope.refs);
