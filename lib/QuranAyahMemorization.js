...
function orderReviewItemsBySurah(items) {
  ...
  (items || []).forEach(item => {
    ...
    if (!surahGroups.has(surahNumber)) surahGroups.set(surahNumber, []);
    surahGroups.get(surahNumber).push(item);
  });
  return Array.from(surahGroups.values()).flatMap(group => group.sort((left, right) =>
...
function reviewSessionRequest(options) {
  ...
  if (rawMode === 'passage') {
    ...
    return { mode: 'passage', reviewUnit, surahNumber: startRef.surah, pageLimit: null, pageNumber: null, startRef: `${startRef.surah}:${startRef.ayah}`, continueForward: options && options.continueForward === true };
  }
  if (rawMode === 'page') {
    ...
    return { mode: 'page', reviewUnit, surahNumber: null, pageLimit: null, pageNumber, startRef: null, continueForward: options && options.continueForward === true };
  }
  ...
  return { mode: 'surah', reviewUnit, surahNumber: ref.surah, reviewType, pageLimit: reviewType === 'all' ? null : 0, pageNumber: null, startRef: null, continueForward: options && options.continueForward === true };
}
...
async function startReviewSession(userUid, settings, options) {
  ...
  const snapshot = await transaction(async function (q) {
    ...
    const items = orderReviewItemsBySurah(
