...
function reviewSessionRequest(options) {
  ...
  const reviewType = (options && options.reviewType || '').toString().trim().toLowerCase();
  if (!['all', 'regular'].includes(reviewType)) {
    const err = new Error('Choose Review all āyāt or Regular review for this Surah Review.');
    err.status = 400;
    throw err;
  }
  return { mode: 'surah', surahNumber: ref.surah, reviewType, pageLimit: reviewType === 'all' ? null : 0, pageNumber: null };
}
...
async function startReviewSession(userUid, settings, options) {
  ...
  const snapshot = await transaction(async function (q) {
    ...
    const enrolledCount = request.mode === 'page' || (request.mode === 'surah' && request.reviewType === 'all')
