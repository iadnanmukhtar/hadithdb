...
const RE_TASHKIL = /[\u0600-\u061f\u064b-\u0652\u0657-\u065f\u066b-\u066d\u06d6-\u06ed\u08d3-\u08ff]/g;
const MAX_STEM_SIZE = 3;
const ABJAD_LETTERS = 'أبجدهوزحطيكلمنسعفصقرشتثخذضظغ';
...
class Arabic {
	...
	static normalize(s, keepHamzah) {
		if (s) {
			...
			if (keepHamzah) {
				...
				s = s.replace(/ء/g, '');
				s = s.replace(/[\u0654\u0655\u0674]/g, '');
				s = s.replace(/([آأإٲٳٵ]|ا[\u0653\u0654\u0655\u0674]|\u0654?ا)/g, 'ا');
	...
	static replaceDagger(s) {
		if (s) {
			s = s.replace(/ى\u0670/g, 'ا');
			s = s.replace(/ي\u0670/g, 'ا');
			s = s.replace(/\u0670/g, 'ا').replace(/ا+/g, 'ا');
			s = s.replace(/\u0656/g, 'ي');
		}
		return s;
	}
	...
	static toArabicDigits(s) {
		s = (''+s)
			.replace(/(?<=[0-9٠-٩۰-۹])[a-z]+/gi, suffix => suffix.toLowerCase().replace(/[a-z]/g, letter => ABJAD_LETTERS[LATIN_LETTERS.indexOf(letter)]))
	...
	static toLatinDigits(s) {
		const arabicOffset = '٠'.charCodeAt();
		const persianOffset = '۰'.charCodeAt();
		s = (''+s)
			.replace(/[٠-٩]/g, d => '0123456789'[d.charCodeAt()-arabicOffset])
			.replace(/[۰-۹]/g, d => '0123456789'[d.charCodeAt()-persianOffset])
			.replace(/(?<=\d)[أابجدهوزحطيكلمنسعفصقرشتثخذض]+/g, suffix => suffix.replace(/[أابجدهوزحطيكلمنسعفصقرشتثخذض]/g, letter => letter === 'ا' ? 'a' : LATIN_LETTERS[ABJAD_LETTERS.indexOf(letter)]));
		return s;
	}
	...
	static arabizi2ALALC(s) {
		if (s) {
			...
			s = s.replace(/AAA/gu, 'Ā');
			s = s.replace(/aaa/gu, 'ā');

			s = s.replace(/III/gu, 'Ī');
			s = s.replace(/iii/gu, 'ī');

			s = s.replace(/UUU/gu, 'Ū');
			s = s.replace(/uuu/gu, 'ū');
	...
	static toALALCName(s) {
		var name = Arabic.toALALC(s || '').trim();
		var words = name.split(/([ \-])/);
		for (var w = 0; w < words.length; w++) {
			var word = words[w].split('');
			var i = word[0] === 'ʿ' || word[0] === 'ʾ' ? 1 : 0;
			if (word.length > i)
				word[i] = word[i].toUpperCase();
			words[w] = word.join('');
		}
		return words.join('').replace(/Al-/g, 'al-').replace(/B\./g, 'b.');
	}
