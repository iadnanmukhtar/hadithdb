...
class Arabic {
	...
	static arabizi2ALALC(s) {
		if (s) {
			s = s.replace(/3(\w)/gu, 'ʿ$1'); // `ayn
			s = s.replace(/(\w)3/gu, '$1ʿ'); // `ayn
	  
			s = s.replace(/2(\w)/gu, 'ʾ$1'); // hamzah
			s = s.replace(/(\w)2/gu, '$1ʾ'); // hamzah
	  
			s = s.replace(/\^6'(\w)/gu, 'Ẓ$1'); // Ẓa
			s = s.replace(/(\w)\^6'/gu, '$1Ẓ'); // Ẓa
			s = s.replace(/6'(\w)/gu, 'ẓ$1'); // ẓa
			s = s.replace(/(\w)6'/gu, 'ẓ1ṭ'); // ẓa
	  
			s = s.replace(/\^6(\w)/gu, 'Ṭ$1'); // Ṭa
			s = s.replace(/(\w)\^6/gu, '$1Ṭ'); // Ṭa
			s = s.replace(/6(\w)/gu, 'ṭ$1'); // ṭa
			s = s.replace(/(\w)6/gu, '$1ṭ'); // ṭa

			s = s.replace(/\^7(\w)/gu, 'Ḥ$1'); // Ṭa
			s = s.replace(/(\w)\^7/gu, '$1Ḥ'); // Ṭa
			s = s.replace(/7(\w)/gu, 'ḥ$1'); // ṭa
			s = s.replace(/(\w)7/gu, '$1ḥ'); // ṭa
	  
			s = s.replace(/\^9'(\w)/gu, 'Ḍ$1'); // Ḍād
			s = s.replace(/(\w)\^9'/gu, '$1Ḍ'); // Ḍād
			s = s.replace(/9'(\w)/gu, 'ḍ$1'); // ḍād
			s = s.replace(/(\w)9'/gu, '$1ḍ'); // ḍād
			
			s = s.replace(/\^9(\w)/gu, 'Ṣ$1'); // Ṣād
			s = s.replace(/(\w)\^9/gu, '$1Ṣ'); // Ṣād
			s = s.replace(/9(\w)/gu, 'ṣ$1'); // ṣād
			s = s.replace(/(\w)9/gu, '$1ṣ'); // ṣād
