...
function normalizeQuranHelpTours(value) {
  const allowed = new Set(['mushaf', 'memorize', 'review', 'progress', 'study', 'tafsir']);
...
function normalizeSettings(settings) {
  ...
  const memorization = source.memorization && typeof source.memorization === 'object' && !Array.isArray(source.memorization)
    ? source.memorization
    : {};
  const weakLimit = normalizeReviewCategoryLimit(
    memorization.weakLimit === undefined ? memorization.relearningLimit : memorization.weakLimit,
    3,
    50
  );
