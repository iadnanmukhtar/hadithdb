...
function normalizeReviewDays(value, fallback) {
...
function normalizeReviewLimit(value) {
  const parsed = parseInt(value, 10);
  return Number.isInteger(parsed) && parsed >= 1 && parsed <= 200 ? parsed : 5;
}

function normalizeReviewOrder(value) {
  const order = (value || '').toString().trim();
  return ['hard_good_easy', 'hard_easy_good', 'due_first', 'page_order', 'random'].includes(order)
    ? order
    : 'hard_good_easy';
}
...
function normalizeSettings(settings) {
  ...
  normalized.memorization = {
    goodReviewDays: normalizeReviewDays(memorization.goodReviewDays, 7),
    easyReviewDays: normalizeReviewDays(memorization.easyReviewDays, 14),
