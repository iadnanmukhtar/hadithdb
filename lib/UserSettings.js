  ...
  ensurePromise = (async () => {
    await global.query(`
      CREATE TABLE IF NOT EXISTS ${TABLE} (
        ...
        user_photo varchar(1024) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    ...
    const photoColumn = await global.query(`
      SELECT COLUMN_NAME
      FROM INFORMATION_SCHEMA.COLUMNS
      WHERE TABLE_SCHEMA=DATABASE()
        AND TABLE_NAME='${TABLE}'
        AND COLUMN_NAME='user_photo'
      LIMIT 1
    `);
    if (!photoColumn || photoColumn.length < 1) {
      await global.query(`
        ALTER TABLE ${TABLE}
...
async function getLoginUser(userId) {
  ...
  const userSql = MySQL.escape(userId);
  const rows = await global.query(`
    SELECT user_uid, user_provider, user_name, user_email, user_photo, admin
    FROM ${TABLE}
    WHERE user_uid=${userSql}
      OR user_email=${userSql}
...
async function getLoginUserBySession(token, options = {}) {
  await ensureTable();
  if (!token) return null;
  const tokenHashSql = MySQL.escape(hashSessionToken(token));
  const touch = options.touch !== false;
  const rows = await global.query(`
    SELECT u.user_uid, u.user_provider, u.user_name, u.user_email, u.user_photo, u.admin, s.lastSeenAt
  ...
  if (touch && (!lastSeenAt || Date.now() - lastSeenAt > 15 * 60 * 1000)) {
    await global.query(`
      UPDATE ${SESSIONS_TABLE}
      SET lastSeenAt=NOW()
      WHERE session_token_hash=${tokenHashSql}
        AND (lastSeenAt IS NULL OR lastSeenAt < DATE_SUB(NOW(), INTERVAL 15 MINUTE))
      LIMIT 1
    `);
  }
  const admin = Number(row.admin) === 1 || isOwnerEmail(row.user_email);
...
function normalizeUser(user) {
  ...
  return {
    ...
    photo: user && (user.photo || user.photoURL) ? (user.photo || user.photoURL) : null
  };
}
...
async function mergeLegacyEmailRow(user) {
  ...
  const photoSql = user.photo ? MySQL.escape(user.photo) : 'NULL';
  ...
  if (!currentRows || !currentRows.length) {
    ...
    await global.query(`
      ...
      SET user_uid=${uidSql},
          ...
          user_email=${emailSql},
          user_photo=COALESCE(${photoSql}, user_photo)
...
async function ensureLoginUser(loginUser) {
  ...
  await global.query(`
    INSERT INTO ${TABLE} (user_uid, user_provider, user_name, user_email, user_photo, settings_json)
    VALUES (${uidSql}, ${providerSql}, ${nameSql}, ${emailSql}, ${photoSql}, CAST(${settingsSql} AS JSON))
    ON DUPLICATE KEY UPDATE
      user_provider=VALUES(user_provider),
      user_name=VALUES(user_name),
      user_email=VALUES(user_email),
      user_photo=COALESCE(VALUES(user_photo), user_photo)
  `);
