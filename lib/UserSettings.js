...
async function ensureTable() {
  ...
  const sessionColumns = await global.query(`
    SELECT COLUMN_NAME
    FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA=DATABASE()
      AND TABLE_NAME='${TABLE}'
      AND COLUMN_NAME IN ('session_token_hash', 'session_expires')
  `);
  const existingSessionColumns = new Set((sessionColumns || []).map(row => row.COLUMN_NAME));
  if (!existingSessionColumns.has('session_token_hash')) {
    await global.query(`
      ALTER TABLE ${TABLE}
      ADD COLUMN session_token_hash varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL AFTER admin
    `);
  }
  if (!existingSessionColumns.has('session_expires')) {
    await global.query(`
      ALTER TABLE ${TABLE}
      ADD COLUMN session_expires datetime DEFAULT NULL AFTER session_token_hash
    `);
  }
  ensured = true;
}
...
async function createLoginSession(userId, maxAgeMs) {
  ...
  await global.query(`
    UPDATE ${TABLE}
    SET session_token_hash=${tokenHashSql},
        session_expires=${expiresSql}
