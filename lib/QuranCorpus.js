...
class QuranCorpus {
	...
	static renderArabicBodyWithTooltips(markdown, words) {
		...
		const annotatedMarkdown = words.map(word => {
			if (!word || !word.meaning_en)
				return '';
			const label = escapeHtml(word.text);
			const meaning = escapeHtml(word.meaning_en);
			const corpusText = escapeHtml(word.text);
			const grammar = escapeHtml(word.grammar_parts_of_speech || word.parts_of_speech || '');
			return `<span class="quran-corpus-word" title="${meaning}" data-quran-word-translation="${meaning}" data-quran-word="${corpusText}" data-quran-word-number="${parseInt(word.word, 10)}" data-quran-word-grammar="${grammar}" tabindex="0">${label}</span>`;
		}).filter(Boolean).join(' ');
		return Utils.markdownToHtml(annotatedMarkdown);
	}
