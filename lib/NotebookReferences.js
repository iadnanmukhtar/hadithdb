...
function removeReference(source, ref, md) {
  ...
  for (const block of md.parse(source, {})) {
    ...
    const pattern = /(`+)[\s\S]*?\1(?!`)|\[(?:[^\]\n]|\[[^\]\n]*\])*\]\((?:[^()\n]|\([^()\n]*\))*\)|(?:https?:\/\/|www\.)[^\s<>]+|(?<![\w/:.\-\\])(?:quran:\d{1,3}:\d{1,3}|[a-z][a-z0-9_-]*:\d+[a-z]?)(?![\w:–-])/gi;
    const text = original.replace(pattern, (match, code) => {
      ...
      if (match.startsWith('[')) {
        ...
        return tokens.some(token => token.type === 'link_open' && token.attrGet('href')?.toLowerCase() === `/${ref}`) ? '' : match;
      }
      return match.toLowerCase() === ref ? '' : match;
    });
...
module.exports = { install, snapshot, reference, removeReference };
