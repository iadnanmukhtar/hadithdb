...
const REF = /^(?:quran:\d{1,3}:\d{1,3}|[a-z][a-z0-9_-]*:\d+[a-z]?)$/i;
...
function install(md) {
  md.core.ruler.push('notebook_references', state => {
    ...
    for (const block of state.tokens) for (const token of block.children || []) {
      if (token.type === 'link_open' && token.attrGet('title') === EXPANDED_TITLE) {
    ...
    function button(ref) {
      ...
      token.content = ` <button type="button" class="notebook-expand-reference btn btn-sm btn-link p-0" data-notebook-reference="${md.utils.escapeHtml(ref)}" title="Save the full Arabic text and translation in this note">Expand</button>`;
      return [token];
    }
    for (const block of state.tokens) {
      ...
      for (const token of block.children) {
        ...
        if (token.type === 'link_close') {
          output.push(token);
          if (linkedRef) output.push(...button(linkedRef));
          inLink = false; linkedRef = null; continue;
        }
        if (token.type !== 'text' || inLink) { output.push(token); continue; }
        const pattern = /(?<![\w/:.-])(?:quran:\d{1,3}:\d{1,3}|[a-z][a-z0-9_-]*:\d+[a-z]?)(?![\w:–-])/gi;
        let offset = 0;
...
function clean(value) {
  // Source fields can contain HTML; preserve text and paragraph breaks as safe Markdown.
  const $ = load(String(value || '').replace(/<br\s*\/?\s*>/gi, '\n').replace(/<\/(p|div)>/gi, '\n\n'));
  $('script,style').remove();
  return $.root().text().trim().replace(/</g, '&lt;').replace(/>/g, '&gt;');
}
async function snapshot(value) {
  ...
  const parts = [`**[${ref}](/${ref} "${EXPANDED_TITLE}")**`, arabic || 'Arabic text is unavailable.', english || 'English translation is unavailable.'];
  if (item.ar?.footnote) parts.push(clean(item.ar.footnote));
  if (item.en?.footnote) parts.push(clean(item.en.footnote));
  return parts.join('\n\n').split('\n').map(line => `> ${line}`).join('\n');
}
module.exports = { install, snapshot, reference };
