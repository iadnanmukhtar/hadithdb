  ...
  ensurePromise = (async function () {
    ...
    await global.query(`
      CREATE TABLE IF NOT EXISTS ${HADITH_TRANSLATION_TABLE} (
        ...
        CONSTRAINT fk_hadiths_translations_language FOREIGN KEY (target_language) REFERENCES languages(code)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    `);
...
async function ensureTranslationLanguageForeignKeys() {
  ...
  await ensureTranslationLanguageForeignKey(HADITH_TRANSLATION_TABLE, 'fk_hadiths_translations_language');
}
...
async function debitAndSaveContentTranslation(user, itemType, itemId, targetLanguage, mode, sourceHash, content, model, points, reason, referenceType, referenceId, metadata) {
  ...
  referenceId = (referenceId || crypto.randomUUID()).toString().slice(0, 255);
  mode = mode || 'translate';
  const isHadith = itemType === 'hadith';
  const hadithId = isHadith ? hadithIdSql(itemId) : null;
  const itemIdValue = isHadith ? null : itemId;
    ...
    if (isHadith) {
      await q(`
        INSERT INTO ${HADITH_TRANSLATION_TABLE}
          (user_uid, hadith_id, target_language, mode, source_hash, content_json, model, points_charged)
        VALUES
          (${sql(user.uid)}, ${hadithId}, ${sql(targetLanguage)}, ${sql(mode)}, ${sql(sourceHash)},
      ...
      await q(`
        INSERT INTO ${TRANSLATION_TABLE}
          (user_uid, item_type, item_id, target_language, mode, source_hash, content_json, model, points_charged)
        VALUES
          (${sql(user.uid)}, ${sql(itemType)}, ${sql(itemIdValue)}, ${sql(targetLanguage)}, ${sql(mode)}, ${sql(sourceHash)},
           CAST(${contentJson} AS JSON), ${modelSql}, ${points})
        ON DUPLICATE KEY UPDATE
          content_json=VALUES(content_json),
          model=VALUES(model),
