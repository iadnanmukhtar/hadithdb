...
const DEPRECATED_HADITH_TRANSLATION_TABLE = 'hadiths_translations';
...
async function ensureTables() {
  ...
  ensurePromise = (async function () {
    ...
    `);
    await migrateDeprecatedHadithTranslations();
...
async function migrateDeprecatedHadithTranslations() {
  if (!await tableExists(DEPRECATED_HADITH_TRANSLATION_TABLE))
    return;
  await ensureDisabledLanguageReferences(DEPRECATED_HADITH_TRANSLATION_TABLE);
  await global.query(`
    INSERT INTO ${TRANSLATION_TABLE}
      (user_uid, item_type, item_id, target_language, mode, source_hash, content_json, model, points_charged, createdAt, updatedAt)
    SELECT
      user_uid,
      'hadith',
      CAST(hadith_id AS CHAR),
      target_language,
      mode,
      source_hash,
      content_json,
      ...
      createdAt,
      updatedAt
    FROM ${DEPRECATED_HADITH_TRANSLATION_TABLE}
    ON DUPLICATE KEY UPDATE
      content_json=IF(VALUES(updatedAt) > ${TRANSLATION_TABLE}.updatedAt, VALUES(content_json), ${TRANSLATION_TABLE}.content_json),
      model=IF(VALUES(updatedAt) > ${TRANSLATION_TABLE}.updatedAt, VALUES(model), ${TRANSLATION_TABLE}.model),
      points_charged=IF(VALUES(updatedAt) > ${TRANSLATION_TABLE}.updatedAt, VALUES(points_charged), ${TRANSLATION_TABLE}.points_charged),
      createdAt=IF(VALUES(createdAt) < ${TRANSLATION_TABLE}.createdAt, VALUES(createdAt), ${TRANSLATION_TABLE}.createdAt),
      updatedAt=IF(VALUES(updatedAt) > ${TRANSLATION_TABLE}.updatedAt, VALUES(updatedAt), ${TRANSLATION_TABLE}.updatedAt)
  `);
}
