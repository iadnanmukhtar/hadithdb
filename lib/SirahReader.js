...
async function sourceReference(book, number) {
 if (!book || book.type !== 'sirah' || !/^\d{5,}$/.test(String(number))) return null;
 const source = book.alias === 'ibnhisham'
  ? { table: 'sirah_source_entries', column: 'source_entry_id' }
  : (book.alias === 'history' ? { table: 'islamweb_history_source_entries', column: 'source_page_id' } : null);
 if (!source) return null;
 const [row] = await global.query(`SELECT h.num FROM ${source.table} s JOIN hadiths h ON h.id=s.item_id AND h.bookId=s.book_id WHERE s.book_id=${Number(book.id)} AND s.${source.column}=${Number(number)} LIMIT 1`);
