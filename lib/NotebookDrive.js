...
function encode(note) {
  const { markdown, title, tags, ...metadata } = note;
...
function decode(text, source) {
  ...
  return { ...note, created_at: metadata.created_at || null };
}
...
class Store {
  ...
  async upload(id, note, existing, create = false) {
    ...
    await this.request(create ? 'upload/drive/v3/files' : `upload/drive/v2/files/${id}`, { method: create ? 'POST' : 'PUT', params: { uploadType: 'multipart', fields: 'id' },
  ...
  async save(body) {
    ...
    if ((previous && previous.version !== body.version) || (!previous && body.version !== 0)) throw conflict();
    ...
    await this.upload(fileId, note, previous, !previous);
    return this.read(fileId, source);
  }
  async remove(source, version) {
    const note = await this.get(source);
    if (!note || note.version !== version) throw conflict();
...
function publicNote(note) { if (!note) return null; const { _etag, _id, ...result } = note; return result; }
async function get(uid, source) { return publicNote(await (await clientFor(uid)).get(source)); }
async function save(uid, body) { return locked(uid, async () => publicNote(await (await clientFor(uid)).save(body))); }
async function remove(uid, source, version) { return locked(uid, async () => (await clientFor(uid)).remove(source, version)); }
...
module.exports = { connectionStatus, connect, disconnect, migrate, get, save, remove, list, status, tagList, exportZip,
