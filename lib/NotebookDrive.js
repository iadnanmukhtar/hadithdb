...
const Attachments = require('./NotebookAttachments');
...
const revision = note => crypto.createHash('sha256').update(JSON.stringify([
  ...
  ...(note.references?.length ? [Attachments.parse(note.references)] : [])
])).digest('hex');
...
function validate(body, previous = {}) {
  ...
  return { source_key: source, source_title: body.source_title, source_url: Legacy.sourceUrl(body.source_url),
    references: Attachments.parse(body.references === undefined ? previous.references : body.references),
...
module.exports = { connectionStatus, connect, disconnect, migrate, get, save, remove, list, status, tagList, links, exportZip,
