...
const debug = require('./Debug')('hadithdb:Model');
...
class Record {
	 ...
	 */
	get book() {
		return Library.instance.findBook(this.book_id ?? this.bookId ?? this.book_alias);
	}
...
class Item extends Record {
	...
	/** @type {string} */ body_en;
	/** @type {string} */ body;
	/** @type {string} */ body_ar_alt;
	/** @type {string} */ body_indopak;
	/** @type {string} */ body_warsh;
	/** @type {number} */ sajda;
	...
	constructor(item) {
		...
		var bookAlias = this.book_alias || this.book?.alias || '';
		var h1PathNumber = bookAlias === 'quran' ? this.h1 : u.formatHadithHeadingNumber(this.h1);
		var h2PathNumber = bookAlias === 'quran' ? this.h2 : u.formatHadithHeadingNumber(this.h2);
		var h3PathNumber = bookAlias === 'quran' ? this.h3 : u.formatHadithHeadingNumber(this.h3);
		if (!this.book_alias && bookAlias)
			this.book_alias = bookAlias;
		applyQuranSurahMetadata(this);
		this.pubdate = this.highlight || this.lastmod;
		...
		var clone = {};
			if (u.isTruthy(this.h3)) {
				...
				clone.path = clone.ref = `${bookAlias}/${h1PathNumber}/${h2PathNumber}/${h3PathNumber}`;
			...
			if (u.isTruthy(this.h2)) {
				...
				clone.path = clone.ref = `${bookAlias}/${h1PathNumber}/${h2PathNumber}`;
			...
			if (u.isTruthy(this.h1)) {
				...
				clone.path = clone.ref = `${bookAlias}/${h1PathNumber}`;
		...
		if (this.book_virtual === 1) {
			...
			this.en.actual.book_shortName = this.book_alias_ref; // intentional
			...
			this.en.book_shortName = bookAlias; // intentional
		...
		this.ar.body_alt = this.body_ar_alt;
		this.en.footnote = this.footnote_en;
		this.ar.footnote = this.footnote;
		this.en.note = this.note_en;
		this.ar.note = this.note;
		this.contentTranslations = contentTranslationsFromIndexedFields(this);
	}
...
function contentTranslationsFromIndexedFields(item) {
	var translations = {};
	var fields = ['title', 'chain', 'body', 'footnote'];
	for (var key in item) {
		var match = key.match(/^(title|chain|body|footnote)_([a-z][a-z0-9]{1,15})$/);
		if (!match)
			continue;
		var field = match[1];
		var code = match[2];
		if (code === 'en' || code === 'ar' || fields.indexOf(field) < 0)
			continue;
		if (!translations[code])
			translations[code] = {
				...
				fontClass: languageFontClass(code)
			};
		translations[code][field] = item[key];
	}
	return Object.keys(translations).sort((a, b) => translations[a].label.localeCompare(translations[b].label) || a.localeCompare(b)).map(code => translations[code]);
}

function languageMetadata(code) {
	return PaymentConfig.supportedLanguage(code) || {};
}

function languageLabel(code) {
	return languageMetadata(code).label || code.toUpperCase();
}

function languageDirection(code) {
	return languageMetadata(code).dir || (isRtlLanguageCode(code) ? 'rtl' : 'ltr');
}

function languageFontClass(code) {
	return languageMetadata(code).fontClass || '';
}

function isRtlLanguageCode(code) {
	return ['ar', 'fa', 'he', 'ur'].indexOf((code || '').toLowerCase()) >= 0;
}
...
class Heading extends Record {
	...
	*/
	...
	constructor(heading) {
		...
		if (this.book_alias !== 'quran' && this.path)
			this.path = u.canonicalHadithHeadingPath(this.path);
		if (this.prevId !== null)
		...
		if (this._children === undefined) {
			...
			switch (this.level) {
				...
				case 1:
					...
					if (this.book_alias !== 'quran' && Number(this.h1) === 0)
						children = children.filter(row => Number(row.h2_count) > 0);
					for (i in children)
	...
	async getItems(offset, size) {
		...
		if (this.book.virtual == 1) {
			switch (this.level) {
				case 1:
					items = await global.query(
						`SELECT * FROM v_hadiths_virtual_snapshot WHERE book_id=${this.book.id}
							AND h1=${this.h1}
						ORDER BY ordinal
						LIMIT ${offset},${size + 1}`);
					break;
			...
			items = await HadithTranslationIndexView.attachTranslations(items, { rowIdField: 'hId_ref' });
		} else {
			// A section's source numbering is the stable display order. Individual
			// search documents can briefly retain an old ordinal after maintenance;
			// use the section-local number first so one stale ordinal cannot render
			// otherwise consecutive hadiths out of order. Keep ordinal and hId as
			// deterministic tie-breakers for collections with duplicate numbers.
			var itemOrder = this.level >= 2
				? 'numInChapter, ordinal, hId'
				: 'ordinal, hId';
			switch (this.level) {
				case 0:
					items = await Index.docsFromQueryString(Item.INDEX,
						`book_alias:${this.book_alias} AND level:1`, offset, size + 1, itemOrder);
					break;
				case 1:
					items = await Index.docsFromQueryString(Item.INDEX,
						`book_alias:${this.book_alias} AND h1:${this.h1}${this.sirahChapterOpeningOnly ? ' AND NOT _exists_:h2' : ''}`, offset, size + 1, itemOrder);
					break;
				case 2:
					items = await Index.docsFromQueryString(Item.INDEX,
						`book_alias:${this.book_alias} AND h1:${this.h1} AND h2:${this.h2}`, offset, size + 1, itemOrder);
					break;
				case 3:
					items = await Index.docsFromQueryString(Item.INDEX,
						`book_alias:${this.book_alias} AND h1:${this.h1} AND h2:${this.h2} AND h3:${this.h3}`, offset, size + 1, itemOrder);
					break;
			}
		}
		for (var i in items)
	...
	static async headingFromRef(ref) {
		...
		if (Number.isInteger(ref)) {
			...
			heading = await Index.docsFromQuery(Heading.INDEX, { term: { path: ref } }, 0, 1);
			if (heading.length < 1)
				throw new ReferenceError(`Not found: Heading ${ref}`);
			heading = heading[0];
		}
	...
	async getFirstSection() {
		if (this.firstSection === undefined) {
			var _sections = await Index.docsFromQueryString(Heading.INDEX,
				`book_alias:${this.book_alias} AND level:2 AND h1:${this.h1} AND h2:[1 TO *]`, 0, Number(this.h1) === 0 ? 1000 : 1, 'ordinal');
			if (this.book_alias !== 'quran')
				_sections = _sections.filter(row => Number(row.h2_count) > 0);
			if (_sections.length > 0)
				this.firstSection = new Section(_sections[0]);
			else if (this.book_alias !== 'quran' && Number.isInteger(Number(this.h1)) && Number(this.h1) > 0) {
				var nextChapter = Number(this.h1) + 1;
				var decimalChapters = await Index.docsFromQuery(Heading.INDEX, {
					bool: { filter: [
						{ term: { book_alias: this.book_alias } },
						{ term: { level: 1 } },
						{ range: { h1: { gt: Number(this.h1), lt: nextChapter } } }
					] }
				}, 0, 100, 'ordinal');
				var firstPopulatedChapter = decimalChapters.find(row => Number(row.h1_count) > 0);
				this.firstSection = firstPopulatedChapter ? new Chapter(firstPopulatedChapter) : null;
			} else
...
class Book {
	...
	async getChapters() {
		if (this.chapters === undefined) {
			var chapters = await headingSummariesForBook(this.alias, 1);
			var sections = await headingSummariesForBook(this.alias, 2);
			var sectionsByChapter = new Map();
			for (var section of sections) {
				section = new Section(section);
				var h1 = `${section.h1}`;
				if (!sectionsByChapter.has(h1))
					sectionsByChapter.set(h1, []);
				sectionsByChapter.get(h1).push(section);
			}
			for (var i in chapters) {
				chapters[i] = new Chapter(chapters[i]);
				chapters[i].sections = sectionsByChapter.get(`${chapters[i].h1}`) || [];
				chapters[i].firstSection = chapters[i].sections.length > 0 ? chapters[i].sections[0] : null;
			}
...
const BOOK_TOC_HEADING_FIELDS = [
	'book_id', 'book_alias', 'tId', 'ordinal', 'prevId', 'nextId', 'path', 'level',
	'h1', 'h1_id', 'h2', 'h2_id', 'h3', 'h3_id',
	'h1_title_en', 'h1_title', 'h1_start', 'h1_count',
	'h2_title_en', 'h2_title', 'h2_start', 'h2_count'
];
const BOOK_TOC_PAGE_SIZE = 500;

/**
 * Load the compact heading records required by the book TOC in bounded pages.
 * Full v_toc documents contain repeated book and ancestor text, which makes
 * large books vulnerable to aborted compressed Elasticsearch responses.
 *
 * @param {string} bookAlias
 * @param {number} level
 * @returns {Promise<Object[]>}
 */
async function headingSummariesForBook(bookAlias, level) {
	var headings = [];
	var offset = 0;
	var page;
	var query = {
	...
	var orderBy = level === 1 ? 'h1, ordinal' : 'h1, h2, ordinal';
	do {
		page = await Index.docsFromQueryFields(
			Heading.INDEX, query, BOOK_TOC_HEADING_FIELDS,
			offset, BOOK_TOC_PAGE_SIZE, orderBy);
		headings.push(...page);
		offset += page.length;
	} while (page.length === BOOK_TOC_PAGE_SIZE);
	return headings;
}
...
class Library {
	...
	findBook(ref) {
		if (ref === undefined || ref === null)
			return undefined;
		var refStr = `${ref}`;
		var refNum = Number(ref);
		return this._books.find(function (book) {
			return (book.alias === refStr || book.id === ref || (Number.isFinite(refNum) && book.id === refNum));
		});
	}
	...
	static async init(forceReload = false) {
		...
		if (forceReload || library._books === undefined) {
			...
			await require('./Surahs').load();
			library._initialized = true;
			debug(`initialized library`);
		}
		return library;
	}
	...
	static async _allBooks() {
		...
		var rows = await global.query(`
			SELECT b.*,
				(
					SELECT COUNT(DISTINCT h.h1)
					FROM hadiths h
					WHERE h.bookId=b.id AND h.h1 > 0
				) AS chapter_count,
				(
					SELECT COUNT(*)
					FROM hadiths h
					WHERE h.bookId=b.id AND COALESCE(h.remark, 0)=0
				) AS hadith_count
			FROM books b
			ORDER BY b.ordinal`);
		var books = [];
		for (var row of rows)
			books.push(new Book(Books.normalizeBook(row, 'books')));
		return books;
	}
	 ...
	 * @returns {Promise<Book[]>}
	 */
	static async reloadBooks() {
		var library = Library._singleton;
		var books = await Library._allBooks();
		library._books = global.books = books;
		await Books.refreshUnifiedCatalog();
		await require('./Surahs').load();
		if (library._initialized === false)
			library._initialized = true;
		if (global.library === undefined)
			global.library = library;
		return books;
 ...
 * @private
 */
function applyQuranSurahMetadata(h) {
	var bookAlias = h.book_alias || '';
	if (bookAlias !== 'quran' || h.h1 === undefined || h.h1 === null)
		return;
	var surah = Surahs.surahs.find(function (item) {
		return Number(item.num) === Number(h.h1);
	});
	if (!surah)
		return;
	h.h1_title_en = surah.name_en || h.h1_title_en;
	h.h1_title = surah.name_ar || h.h1_title;
	h.surah_ayahs = surah.ayahs || h.surah_ayahs;
	h.surah_revelation = surah.revelation || h.surah_revelation;
	h.surah_aliases = surah.aliases || h.surah_aliases;
}
