...
class QuranMushaf {
	static ayahSelection(value) {
		...
		return { surah, start, end, ref: `${surah}:${start}${end === start ? '' : `-${end}`}` };
	}
	...
	static async page(number) {
		...
		const loadingGeneration = pageCacheGeneration;
		pageLoading.set(number, loading);
		try {
			const page = await loading;
			if (loadingGeneration === pageCacheGeneration)
	...
	static async pageForRef(surah, ayah, options = {}) {
		surah = Number(surah);
		ayah = Number(ayah);
		if (!Number.isInteger(surah) || !Number.isInteger(ayah) || surah < 1 || ayah < 1)
			return null;
		const key = `${options.last ? 'last:' : ''}${surah}:${ayah}`;
		...
		const rows = await global.query(`
			...
			ORDER BY page.page_number ${options.last ? 'DESC' : 'ASC'}
			LIMIT 1`);
	...
	static invalidatePage(number) {
		...
		pageLoading.delete(number);
		sectionForPageCache.delete(number);
	}
