...
async function list(uid, offset = 0) {
  await ensureTable();
  const rows = await global.query(mysql.format('SELECT source_key, source_title, source_url, markdown, version, updated_at FROM user_notebook WHERE user_uid=? ORDER BY (source_key=\'general\') DESC, updated_at DESC, source_key LIMIT 13 OFFSET ?', [uid, offset]));
