...
async function introductionArticles(bookId, includeHadithSections = false) {
	bookId = number(bookId);
	if (!bookId)
		return [];
	return global.query(`SELECT * FROM toc
		WHERE bookId=${bookId} AND level=2 AND h1=0
		${includeHadithSections ? '' : `AND NOT EXISTS (SELECT 1 FROM hadiths WHERE tocId=toc.id) AND NOT EXISTS (SELECT 1 FROM hadiths_virtual WHERE tocId=toc.id)`}
...
async function addIntroductionArticle(bookId, value, userId) {
	...
	const existing = await introductionArticles(contentBook.id, true);
...
async function deleteIntroductionArticle(articleId) {
	articleId = number(articleId);
	if (!articleId || articleId <= 0)
		throw new RangeError('Invalid introduction article id');
	const article = (await global.query(`SELECT * FROM toc WHERE id=${articleId} AND level=2 AND h1=0`))[0];
	if (!article || !await introductionBook(article.bookId))
		throw new ReferenceError('Introduction article not found');
	const linked = await global.query(`SELECT id FROM hadiths WHERE tocId=${articleId}
		UNION ALL SELECT id FROM hadiths_virtual WHERE tocId=${articleId} LIMIT 1`);
	if (linked.length)
		throw new RangeError('This heading contains hadiths and cannot be deleted as an introduction article');
	const result = await global.query(`DELETE FROM toc WHERE id=${articleId} AND level=2 AND h1=0`);
	return { message: 'Introduction article deleted', value: { id: articleId, bookId: article.bookId, deleted: result.affectedRows === 1 } };
}

module.exports = {
	deleteIntroductionArticle,
