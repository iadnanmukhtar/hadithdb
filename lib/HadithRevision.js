	...
	return [
			{
				role: 'system',
				content:
`You are correcting and translating a hadith record from classical Islamic hadith literature.
Use the supplied collection and chapter context to disambiguate names, technical hadith terminology, legal wording, and the scope of the title without adding interpretation.
...
Surround only statements spoken by the Prophet ﷺ in the body with guillemets inside Markdown italics, using a single asterisk on each side, like *«...»*.
Surround Qur'anic verses and identifiable Qur'anic phrases in the body with Qur'anic verse marks, like ﴿...﴾.
Do not italicize statements by Companions, narrators, scholars, or any other speakers, narration about the Prophet ﷺ, or Qur'anic quotations. Remove existing italics from such text when revising it. Determine italics by who spoke the words, not merely by their inclusion in a hadith report, and end the italic span when the speaker changes.
Add a missing Qur'an reference when an explicit quotation or unmistakable phrase can be identified confidently; never guess. Convert every plain-text Qur'an reference to a Markdown link such as ([آل عمران ٣:٢٩](https://quran.islamunlocked.com/quran:3:29)). Use the correct Arabic surah name and Arabic-Indic digits in the label, and ASCII digits in the URL.
Convert every identifiable plain-text hadith citation to a Markdown link such as ([مسلم ٢٩٨٥](https://hadithunlocked.com/muslim:2985)). Use the canonical Hadith Unlocked collection alias and ASCII reference in the URL. Do not manufacture a citation when the collection and number cannot be established confidently.
...
In personal names in every English output field, render ibn/bin as "b." and bint as "bt.".
...
Surround only translated statements spoken by the Prophet ﷺ with standard English double quotation marks inside Markdown italics, using a single asterisk on each side, like *"..."*.
Use standard English double quotation marks without italics for statements by other speakers, like "...". Do not italicize narration about the Prophet ﷺ. Remove existing italics from non-Prophetic speech when revising it, and end the italic span when the speaker changes.
Surround translated Qur'anic verses and identifiable Qur'anic phrases with standard English double quotation marks without italics, like "...".
Do not use Arabic guillemets «...» or Qur'anic verse marks ﴿...﴾ in English output.
Add a missing Qur'an reference when an explicit quotation or unmistakable phrase can be identified confidently; never guess. Convert every plain-text Qur'an reference to a Markdown link such as ([Āl ʿImrān 3:29](https://quran.islamunlocked.com/quran:3:29)). Use the correct transliterated surah name in the label and ASCII digits in the URL.
Convert every identifiable plain-text hadith citation to a Markdown link such as ([Muslim 2985](https://hadithunlocked.com/muslim:2985)). Use the canonical Hadith Unlocked collection alias and ASCII reference in the URL. Do not manufacture a citation when the collection and number cannot be established confidently.
Do not put the quoted verse or hadith itself inside its citation link. Preserve already correct link targets.
...
content_type: hadith record containing an isnad, matn, possible footnote, and hadith heading
book_alias: ${Utils.trimToEmpty(item.book_alias)}
book_title: ${Utils.trimToEmpty(item.book_name_en || item.book_shortName_en || item.book_name || item.book_shortName)}
book_title_arabic: ${Utils.trimToEmpty(item.book_name || item.book_shortName)}
chapter_title: ${Utils.trimToEmpty(item.h1_title_en || item.h2_title_en || item.h3_title_en)}
chapter_title_arabic: ${Utils.trimToEmpty(item.h1_title || item.h2_title || item.h3_title)}
...
function normalizeEnglish(text, prefixAI) {
	text = Utils.trimToEmpty(Utils.normalizeEnglishAIMarker(text));
	text = text.replace(/^✧\s*/, '');
	text = text.replace(/[«»﴿﴾]/g, '"');
