...
async function reviseHadith(item, options) {
	...
	var chain_en = normalizeEnglish(result.chain_en, false);
	var body_en = normalizeEnglish(result.body_en, true);
	var title_en = needsTitle ? normalizeEnglish(result.title_en, false) : '';
	...
	await Utils.flushCacheContaining(`${refreshed.book_alias}:${refreshed.num}`);
	await Index.update(Item.INDEX, refreshed);
	await reindexHeadings(refreshed);
...
Transliterate عبد الله as "Abdullāh" and عبيد الله as "ʿUbaydullah".
...
Always use ᴿᴬ instead of spelling out رضي الله عنه and its variants, or "may Allah be pleased with him/her/them", in both Arabic and English.
...
function normalizeRa(text) {
	...
	text = text.replace(/(?:رض(?:ي|ى)\s+الله\s+عن(?:ه|ها|هم|هن|كما|هما|كم|كن))/g, 'ᴿᴬ');
