...
function assertTranslationAccess(user) {
  if (PaymentConfig.adminOnlyTranslations() && (!user || user.admin !== true))
    throw createError(403, 'Only admins can translate content.');
}
...
function estimatePoints(source, mode) {
  if (PaymentConfig.adminOnlyTranslations())
    return 0;
...
async function loadHadithSource(itemId) {
  ...
  return {
    ...
    promptContext: {
      ...
      book_title: compactText(row.book_name_en || row.book_shortName_en || row.book_name || row.book_shortName || ''),
      book_title_arabic: compactText(row.book_name || row.book_shortName || ''),
      chapter_title: compactText(row.h1_title_en || row.h2_title_en || row.h3_title_en || ''),
      chapter_title_arabic: compactText(row.h1_title || row.h2_title || row.h3_title || '')
    },
    languages: { ar, en },
...
function translationModelOptions() {
  const settings = global.settings || {};
  const options = [];
  if (settings.openAI && Utils.isTruthy(settings.openAI.key) && Utils.isTruthy(settings.openAI.model)) {
    options.push({
      id: 'openai',
      label: 'OpenAI',
      provider: 'openai',
      model: Utils.trimToEmpty(settings.openAI.model)
    });
  }
  if (settings.deepSeek && Utils.isTruthy(settings.deepSeek.key) && Utils.isTruthy(settings.deepSeek.model)) {
    options.push({
      id: 'deepseek',
      label: 'DeepSeek',
      provider: 'deepseek',
      model: Utils.trimToEmpty(settings.deepSeek.model)
    });
  }
  return options;
}

function selectedTranslationModel(targetLanguage) {
  const contentSettings = global.settings?.payments?.content || {};
  const languageCode = Utils.trimToEmpty(targetLanguage && (targetLanguage.code || targetLanguage)).toLowerCase();
  const configured = Utils.trimToEmpty(
    languageCode === 'en' && contentSettings.englishTranslationModel
      ? contentSettings.englishTranslationModel
      : contentSettings.translationModel || 'openai'
  ).toLowerCase();
  const selected = translationModelOptions().find(option => option.id === configured);
  if (selected)
    return selected;
  throw new Error(`Configured translation model is unavailable: ${configured}`);
}

function translationProviderRequest(modelOption, fields) {
  if (modelOption.provider === 'deepseek') {
    return {
      url: 'https://api.deepseek.com/chat/completions',
      key: global.settings.deepSeek.key,
  ...
  return {
    ...
    key: global.settings.openAI.key,
    responseFormat: responseSchema(fields)
  };
}
  ...
  return [
    {
      role: 'system',
      content:
`Treat the source as classical Islamic literature and translate according to its genre, book, author, chapter, and passage context.
The content type is ${source.itemType === 'hadith' ? 'a hadith record with title, isnad, matn, and possible footnote fields' : 'a tafsir passage commenting on the specified Quran range'}.
...
Use the provided book and passage context only to disambiguate technical terminology, names, pronouns, and the scope of headings or commentary.
In translated or transliterated personal names, render ibn/bin as "b." and bint as "bt.".
...
async function requestModel(source, targetLanguage, mode) {
  const modelOption = selectedTranslationModel(targetLanguage);
  const model = modelOption.model;
  const provider = translationProviderRequest(modelOption, source.fields);
  const data = {
    model,
  ...
  let response;
  try {
    debug(`content translation start mode=${mode} type=${source.itemType} id=${source.itemId} lang=${targetLanguage.code} provider=${modelOption.provider} model=${model}`);
    response = await axios.post(provider.url, data, {
      headers: {
    ...
    debug.slow(`${modelOption.label} content translation`, elapsedMs, `mode=${mode} type=${source.itemType} id=${source.itemId} lang=${targetLanguage.code}`);
  } catch (err) {
    ...
    if (err.response) {
      ...
      throw createError(status >= 500 ? 502 : (status || 502), `${modelOption.label} translation failed: ${message || 'upstream request failed'}`);
    }
    throw createError(503, `${modelOption.label} translation failed: ${err.message}`);
  }
  const text = Utils.trimToEmpty(response.data?.choices?.[0]?.message?.content);
  if (!text)
    throw createError(502, `${modelOption.label} translation returned an empty response.`);
  let parsed;
  try {
    ...
    throw createError(502, `${modelOption.label} translation returned an invalid response.`);
  }
  return {
    model,
...
async function translate(user, itemType, itemId, targetLanguage, mode, options) {
  ...
  assertTranslationAccess(user);
  options = options || {};
  user = Object.assign(await UserPoints.ensureUser(user), { admin: user.admin === true });
  await PaymentConfig.loadLanguages();
  mode = normalizeMode(mode);
  targetLanguage = normalizeLanguage(targetLanguage, { allowArabicFix: mode === 'fix' });
  const source = await loadSource(itemType, itemId);
  ...
  const useHadithRevision = shouldUseHadithRevision(user, source, targetLanguage, mode);
  const existingFree = useHadithRevision ? null : freeExistingContent(source, targetLanguage, mode);
  const checkoutSessionId = PaymentConfig.adminOnlyTranslations() ? '' : Utils.trimToEmpty(options.checkoutSessionId);
