...
let narratorCatalogCheckedAt = 0;

async function refreshNarratorCatalog() {
	const [clock] = await global.query('SELECT CURRENT_TIMESTAMP AS started_at');
	const rows = await discoveredPairs('narrator');
	const byKey = new Map();
	for (const row of rows) {
		const key = normalize(row.value_ar);
		if (!key) continue;
		const previous = byKey.get(key);
		if (!previous || Number(row.usage_count) > Number(previous.usage_count)) byKey.set(key, row);
	}
	const generation = require('crypto').randomBytes(16).toString('hex');
	// One atomic statement publishes the snapshot; searches always see a complete catalog.
	const values = [...byKey].map(([key, row]) => [key, row.value_ar, row.value_en || '',
		normalize(row.value_en), Number(row.usage_count) || 0, generation]);
	if (values.length) {
		await global.query(`INSERT INTO hdith_narrator_search (search_ar,value_ar,value_en,search_en,usage_count,generation)
			VALUES ${values.map(row => '(' + row.map(MySQL.escape).join(',') + ')').join(',')}
			ON DUPLICATE KEY UPDATE value_ar=VALUES(value_ar),value_en=VALUES(value_en),search_en=VALUES(search_en),
			usage_count=VALUES(usage_count),generation=VALUES(generation),updated_at=CURRENT_TIMESTAMP`);
		await global.query(`DELETE FROM hdith_narrator_search WHERE updated_at<${MySQL.escape(clock.started_at)} AND generation<>${MySQL.escape(generation)}`);
	}
}

async function ensureNarratorCatalog() {
	if (narratorCatalogPromise) return narratorCatalogPromise;
	if (Date.now() - narratorCatalogCheckedAt < 300000) return;
	narratorCatalogPromise = (async () => {
		await global.query(`CREATE TABLE IF NOT EXISTS hdith_narrator_search (
			search_ar VARCHAR(255) NOT NULL PRIMARY KEY, value_ar VARCHAR(255) NOT NULL,
			value_en VARCHAR(255) NOT NULL, search_en VARCHAR(255) NOT NULL,
			usage_count INT NOT NULL, generation CHAR(32) NOT NULL,
			updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
			KEY narrator_search_en (search_en)
		) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);
		const rows = await global.query('SELECT MAX(updated_at) AS updated_at FROM hdith_narrator_search');
		if (!rows[0]?.updated_at) await refreshNarratorCatalog();
		else if (Date.now() - new Date(rows[0].updated_at).getTime() > 300000) {
			// Keep searches responsive while refreshing imported names in the background.
			refreshNarratorCatalog().catch(error => { narratorCatalogCheckedAt = 0; console.error('Narrator search refresh failed:', error.message); });
		}
		narratorCatalogCheckedAt = Date.now();
	})().finally(() => { narratorCatalogPromise = null; });
	return narratorCatalogPromise;
}

async function searchNarrators(queryValue, limitValue) {
	const term = normalize(queryValue);
	if (term.length < 2) return [];
	const limit = Math.min(Math.max(Number.parseInt(limitValue, 10) || 100, 1), 100);
	const pattern = MySQL.escape('%' + term.replace(/[!%_]/g, '!$&') + '%');
	await ensureSchema();
	await ensureNarratorCatalog();
	const [discovered, managedRows] = await Promise.all([
		global.query(`SELECT value_ar,value_en,usage_count FROM hdith_narrator_search
			WHERE search_ar LIKE ${pattern} ESCAPE '!' OR search_en LIKE ${pattern} ESCAPE '!'
			ORDER BY usage_count DESC,search_ar LIMIT ${limit}`),
		global.query(`SELECT id,pair_key,value_ar,value_en,hidden FROM hdith_bilingual_pairs WHERE pair_type='narrator'`)
	]);
	const byKey = new Map(discovered.map(row => [normalize(row.value_ar), {
		id: null, key: normalize(row.value_ar), value_ar: row.value_ar,
		value_en: row.value_en || '', usage_count: Number(row.usage_count) || 0, managed: false
	}]));
	for (const row of managedRows) {
		const key = row.pair_key || normalize(row.value_ar);
		byKey.delete(key);
		byKey.delete(normalize(row.value_ar));
		if (!Number(row.hidden) && (normalize(row.value_ar).includes(term) || normalize(row.value_en).includes(term)))
			byKey.set(key, { id: Number(row.id), key, value_ar: row.value_ar, value_en: row.value_en || '', usage_count: 0, managed: true });
	}
	return [...byKey.values()].sort((a, b) => Number(b.managed) - Number(a.managed)
		|| a.value_ar.localeCompare(b.value_ar, 'ar')).slice(0, limit);
}
...
async function propagate(type, originalAr, valueAr, valueEn) {
	...
	if (type === 'sharh_title') {
		...
		contexts = await global.query(`SELECT DISTINCT hadith_id,alias FROM (
			...
			JOIN hadiths h ON h.id=m.hadith_id JOIN books b ON b.id=h.bookId WHERE m.narrator IN (${oldValue},${newArabic})
			...
			WHERE COALESCE(NULLIF(n.name_tashkil, ''),n.name) IN (${oldValue},${newArabic})
		) affected`);
		// Follow canonical identity for primary narrators even when the cached label is stale.
		await global.query(`UPDATE hdith_hadith_metadata m
			JOIN hdith_hadith_narrators hn ON hn.hadith_id=m.hadith_id AND hn.ordinal=1
			JOIN hdith_narrators n ON n.id=hn.narrator_id
			SET m.narrator=${newArabic},m.narrator_en=${newEnglish}
...
function resetSchemaForTests() {
	schemaPromise = null;
	narratorCatalogPromise = null;
	narratorCatalogCheckedAt = 0;
}

module.exports = { refreshNarratorCatalog, searchNarrators, ensureSchema, hide, list, managed, normalize, pairType, resetSchemaForTests, resolve, save, TYPES };
