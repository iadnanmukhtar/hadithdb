...
`;
	...
	static adminGearHtml(editMode, mobile) {
		...
		var onclick = "var editMode=(document.cookie ? document.cookie.match(/editMode=1/) : false); editMode ? document.cookie='editMode=0;path=/;' : document.cookie='editMode=1;path=/;'; location.reload();";
		if (mobile)
	...
	static injectCachedAdminControls(html, req) {
		if (!html || !req || !req.cookies || req.cookies.admin !== global.settings.admin.key)
			return html;
		html = html.toString();
		if (html.indexOf('<li class="nav-item edit-gear"') >= 0)
			return html;

		var editMode = req.cookies.editMode == 1;
	...
	static quranBaseUrl() {
		return 'https://quran.islamunlocked.com';
	}
	...
	static quranUrl(req, path) {
		...
		if (!Utils.isLocalhostRequest(req))
			return Utils.quranBaseUrl() + quranPath;
