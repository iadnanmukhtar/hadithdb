...
`;
	...
	static browserReadyHtmlArtifact(cachedFile, req) {
		...
		return { html: html, gzip: Utils.cacheGzipFile(html) };
	}
	...
	static injectCachedAdminControls(html, req) {
		...
		html = html.replace(
			/(\s*)<li class="nav-item d-flex align-items-center" id="nav-auth-desktop">/,
			`$1${desktopGear}$1<li class="nav-item d-flex align-items-center" id="nav-auth-desktop">`
		);
		html = html.replace(
