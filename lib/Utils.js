...
class Utils {
	...
	static CACHENDX;
	static CACHE_SUFFIX_SLUG = '.v234';
	static CACHE_SUFFIX_SLUG_HEADER_FOOTER = '.v234';
	static SCRIPT_ASSET_SUFFIX_SLUG = '.v234';
	static CRC32_TABLE;
...
`;
	...
	static cacheFileFromFilename(filename, extension, directory) {
		extension = extension || 'html';
		const cacheDir = directory
			? `${homedir}/.hadithdb/cache/${Utils.safeFilename(directory)}${Utils.cacheSuffix()}`
	...
	static htmlCacheFile(req, options) {
		const bookAlias = Utils.cacheBookAlias(req);
	...
	static cacheBookAlias(req) {
		...
		return params.bookAlias || params.tafsir || params.translationAlias || null;
	}
