...
class Utils {
	...
	static ASSET_SUFFIX_SLUG = '.v506';
	static SCRIPT_ASSET_VERSION;
	static CRC32_TABLE;
	static CACHE_GZIP_SUFFIX = '.gz';
	static BROWSER_HTML_SUFFIX = '.browser';
	static DEFAULT_CACHE_MAX_AGE_DAYS = 3;
	static DEFAULT_CACHE_MAX_DISK_GB = 10;
	static DEFAULT_TAFSIR_CACHE_MAX_DISK_GB = 6;
	static DEFAULT_CACHE_CLEANUP_INTERVAL_MINUTES = 10;
	static CACHE_MAINTENANCE_LAST_RUN = 0;
	static CACHE_MAINTENANCE_PENDING = null;
	static CACHE_MAINTENANCE_LOCK = `${homedir}/.hadithdb/cache-maintenance.lock`;

	static formatHadithHeadingNumber(value) {
		var text = Arabic.toLatinDigits(Utils.trimToEmpty(value));
	...
	static canonicalHadithHeadingPath(value) {
		...
		for (var index = 1; index < segments.length; index++)
			segments[index] = Utils.formatHadithHeadingNumber(segments[index]);
	...
	static cacheLimitBytes(type) {
		const setting = type === 'tafsir' ? 'maxTafsirDiskGB' : 'maxDiskGB';
		const fallback = type === 'tafsir'
			? Utils.DEFAULT_TAFSIR_CACHE_MAX_DISK_GB
			: Utils.DEFAULT_CACHE_MAX_DISK_GB;
		const configured = Number(global.settings?.cache?.[setting]);
		const gigabytes = Number.isFinite(configured) && configured > 0 ? configured : fallback;
		return Math.floor(gigabytes * 1024 * 1024 * 1024);
	}

	static cacheCleanupIntervalMs() {
		const configured = Number(global.settings?.cache?.cleanupIntervalMinutes);
		const minutes = Number.isFinite(configured) && configured > 0
			? configured
			: Utils.DEFAULT_CACHE_CLEANUP_INTERVAL_MINUTES;
	...
	static defaultQuranTranslationBook() {
		return (global.commentaries || []).find(function (book) {
			return book && book.type === 'trans' && book.source === 'default';
		}) || {};
	}

	static quranTranslationBookLabel(book) {
		book = book || {};
		return book.shortName_en || book.shortName || book.author_en || book.author || book.alias || 'Default translation';
	}

	static defaultQuranTranslationLabel() {
		return Utils.quranTranslationBookLabel(Utils.defaultQuranTranslationBook());
	}

	static staticAssetUrl(url) {
		if (typeof url !== 'string')
			return url;
		return url.replace(
			/^(https?:\/\/[^/]+)?\/(?:static\/)?(audio|fonts|img|images|js|javascripts|css|stylesheets)\//i,
			function (match, origin, directory) {
				const directoryMap = { images: 'img', javascripts: 'js', stylesheets: 'css' };
				return `${origin || ''}/static/${directoryMap[directory.toLowerCase()] || directory.toLowerCase()}/`;
			}
		);
	}

	static rewriteStaticAssetUrls(value) {
		if (!value)
			return value;
		return value.toString().replace(
			/(^|[\s"'(=:])((?:https?:\/\/[^/\s"'<>]+)?\/)(?:static\/)?(audio|fonts|img|images|js|javascripts|css|stylesheets)\//gi,
			function (match, prefix, base, directory) {
				const directoryMap = { images: 'img', javascripts: 'js', stylesheets: 'css' };
				return `${prefix}${base}static/${directoryMap[directory.toLowerCase()] || directory.toLowerCase()}/`;
			}
		);
	}
	...
	static quranSurahBreadcrumbLabel(surahNumber, surahTitle) {
		var number = Utils.trimToEmpty(Arabic.toLatinDigits(Utils.emptyIfNull(surahNumber).toString()));
		var title = Utils.trimToEmpty(surahTitle).replace(/^(?:surat|surah)\s+/i, '');
		return [number, title].filter(Boolean).join(' ');
	}

	static quranSurahBreadcrumbLabelAr(surahNumber, surahTitle) {
		var number = Arabic.toArabicDigits(Utils.trimToEmpty(Arabic.toLatinDigits(Utils.emptyIfNull(surahNumber).toString())));
		var title = Utils.trimToEmpty(surahTitle).replace(/^س[\u064B-\u065F\u0670]*و[\u064B-\u065F\u0670]*ر[\u064B-\u065F\u0670]*ة[\u064B-\u065F\u0670]*\s+/u, '');
		return [number, title].filter(Boolean).join(' ');
	}

	static quranSurahHeaderSvgPath(surahNumber) {
		surahNumber = parseInt(Arabic.toLatinDigits(Utils.emptyIfNull(surahNumber).toString()), 10);
		if (!Number.isInteger(surahNumber) || surahNumber < 1 || surahNumber > 114)
			return '';
		return `/static/img/quran/surah-header/names/${surahNumber}.svg`;
	}
...
<body>
  ...
  ${footnotes}
	...
	static addEpubFonts(files) {
		var fonts = [
			{ id: 'font-kitab', href: 'fonts/kitab-base.woff2', media: 'font/woff2', file: `${__dirname}/../public/static/fonts/kitab-base.woff2` },
			{ id: 'font-quran-v1', href: 'fonts/DigitalKhattV1.otf', media: 'font/otf', file: `${__dirname}/../public/static/fonts/DigitalKhattV1.otf` }
		];
		var manifest = [];
		fonts.forEach(font => {
...
`;
	...
	static classicalIslamicLiteraturePrompt() {
		return `Treat the source as classical Islamic literature by default. Use the technical meanings established by the content type and its book context, especially for hadith, hadith headings, tafsir, sirah, fiqh, isnad, and rijal terminology. Preserve names, citations, honorifics, doctrinal and legal nuance, and the distinction between source text and commentary. Do not modernize, flatten, summarize, or add interpretation. In translated or transliterated personal names, render ibn/bin as "b." and bint as "bt.".`;
	}
	...
	static replacePBUH(s) {
		if (Utils.isFalsey(s))
			return s;
		s = s.replace(/(?:\(\s*)?may\s+(?:Allah|God)\s+be\s+pleased\s+with\s+(?:the\s+)?Prophet(?:\s+Muhammad)?(?:\s*\))?/gi, ' ﷺ ');
		const arabicMark = '[\\u0610-\\u061A\\u0640\\u064B-\\u065F\\u0670\\u06D6-\\u06ED]*';
		const pbuhArabic = new RegExp(
			`ص${arabicMark}ل${arabicMark}[ىي]${arabicMark}\\s+` +
			`[اٱ]?${arabicMark}ل${arabicMark}ل${arabicMark}ه${arabicMark}\\s+` +
			`ع${arabicMark}ل${arabicMark}ي${arabicMark}ه${arabicMark}\\s+` +
			`(?:و${arabicMark}\\s*(?:ع${arabicMark}ل${arabicMark}[ىي]${arabicMark}\\s+)?[آا]${arabicMark}ل${arabicMark}ه${arabicMark}\\s+)?` +
			`و${arabicMark}س${arabicMark}ل${arabicMark}م${arabicMark}(?![\\u0621-\\u063A\\u0641-\\u064A])`,
			'gu'
		);
	...
	static replaceRA(s) {
		if (Utils.isFalsey(s))
			return s;
		s = s.replace(/(?:\(\s*)?may\s+(?:Allah|God)(?:\s+the\s+Most\s+High)?\s+be\s+pleased\s+with\s+(?:him|her|them(?:\s+(?:all|both))?)(?:\s*\))?/gi, ' ؓ ');
		const arabicMark = '[\\u0610-\\u061A\\u0640\\u064B-\\u065F\\u0670\\u06D6-\\u06ED]*';
		const marked = letters => [...letters].map(letter => `${letter}${arabicMark}`).join('');
		const pronoun = ['هما', 'كما', 'هم', 'هن', 'ها', 'كم', 'كن', 'ه', 'ك', 'ي', 'ا']
			.map(marked).join('|');
		const raArabic = new RegExp(
			...
			`ع${arabicMark}ن${arabicMark}(?:${pronoun})(?![\\u0621-\\u063A\\u0641-\\u064A])`,
			'gu'
		);
		return s.replace(raArabic, ' ؓ ');
	}

	static normalizeArabicHonorifics(s) {
		if (Utils.isFalsey(s))
			return s;
		return Utils.replaceRA(Utils.replacePBUH(s))
			.replace(/-\s*([ؓﷺ])\s*-/gu, ' $1 ');
	}

	static normalizeEnglishAIMarker(s) {
		if (s === null || s === undefined)
			return s;
		return String(s).replace(/\[(?:Machine|AI)\]/gi, '✧');
	}
	...
	static cacheSuffix() {
		return Utils.ASSET_SUFFIX_SLUG;
	}
	...
	static cacheHtmlFragment(kind, html) {
		...
		if (!Utils.isCacheFileFresh(fragmentFile)) {
			...
			// Browser-ready HTML artifacts use the directory timestamp as a cheap
			// dependency generation. Touch it even when an expired fragment is
			// rewritten under the same content-derived filename.
			const now = new Date();
			fsSync.utimesSync(fragmentDir, now, now);
		}
	...
	static cachedRenderLocals(res, locals) {
		return Object.assign(
			{},
			res && res.app ? res.app.locals : {},
			res && res.locals ? res.locals : {},
			locals || {}
		);
	}
	...
	static readCachedHtmlResponse(cachedFile, req) {
		...
		const browserArtifact = Utils.browserReadyHtmlArtifact(cachedFile, req);
		if (browserArtifact && Utils.isBrowserReadyHtmlArtifactFresh(browserArtifact, cachedPath)) {
			const artifactPath = shouldGzip ? browserArtifact.gzip : browserArtifact.html;
			try {
				return {
					encoding: shouldGzip ? 'gzip' : null,
					body: fsSync.readFileSync(artifactPath),
					complete: true,
					lastModified: fsSync.statSync(artifactPath).mtime
				};
			} catch (error) {
				if (error.code !== 'ENOENT')
					debug('unable to read browser-ready HTML artifact:', error);
			}
		}
		const html = Utils.readCachedHtml(cachedFile, req);
		const complete = Boolean(html) && !Utils.hasUnresolvedCachedHtmlFragments(html);
		if (complete && browserArtifact) {
			try {
				Utils.writeBrowserReadyHtmlArtifact(browserArtifact, html);
				const artifactPath = shouldGzip ? browserArtifact.gzip : browserArtifact.html;
				return {
					encoding: shouldGzip ? 'gzip' : null,
					body: fsSync.readFileSync(artifactPath),
				...
				debug('unable to write browser-ready HTML artifact:', error);
			}
		}
		if (!shouldGzip)
	...
	static sendCachedHtml(res, req, cachedFile, contentType) {
		...
		if (payload.lastModified)
			res.setHeader('Last-Modified', payload.lastModified.toUTCString());
		else
	...
	static browserReadyHtmlArtifact(cachedFile, req) {
		// Admin HTML contains request-specific controls and must never be shared.
		if (!cachedFile || !req || req.admin || req.editMode)
			return null;
		const variant = JSON.stringify({
			hostname: (req.hostname || req.headers && req.headers.host || '').toString().toLowerCase(),
			searchBaseUrl: Utils.globalSearchBaseUrl(req),
			analyticsTagId: GoogleAnalytics.googleAnalyticsTagId(req),
			assetVersion: Utils.assetVersionSuffix(),
			scriptAssetVersion: Utils.scriptAssetVersion()
		});
		const hash = crypto.createHash('sha256').update(variant).digest('hex').substring(0, 16);
		const html = `${cachedFile}${Utils.BROWSER_HTML_SUFFIX}-${hash}`;
		return { html: html, gzip: Utils.cacheGzipFile(html), source: cachedFile };
	}

	static isBrowserReadyHtmlArtifactFresh(artifact, cachedPath) {
		try {
			const sourceMtime = fsSync.statSync(cachedPath).mtimeMs;
			let fragmentsMtime = 0;
			try {
				fragmentsMtime = fsSync.statSync(Utils.htmlCacheFragmentDir()).mtimeMs;
			} catch (error) {
				if (error.code !== 'ENOENT')
					throw error;
			}
			return fsSync.statSync(artifact.html).mtimeMs >= Math.max(sourceMtime, fragmentsMtime)
				&& fsSync.statSync(artifact.gzip).mtimeMs >= Math.max(sourceMtime, fragmentsMtime);
		} catch (error) {
			return false;
		}
	}

	static writeBrowserReadyHtmlArtifact(artifact, html) {
		const body = Buffer.from(String(html || ''), 'utf8');
		const gzip = zlib.gzipSync(body);
		fsSync.mkdirSync(path.dirname(artifact.html), { recursive: true });
		Utils.writeFileAtomically(artifact.html, body);
		Utils.writeFileAtomically(artifact.gzip, gzip);
		if (artifact.source)
			Utils.scheduleCacheFileRefresh(artifact.source);
	}

	static writeFileAtomically(filename, body) {
		const temporary = `${filename}.${process.pid}.${crypto.randomBytes(6).toString('hex')}.tmp`;
		try {
			fsSync.writeFileSync(temporary, body);
	...
	static scriptAssetVersion() {
		if (!Utils.SCRIPT_ASSET_VERSION) {
			try {
				const script = Buffer.concat(['script.js', 'notebook.js', 'notebook-drive.js', 'notebook-wiki.js', 'notebook-references.js', 'account-controls.js'].map(filename => fsSync.readFileSync(path.join(__dirname, '../public/static/js', filename))));
				Utils.SCRIPT_ASSET_VERSION = `${Utils.assetVersionSuffix()}-${crypto.createHash('sha256').update(script).digest('hex').substring(0, 12)}`;
			} catch (_err) {
				Utils.SCRIPT_ASSET_VERSION = Utils.assetVersionSuffix();
			}
		}
		return Utils.SCRIPT_ASSET_VERSION;
	}

	static styleAssetVersion() {
		if (!Utils.STYLE_ASSET_VERSION) {
			try {
				const stylesheet = fsSync.readFileSync(path.join(__dirname, '../public/static/css/style.css'));
				Utils.STYLE_ASSET_VERSION = `${Utils.assetVersionSuffix()}-${crypto.createHash('sha256').update(stylesheet).digest('hex').substring(0, 12)}`;
			} catch (_err) {
				Utils.STYLE_ASSET_VERSION = Utils.assetVersionSuffix();
			}
		}
		return Utils.STYLE_ASSET_VERSION;
	}

	static injectCachedAssetVersions(html) {
		if (!html)
			return html;
		return Utils.rewriteStaticAssetUrls(html).toString()
			.replace(
				/(<link\s+rel="stylesheet"\s+href="\/static\/css\/style\.css)(?:\?v=[^"]*)?("\s+rel="stylesheet">)/g,
				`$1?v=${Utils.styleAssetVersion()}$2`
			)
			.replace(
				/(<script\s+src="\/static\/js\/(?:script|notebook|notebook-drive|notebook-wiki|notebook-references|account-controls)\.js)(?:\?v=[^"]*)?("[^>]*><\/script>)/g,
	...
	static injectCachedMenuIconMarkup(html) {
		if (!html)
			return html;
		return html.toString()
			.replace(/<i class="bi bi-book"><\/i> Quran/g, '<img class="app-menu-icon app-menu-icon-img" src="/static/img/quran-icon.svg" alt="" aria-hidden="true"> Quran')
			...
			.replace(/<i class="bi bi-journals"><\/i> Books/g, '<img class="app-menu-icon app-menu-icon-img" src="/static/img/books-icon.svg" alt="" aria-hidden="true"> Books')
	...
	static injectCachedAdminControls(html, req) {
		...
		html = html.replace(
			/(<li class="nav-item d-flex align-items-center" id="nav-auth-desktop"><\/li>)(\s*)/,
			`$1$2${desktopGear}$2`
		);
		html = html.replace(
	...
	static quranStudySectionPath(heading, translationSlug) {
		translationSlug = Utils.trimToEmpty(translationSlug);
		if (!/^[A-Za-z0-9_-]+$/.test(translationSlug))
			translationSlug = '';
		var path = `/quran${translationSlug ? `/${encodeURIComponent(translationSlug)}` : ''}`;
		if (!heading || typeof heading !== 'object')
			return path;
		var surah = Number(heading.surah ?? heading.h1 ?? heading.number);
		if (!Number.isInteger(surah) || surah < 1 || surah > 114)
			return path;
		path += `/${surah}`;
		var section = Number(heading.section ?? heading.h2);
		if (!Number.isInteger(section) || section < 1)
			return path;
		path += `/${section}`;
		var subsection = Number(heading.subsection ?? heading.h3);
		if (Number.isInteger(subsection) && subsection > 0)
			path += `/${subsection}`;
		return path;
	}
	...
	static async setupCacheIndex() {
		...
		if (!Utils.CACHENDX_PENDING) {
			debug('initializing cache index');
			Utils.CACHENDX_PENDING = Utils.openCacheIndex(`${homedir}/.hadithdb/cachendx.db`)
				.then(db => {
					Utils.CACHENDX = db;
					return db;
				}).finally(() => { Utils.CACHENDX_PENDING = undefined; });
		}
		return Utils.CACHENDX_PENDING;
	}

	static async openCacheIndex(filename) {
		...
		try {
			// PM2 workers share this index. Wait for short writes and let readers
			// continue while another connection writes.
			await db.runAsync('PRAGMA busy_timeout = 5000;');
			await db.runAsync('PRAGMA journal_mode = WAL;');
			// Serialize schema inspection/migration across workers, before publishing
			// this connection to any request or background maintenance task.
			...
			if (transaction) await db.runAsync('ROLLBACK;').catch(() => {});
			await util.promisify(db.close.bind(db))().catch(() => {});
			throw error;
		}
	}
	...
	static async ensureCacheIndexSchema(db) {
		...
		await db.runAsync('CREATE TABLE IF NOT EXISTS cache_files (filename TEXT PRIMARY KEY, created_at INTEGER NOT NULL, size_bytes INTEGER NOT NULL, cache_type TEXT NOT NULL);');
		await db.runAsync('CREATE INDEX IF NOT EXISTS cache_files_fifo ON cache_files(created_at, filename);');
		await db.runAsync('CREATE INDEX IF NOT EXISTS cache_files_type_fifo ON cache_files(cache_type, created_at, filename);');
	}

	static async purgeStaleCacheIndexEntries(db) {
		...
		while (true) {
			const rows = await db.allAsync(
				'SELECT filename, COUNT(*) AS refs FROM cachendx WHERE filename > ? GROUP BY filename ORDER BY filename LIMIT ?;',
				[lastFilename, batchSize]
			);
			if (!rows || rows.length === 0)
				break;
			for (const row of rows) {
				lastFilename = row.filename;
				const fresh = Utils.isCacheFileFresh(row.filename)
					|| Utils.isCacheFileFresh(Utils.cacheGzipFile(row.filename));
				if (fresh)
					continue;
				await Utils.deleteManagedCacheFile(db, row.filename);
				purgedFiles++;
				purgedRows += Number(row.refs) || 0;
			}
			if (rows.length < batchSize)
				break;
		}
		if (purgedFiles > 0)
			debug(`purged ${purgedRows} stale or orphaned cache index entries for ${purgedFiles} files`);
		return { files: purgedFiles, rows: purgedRows };
	}

	static cacheFileType(filename) {
		const normalized = String(filename || '').replace(/\\/g, '/');
		const match = normalized.match(/\/cache\/(tafsir|hadith|trans|quran)(?:\.v[^/]+)?\//i);
		return match ? match[1].toLowerCase() : 'other';
	}

	static cacheStoredFile(filename) {
		let size = 0;
		let createdAt = null;
		for (const candidate of [Utils.cacheGzipFile(filename), filename]) {
			try {
				const stats = fsSync.statSync(candidate);
				if (stats.isFile()) {
					size += stats.size;
					createdAt = createdAt === null
						? Math.floor(stats.birthtimeMs || stats.mtimeMs)
						: Math.min(createdAt, Math.floor(stats.birthtimeMs || stats.mtimeMs));
				}
			} catch (error) {
				if (error.code !== 'ENOENT')
					debug('unable to inspect cache file:', error);
			}
		}
		try {
			const directory = path.dirname(filename);
			const prefix = `${path.basename(filename)}${Utils.BROWSER_HTML_SUFFIX}-`;
			for (const entry of fsSync.readdirSync(directory)) {
				if (!entry.startsWith(prefix))
					continue;
				const stats = fsSync.statSync(path.join(directory, entry));
				if (stats.isFile())
					size += stats.size;
			}
		} catch (error) {
			if (error.code !== 'ENOENT')
				debug('unable to inspect browser-ready cache files:', error);
		}
		return createdAt === null ? null : { size: size, createdAt: createdAt };
	}

	static async recordCacheFile(db, filename, options) {
		options = options || {};
		const stored = Utils.cacheStoredFile(filename);
		if (!stored) {
			await db.runAsync('DELETE FROM cache_files WHERE filename=?;', [filename]);
			return false;
		}
		const updateCreatedAt = options.refreshCreatedAt ? ', created_at=excluded.created_at' : '';
		await db.runAsync(
			`INSERT INTO cache_files(filename, created_at, size_bytes, cache_type) VALUES (?, ?, ?, ?) ON CONFLICT(filename) DO UPDATE SET size_bytes=excluded.size_bytes, cache_type=excluded.cache_type${updateCreatedAt};`,
			[filename, stored.createdAt, stored.size, Utils.cacheFileType(filename)]
		);
		return true;
	}

	static async reconcileCacheFiles(db) {
		let lastFilename = '';
		while (true) {
			const rows = await db.allAsync(
				'SELECT DISTINCT filename FROM cachendx WHERE filename > ? ORDER BY filename LIMIT 1000;',
				[lastFilename]
			);
			if (!rows || rows.length === 0)
				break;
			for (const row of rows) {
				lastFilename = row.filename;
				await Utils.recordCacheFile(db, row.filename);
			}
			if (rows.length < 1000)
				break;
		}
		await db.runAsync('DELETE FROM cache_files WHERE filename NOT IN (SELECT DISTINCT filename FROM cachendx);');
	}

	static async deleteManagedCacheFile(db, filename) {
		const candidates = [filename, Utils.cacheGzipFile(filename)];
		try {
			const directory = path.dirname(filename);
			const prefix = `${path.basename(filename)}${Utils.BROWSER_HTML_SUFFIX}-`;
		...
		await db.runAsync('DELETE FROM cachendx WHERE filename=?;', [filename]);
		await db.runAsync('DELETE FROM cache_files WHERE filename=?;', [filename]);
	}

	static async enforceCacheLimit(db, type, limitBytes) {
		const where = type ? 'WHERE cache_type=?' : '';
		const params = type ? [type] : [];
		const totals = await db.allAsync(`SELECT COALESCE(SUM(size_bytes), 0) AS bytes FROM cache_files ${where};`, params);
		let bytes = Number(totals?.[0]?.bytes) || 0;
		let files = 0;
		while (bytes > limitBytes) {
			const rows = await db.allAsync(
				`SELECT filename, size_bytes FROM cache_files ${where} ORDER BY created_at, filename LIMIT 1000;`,
				params
			);
			if (!rows || rows.length === 0)
				break;
		...
		return { files: files, bytes: Math.max(0, bytes) };
	}

	static async runCacheMaintenance(options) {
		options = options || {};
		if (!Utils.diskCacheEnabled())
			return { disabled: true };
		if (!options.force && Date.now() - Utils.CACHE_MAINTENANCE_LAST_RUN < Utils.cacheCleanupIntervalMs())
			return null;
		if (Utils.CACHE_MAINTENANCE_PENDING)
			return Utils.CACHE_MAINTENANCE_PENDING;
		Utils.CACHE_MAINTENANCE_PENDING = (async () => {
			let lockAcquired = false;
			try {
				await fs.mkdir(Utils.CACHE_MAINTENANCE_LOCK);
				lockAcquired = true;
			} catch (error) {
				if (error.code === 'EEXIST') {
					try {
						const stats = await fs.stat(Utils.CACHE_MAINTENANCE_LOCK);
						if (Date.now() - stats.mtimeMs > 30 * 60 * 1000) {
							await fs.rmdir(Utils.CACHE_MAINTENANCE_LOCK);
							await fs.mkdir(Utils.CACHE_MAINTENANCE_LOCK);
							lockAcquired = true;
						} else {
							return { skipped: true };
			...
			try {
				const db = options.db || await Utils.setupCacheIndex();
				if (options.reconcile) {
					await Utils.purgeStaleCacheIndexEntries(db);
					await Utils.reconcileCacheFiles(db);
				}
				const tafsir = await Utils.enforceCacheLimit(db, 'tafsir', Utils.cacheLimitBytes('tafsir'));
				const total = await Utils.enforceCacheLimit(db, null, Utils.cacheLimitBytes());
				Utils.CACHE_MAINTENANCE_LAST_RUN = Date.now();
				if (tafsir.files || total.files)
					debug(`FIFO cache eviction removed ${tafsir.files + total.files} files`);
				return { tafsir: tafsir, total: total };
			} finally {
				if (lockAcquired)
					await fs.rmdir(Utils.CACHE_MAINTENANCE_LOCK).catch(error => {
						if (error.code !== 'ENOENT')
							debug('unable to release cache maintenance lock:', error);
					});
			}
		})().finally(() => {
			Utils.CACHE_MAINTENANCE_PENDING = null;
		});
		return Utils.CACHE_MAINTENANCE_PENDING;
	}

	static scheduleCacheMaintenance() {
		if (Date.now() - Utils.CACHE_MAINTENANCE_LAST_RUN < Utils.cacheCleanupIntervalMs())
			return;
		setImmediate(() => Utils.runCacheMaintenance().catch(error => debug('cache maintenance failed:', error)));
	}

	static scheduleCacheFileRefresh(filename) {
		setImmediate(async () => {
			try {
				const db = await Utils.setupCacheIndex();
				await Utils.recordCacheFile(db, filename);
				Utils.scheduleCacheMaintenance();
			} catch (error) {
				debug('unable to refresh cache file metadata:', error);
			}
		});
	}
	...
	static async indexCachedItem(keys, value) {
		...
		await Utils.recordCacheFile(cachedb, value, { refreshCreatedAt: true });
		Utils.scheduleCacheMaintenance();
	}

	static async flushCachedFile(filename, options) {
		options = options || {};
		let deleted = false;
		const failures = [];
		...
		try {
			...
			if (error && error.code !== 'ENOENT') {
				debug('unable to flush browser-ready cache files:', error);
				failures.push(error);
			}
		}
		try {
			...
			await cachedb.runAsync(`DELETE FROM cache_files WHERE filename=?;`, [filename]);
		} catch (error) {
	...
	static async flushCacheContaining(key, options) {
		options = options || {};
		...
		try {
			...
			if (rows) {
				for (const row of rows)
					await Utils.flushCachedFile(row.filename, { strict: strict });
	...
	static async flushBookDiskCache(alias, options) {
		options = options || {};
		alias = Utils.trimToEmpty(alias).toLowerCase();
		if (!/^[a-z0-9][a-z0-9._-]*$/.test(alias) || Utils.safeFilename(alias) !== alias)
			throw new Error(`Invalid book cache alias: ${alias || '(empty)'}`);
		const cacheRoot = path.resolve(options.cacheRoot || `${homedir}/.hadithdb/cache`);
		const cachedb = options.db || await Utils.setupCacheIndex();
		const targets = new Set();
		const indexed = await cachedb.allAsync('SELECT DISTINCT filename FROM cachendx WHERE id IN (?, ?);', [alias, `book:${alias}`]);
		for (const row of indexed || []) {
			const filename = path.resolve(row.filename);
			const relative = path.relative(cacheRoot, filename);
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				const bookDirectory = path.join(entryPath, alias);
				try {
					await fs.lstat(bookDirectory);
					targets.add(bookDirectory);
				} catch (error) {
					if (error.code !== 'ENOENT') throw error;
				}
			} else {
				const base = entry.name.replace(/\.gz$/, '');
				if (base === `_${alias}` || base.startsWith(`_${alias}.`) || base.startsWith(`_${alias}_`) || base.startsWith(`_${alias}?`))
		...
		for (const target of targets) {
			const stats = await Utils.cachePathCounts(target);
			files += stats.files;
			directories += stats.directories;
			await fs.rm(target, { recursive: true, force: true });
			const prefix = `${target}${path.sep}`;
			await cachedb.runAsync('DELETE FROM cachendx WHERE filename=? OR substr(filename, 1, ?)=?;', [target, prefix.length, prefix]);
			await cachedb.runAsync('DELETE FROM cache_files WHERE filename=? OR substr(filename, 1, ?)=?;', [target, prefix.length, prefix]);
		}
		await cachedb.runAsync('DELETE FROM cachendx WHERE id IN (?, ?);', [alias, `book:${alias}`]);
		return { alias, files, directories, targets: targets.size };
	}

	static async cachePathCounts(target) {
		try {
			...
			for (const entry of await fs.readdir(target, { withFileTypes: true })) {
				const child = await Utils.cachePathCounts(path.join(target, entry.name));
				files += child.files;
				directories += child.directories;
			}
			return { files, directories };
		} catch (error) {
			if (error.code === 'ENOENT') return { files: 0, directories: 0 };
			throw error;
		}
	}
