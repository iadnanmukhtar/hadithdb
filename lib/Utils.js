  ...
  ${Utils.epubFootnoteAsides(footnoteContext)}
	...
	static epubLocalizedParagraphs(value, quranText, footnoteContext) {
		if (!value)
			return '';
		return [
			Utils.epubParagraph(value.en, '', 'en', false, footnoteContext),
			Utils.epubParagraph(value.ar, 'rtl', 'ar', quranText, footnoteContext)
		].join('');
	}

	static epubParagraph(text, dir, lang, quranText, footnoteContext) {
		text = Utils.trimToEmpty(text);
		if (!text)
			return '';
		var html = Utils.epubTextWithFootnoteRefs(text, lang, footnoteContext);
		var attrs = [
			dir ? `dir="${dir}"` : '',
			lang ? `lang="${lang}"` : ''
		].filter(Boolean).join(' ');
		return `<p${attrs ? ` ${attrs}` : ''}${quranText ? ' class="quran-text"' : ''}>${html}</p>`;
	}
	...
	static epubFootnoteContext(item) {
		...
		return {
			base: base || 'item',
			notes: {
				en: Utils.epubParseFootnotes(item.footnotes?.en, 'en', base),
				ar: Utils.epubParseFootnotes(item.footnotes?.ar, 'ar', base)
			},
			used: new Set()
		};
	}

	static epubParseFootnotes(value, lang, base) {
    ...
    <p><a href="#${note.id}-ref">[${Utils.escXml(note.key)}]</a> ${Utils.escXml(note.text)}</p>
  </aside>`);
			});
		});
