...
`;
	...
	static adminGearHtml(editMode, mobile) {
		...
		var onclick = "var editMode=(document.cookie ? document.cookie.match(/editMode=1/) : false); editMode ? document.cookie='editMode=0;path=/;max-age=7776000;samesite=lax' : document.cookie='editMode=1;path=/;max-age=7776000;samesite=lax'; try { localStorage.setItem('hadithdb_edit_mode', editMode ? '0' : '1'); } catch (err) {} location.reload();";
		if (mobile)
