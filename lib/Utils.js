...
class Utils {
	...
	static CACHENDX;
	static CACHE_SUFFIX_SLUG = '.v233';
	static CACHE_SUFFIX_SLUG_HEADER_FOOTER = '.v233';
	static SCRIPT_ASSET_SUFFIX_SLUG = '.v233';
	static CRC32_TABLE;
	static CACHE_GZIP_SUFFIX = '.gz';
	static _htmlCacheRecompressWorkers = new Set();
...
`;
	...
	static htmlCacheFragmentSuffix(kind) {
		if (kind === 'header' || kind === 'footer')
			return Utils.CACHE_SUFFIX_SLUG_HEADER_FOOTER;
		return Utils.cacheSuffix();
	}
	...
	static cachedTextPathForRead(cachedFile) {
		const gzipFile = Utils.cacheGzipFile(cachedFile);
		if (fsSync.existsSync(gzipFile) && (!fsSync.existsSync(cachedFile) || Utils.isCachedGzipFresh(cachedFile)))
			return gzipFile;
	...
	static cacheHtmlFragment(kind, html) {
		...
		var filename = `${kind}-${hash}${Utils.htmlCacheFragmentSuffix(kind)}.html`;
	...
	static writeCachedTextFile(cachedFile, value) {
		...
		Utils.recompressCachedFileAsync(cachedFile);
	}
	...
	static readCachedTextFile(cachedFile) {
		...
		try {
			...
			const gzipPath = Utils.cacheGzipFile(cachedFile);
			try {
				if (cachedPath === cachedFile && fsSync.existsSync(gzipPath))
					return zlib.gunzipSync(fsSync.readFileSync(gzipPath)).toString('utf8');
	...
	static readCachedHtmlResponse(cachedFile, req) {
		...
		if (!Utils.shouldUseGzipEncoding(req))
			return { encoding: null, body: Buffer.from(String(html || ''), 'utf8') };
		try {
			return { encoding: 'gzip', body: zlib.gzipSync(Buffer.from(String(html || ''), 'utf8')) };
		} catch (error) {
	...
	static readCachedTextResponse(cachedFile, req) {
		...
		try {
			return { encoding: 'gzip', body: zlib.gzipSync(Buffer.from(String(text || ''), 'utf8')) };
		} catch (error) {
			debug('unable to gzip cached response:', error);
			return { encoding: null, body: Buffer.from(String(text || ''), 'utf8') };
		}
	}
	...
	static recompressCachedFileAsync(cachedFile) {
		if (!cachedFile || !/\.(html|xml|txt|rss|atom|json)$/i.test(cachedFile))
			return;
		if (Utils._htmlCacheRecompressWorkers.has(cachedFile))
			return;
		Utils._htmlCacheRecompressWorkers.add(cachedFile);
		const workerFile = path.join(__dirname, 'workers', 'recompressCachedHtml.js');
		if (!fsSync.existsSync(workerFile)) {
			Utils._htmlCacheRecompressWorkers.delete(cachedFile);
			return;
		}
		setTimeout(() => {
			try {
				...
				worker.on('error', (error) => {
					debug('cache gzip rebuild worker error:', error);
					Utils._htmlCacheRecompressWorkers.delete(cachedFile);
				});
				worker.on('message', (message) => {
					...
					if (message.type === 'done')
						...
						debug(`cache gzip rebuild file error: ${message.file} (${message.error})`);
				});
				worker.on('exit', (code) => {
					if (code && code !== 0)
						debug(`cache gzip rebuild worker exited with code ${code}`);
					Utils._htmlCacheRecompressWorkers.delete(cachedFile);
				});
			} catch (error) {
				debug('unable to start cache gzip rebuild worker:', error);
				Utils._htmlCacheRecompressWorkers.delete(cachedFile);
			}
		}, 0);
	}
