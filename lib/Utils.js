...
<head>
  ...
  <style>
    ...
    .hadith-number { font-weight: 700; white-space: nowrap; }
  ...
  ${Utils.epubFootnoteAsides(footnoteContext)}
	...
	static epubNormalizeItem(item) {
		...
		return {
			...
			title: {
				en: item.title_en || item.part_en || item.ref || item.num,
	...
	static epubHadithTextWithNumbers(item) {
		if (Utils.epubIsQuranItem(item))
			return item.text;
		var ref = Utils.epubHadithReference(item);
		var attribution = Utils.epubHadithAttribution(item);
		return {
			...item.text,
			en: Utils.epubAppendInlineAttribution(Utils.epubPrefixText(item.text?.en, ref?.en), attribution?.en),
			ar: Utils.epubAppendInlineAttribution(Utils.epubPrefixText(item.text?.ar, ref?.ar), attribution?.ar)
		};
	}
	...
	static epubHadithReference(item) {
		...
		return {
			en: ref,
			ar: Utils.hadithReferenceArabic(ref)
		};
	}
	...
	static epubHadithAttribution(item) {
		...
		var sourceRef = Utils.trimToEmpty(item?.source?.ref || '');
		return {
			en: [sourceRef, gradeEn].filter(Boolean).join(' '),
			ar: gradeAr
		};
	}

	static hadithReferenceArabic(ref) {
		return Utils.trimToEmpty(ref).replace(/\d+/g, match => Arabic.toArabicDigits(match));
	}
	...
	static epubPrefixText(text, ref) {
		...
		if (new RegExp(`^(?:<[^>]+>)*${escaped.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`).test(text))
			return text;
		return `<span class="hadith-number">${escaped}</span>. ${text}`;
	}
