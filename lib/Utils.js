...
`;
	...
	static scriptAssetVersion() {
		if (!Utils.SCRIPT_ASSET_VERSION) {
			try {
				const script = Buffer.concat(['script.js', 'notebook.js', 'notebook-drive.js', 'notebook-wiki.js', 'account-controls.js'].map(filename => fsSync.readFileSync(path.join(__dirname, '../public/static/js', filename))));
	...
	static injectCachedAssetVersions(html) {
		...
		return Utils.rewriteStaticAssetUrls(html).toString()
			...
			.replace(
				/(<script\s+src="\/static\/js\/(?:script|notebook|notebook-drive|notebook-wiki|account-controls)\.js)(?:\?v=[^"]*)?("[^>]*><\/script>)/g,
