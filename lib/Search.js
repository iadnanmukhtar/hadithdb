...
async function a_search(qs, b, lang, offset) {
	...
	if (selectedFilters.length > 0) {
		var filterClauses = [];
		var itoc = selectedFilters.indexOf('toc');
		if (itoc >= 0)
			filterClauses.push('doctype:toc');
		var bookAliases = expandBookFilters(selectedFilters.filter(filter => filter !== 'toc'));
		if (bookAliases.length > 0)
			filterClauses.push('(book_alias:' + bookAliases.join(' OR book_alias:') + ')');
		if (filterClauses.length > 0)
			bookFilter = ' AND ' + filterClauses.join(' AND ');
	}
