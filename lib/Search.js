...
class Search {
	...
	static async a_lookupQuranByRange(surah, ayah1, ayah2) {
		...
		var result = await global.query(`SELECT * FROM hadiths
				WHERE
					bookId = 0
				AND h1 = ${surah}
				AND (numInChapter BETWEEN ${ayah1} AND ${ayah2})`);
		var passage_en = '', passage = '';
		if (result.length > 0) {
			for (var i = 0; i < result.length; i++) {
				var ayah = new Hadith(result[i]);
				await Hadith.a_dbHadithInit(ayah);
				if (i == 0) {
					results.chapter = ayah.chapter;
					results.section = ayah.section;
					passage_en += `<sup>${ayah.h1}:${ayah.numInChapter}</sup>`;
					passage += `<sup>${Arabic.toArabicDigits(''+ayah.h1)}:${Arabic.toArabicDigits(''+ayah.numInChapter)}</sup>`;
				} else {
					passage_en += ` <sup>${ayah.numInChapter}</sup>`;
					passage += ` <sup>${Arabic.toArabicDigits(''+ayah.numInChapter)}</sup>`;
				}
				passage_en += ` ${ayah.body_en}`;
				passage += ` ${ayah.body} ۝ `;
			}
			results.passage_en = passage_en.trim();
			results.passage = passage.trim();
		} else
