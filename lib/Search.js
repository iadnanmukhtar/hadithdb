...
function allowsQuranSuggestions(selectedFilters) {
	...
	if (!selectedFilters || selectedFilters.length < 1)
		return true;
	var bookFilters = expandBookFilters(selectedFilters.filter(filter => filter !== 'toc' && filter !== 'commentaries' && filter !== 'sharh' && !isSharhBookAlias(filter)));
...
function isSharhSearch(filters) {
	return (filters || []).includes('sharh') || selectedSharhBooks(filters).length > 0;
}
...
function buildSearchFilters(selectedFilters, options = {}) {
	...
	if (isSharhSearch(selectedFilters) || sharhBooks.length) doctypes.push('sharh');
	if (sharhBooks.length) filters.push({ terms: { bookId: sharhBooks.map(book => Number(book.id)) } });
	...
	var bookAliases = expandBookFilters(selectedFilters.filter(filter => filter !== 'toc' && filter !== 'commentaries' && filter !== 'sharh' && !isSharhBookAlias(filter)));
