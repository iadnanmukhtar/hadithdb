...
const QURAN_SEARCH_ES_LIMIT = 10000;
...
function tafsirAutocompleteSuggestions(qs, lang, selectedFilters) {
	if (!allowsTafsirAutocompleteSuggestion(selectedFilters))
		return [];
	return Tafsir.expandBilingualLocalCommentaries(global.commentaries || [])
		.filter(tafsir => Number(tafsir.hidden) === 0 && Books.matchesQuery(tafsir, qs, lang))
		.slice(0, 3)
...
async function executeQuranThenRestSearch(qs, lang, offset, selectedFilters) {
	...
	var quranDocs = await Index.docsFromQuery(quranOnlyIndexNames(), quranQuery, 0, QURAN_SEARCH_ES_LIMIT, SEARCH_ORDER_BY, true);
	quranDocs.sort(compareSearchResults);
	docs = addUniqueSearchDocs(docs, quranDocs, seen);
	...
	if (pageDocs.length < size) {
		...
		var restSize = size - pageDocs.length;
