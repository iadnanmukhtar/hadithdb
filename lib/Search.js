...
class Search {
	...
	static async a_lookupQuranByRange(surah, ayah1, ayah2) {
		...
		var r = await global.query(`SELECT * FROM hadiths
				WHERE
					bookId = 0
				AND h1 = ${surah}
		...
		if (r.length > 0) {
			...
			results[0].num = `${r[0].h1}:${r[0].numInChapter}-${r[r.length - 1].numInChapter}`;
			results[0].num_ar = `${Arabic.toArabicDigits(r[0].h1)}:${Arabic.toArabicDigits(r[0].numInChapter)}-${Arabic.toArabicDigits(r[r.length - 1].numInChapter)}`;
			results[0].numStart = r[0].numInChapter;
			results[0].numEnd = r[r.length - 1].numInChapter;
