...
class Search {
	...
	static async a_autocomplete(qs, b, limit, options = {}) {
		...
		try {
			...
			var docs = await Index.docsFromQuery('hadiths,toc,commentaries', query, 0, remaining, SEARCH_ORDER_BY, true);
...
async function executeCanonicalSearchQuery(qs, query, offset, selectedFilters, options = {}, lang) {
	...
	var indexNames = lang ? quranFirstRestIndexNames(selectedFilters) : 'hadiths,toc,commentaries';
	var indexedTotal = 0;
	if (indexNames) {
...
function quranFirstRestIndexNames(selectedFilters) {
	...
	return 'hadiths,toc,commentaries';
}
...
function firstHighlightedFragment(doc, lang) {
	...
	var fields = lang === 'ar'
				? (isQuranAyah
					...
					: ['text', 'footnotes', ...TRANSLATED_COMMENTARY_HIGHLIGHT_FIELDS, 'body', 'body_search_ar', 'body_start', 'sharh', ...TRANSLATED_HADITH_HIGHLIGHT_FIELDS, 'book_name_search_ar', 'book_shortName_search_ar', 'title_search_ar', 'title', 'footnote', 'chain', 'intro_search_ar', 'intro', 'h1_title', 'h2_title', 'h3_title', 'h1_intro', 'h2_intro', 'h3_intro'])
				: (isQuranAyah
