...
async function hydrateCommentaryAyahs(results) {
	...
	if (ranges.length < 1)
		return;
	var clauses = ranges.map(range => `(h1=${range.surah} AND numInChapter BETWEEN ${range.ayahFrom} AND ${range.ayahTo})`).join(' OR ');
	var rows = await global.query(`
		SELECT h1 AS surah, h1_title_en, h1_title, numInChapter AS ayah, body
		FROM v_hadiths
		WHERE book_alias='quran' AND (${clauses})
...
function firstHighlightedFragment(doc, lang) {
	...
	if (!highlight)
		return '';
	var fields = lang === 'ar'
			? ['text', 'footnotes', 'commentary_name', 'commentary_shortName', 'commentary_author', 'body', 'book_name_search_ar', 'book_shortName_search_ar', 'title_search_ar', 'title', 'footnote', 'chain', 'intro_search_ar', 'intro', 'h1_title', 'h2_title', 'h3_title', 'h1_intro', 'h2_intro', 'h3_intro']
