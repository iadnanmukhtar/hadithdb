...
function isSharhSearch(filters) {
	return !(filters || []).includes('hadith') && ((filters || []).includes('sharh') || selectedSharhBooks(filters).length > 0);
}
...
function expandBookFilters(filters) {
	...
	for (var i = 0; i < filters.length; i++) {
		...
		if (filter === 'hadith' || filter === 'sharh' || isSharhBookAlias(filter)) continue;
		if (BOOK_FILTER_GROUPS[filter])
