...
function tafsirAutocompleteSuggestions(qs, lang, selectedFilters, options = {}) {
	...
	return Tafsir.visibleTafsirsSync()
		.filter(tafsir => !options.tafsirAlias || tafsir.alias === options.tafsirAlias)
...
function buildSearchFilters(selectedFilters, options = {}) {
	...
	if (options.tafsirAlias)
		filters.push({ term: { commentary_alias: options.tafsirAlias } });
	if (!selectedFilters || selectedFilters.length < 1)
		return filters;
	var doctypes = [];
...
function addSearchOptionFilters(query, options = {}) {
	var searchOptions = normalizeSearchOptions(options);
	if (!searchOptions.tafsirAlias)
		return query;
	return {
		bool: {
			must: query,
			filter: [
				{ term: { commentary_alias: searchOptions.tafsirAlias } }
			]
		}
	};
}
