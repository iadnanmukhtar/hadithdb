	...
	static async a_autocomplete(qs, b, limit) {
		...
		try {
			...
			var generated = quranAutocompleteSuggestions(qs, suggestionLang, selectedFilters)
				.concat(quranSurahAutocompleteSuggestions(qs, suggestionLang, selectedFilters))
				.concat(bookReferenceAutocompleteSuggestions(qs, suggestionLang, selectedFilters))
				.concat(tafsirAutocompleteSuggestions(qs, suggestionLang, selectedFilters))
			...
			if (allowsQuranAyahMemorySearch(selectedFilters))
...
function quranSurahAutocompleteSuggestions(qs, lang, selectedFilters) {
	if (!allowsQuranSuggestions(selectedFilters))
		return [];
...
function tafsirAutocompleteSuggestions(qs, lang, selectedFilters) {
	if (!allowsTafsirAutocompleteSuggestion(selectedFilters))
		return [];
	return Tafsir.visibleTafsirsSync()
...
function autocompleteSearchAliases(data, lang) {
	...
	if (lang === 'ar') {
		aliases = aliases.concat([
			data.book_author_ar || data.author_ar,
...
function inMemorySurahMatches(query, lang) {
	...
	if (tokens.length < 1)
		return [];
	return Surahs.surahs.filter(function (surah) {
...
function formatQuranSurahSuggestion(surah, lang) {
	var titleEn = `Surah ${surah.name_en}`;
	var titleAr = `سورة ${surah.name_ar}`;
	var title = lang === 'ar' ? titleAr : titleEn;
...
function quranSurahRevelation(num) {
	var isMadani = MADANI_SURAHS.has(parseInt(num, 10));
	return {
		en: isMadani ? 'Madani' : 'Makki',
		ar: isMadani ? 'مدنية' : 'مكية'
	};
}
...
async function a_search(qs, b, lang, offset) {
	var selectedFilters = normalizeBookFilters(b);
	if (allowsQuranAyahMemorySearch(selectedFilters))
		return await executeQuranMemoryThenRestSearch(qs, lang, offset, selectedFilters);
	var query = buildBookOrdinalQuery(qs, lang, selectedFilters);
	return await executeSearchQuery(qs, query, offset, lang, selectedFilters);
}
...
async function executeSearchQuery(qs, query, offset, lang, selectedFilters) {
	...
	var seen = new Set();
	var generatedDocs = lang ? generatedSearchSuggestionDocs(qs, lang, selectedFilters) : [];
...
function generatedSearchSuggestionDocs(qs, lang, selectedFilters) {
	var suggestions = quranAutocompleteSuggestions(qs, lang, selectedFilters)
		.concat(quranSurahAutocompleteSuggestions(qs, lang, selectedFilters))
		.concat(tafsirAutocompleteSuggestions(qs, lang, selectedFilters))
		.concat(bookAutocompleteSuggestions(qs, lang, selectedFilters));
...
async function executeQuranMemoryThenRestSearch(qs, lang, offset, selectedFilters) {
	offset = Number.isInteger(offset) ? offset : 0;
	if (offset >= SEARCH_ES_RESULT_LIMIT)
		return searchResultsWithTotal([], SEARCH_ES_RESULT_LIMIT);
	var size = elasticSearchPageSize(offset);
	var seen = new Set();
	var generatedDocs = generatedSearchSuggestionDocs(qs, lang, selectedFilters);
	var docs = addUniqueSearchDocs([], generatedDocs, seen);
	var quranMemoryDocs = await inMemoryQuranAyahSearch(qs, lang);
	logInMemoryQuranHits(qs, lang, quranMemoryDocs);
...
function formatBookAutocompleteSuggestion(doc, lang) {
	...
	var shortNameAr = Utils.trimToEmpty(doc.book_name || doc.name || book.name || doc.book_shortName || doc.shortName || book.shortName || bookAlias);
	...
	var authorAr = Utils.trimToEmpty(doc.book_author_ar || doc.author_ar || book.author_ar || doc.book_author || doc.author);
	var death = doc.book_author_death || doc.author_death || doc.yearOfDeath || doc.death;
...
function formatTafsirAutocompleteSuggestion(tafsir, lang) {
	...
	var nameAr = Utils.trimToEmpty(tafsir.name || shortNameAr);
...
function allowsQuranAyahMemorySearch(selectedFilters) {
...
function commentaryFragmentHtml(item, lang) {
	var fragment = highlightedCommentaryFragment(item, lang);
	if (!fragment)
...
function highlightedCommentaryFragment(item, lang) {
	...
	if (!highlight)
		return '';
	var fields = lang === 'ar'
		? ['text', 'footnotes', 'commentary_name', 'commentary_shortName', 'commentary_author']
		: ['text_en', 'footnotes_en', 'commentary_name_en', 'commentary_shortName_en', 'commentary_author_en'];
	for (var i = 0; i < fields.length; i++) {
		var fragments = highlight[fields[i]];
		if (!Array.isArray(fragments))
			continue;
