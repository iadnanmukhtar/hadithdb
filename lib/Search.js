...
class Search {
	...
	static async a_lookupQuranByRange(num, ayah1, ayah2) {
		...
		var rs = await global.query(`SELECT id FROM hadiths
				WHERE
					bookId = 0
				AND h1 = ${num}
				AND (numInChapter BETWEEN ${ayah1} AND ${ayah2})`);
		rs = await Index.docsFromObjectArray(Item.INDEX, rs, 'id');
		var body_en = '', body = '';
		if (rs.length > 0) {
			var firstAyah = null;
			for (var i = 0; i < rs.length; i++) {
				var ayah = new Item(rs[i]);
				if (i == 0) {
					firstAyah = ayah;
...
async function a_search(qs, b, lang, matchType) {
	...
	var _results = await Index.docsFromQueryString('hadiths,toc', queryString);
	var items = [];
