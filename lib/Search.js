...
class Search {
	...
	static async a_autocomplete(qs, b, limit) {
		...
		try {
			...
			var docs = await Index.docsFromQuery('hadiths,toc', query, 0, limit, SEARCH_ORDER_BY, true);
...
function firstHighlightedFragment(doc, lang) {
	...
	if (!highlight)
		return '';
	var fields = lang === 'ar'
		? ['body', 'title_search_ar', 'title', 'footnote', 'chain', 'intro_search_ar', 'intro', 'h1_title', 'h2_title', 'h3_title', 'h1_intro', 'h2_intro', 'h3_intro']
