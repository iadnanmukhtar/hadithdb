...
async function executeCanonicalSearchQuery(qs, query, offset, selectedFilters, options = {}, lang) {
	...
	if (lang && allowsQuranAyahMemorySearch(selectedFilters, options)) {
		...
		logInMemoryQuranHits(qs, lang, quranDocs);
	...
	var indexedTotal = 0;
	if (indexNames) {
		var indexedDocs = await Index.docsFromQuery(indexNames, query, 0, SEARCH_ES_RESULT_LIMIT, SEARCH_ORDER_BY, true);
	...
	results.total = cappedSearchTotal(suggestions.length + Math.max(canonicalDocs.length, indexedTotal));
	return results;
}
