...
const BookGroups = require('./BookGroups');
...
const TRANSLATED_HADITH_SEARCH_FIELDS = Object.freeze(HadithTranslationIndexView.searchFields());
const TRANSLATED_HADITH_HIGHLIGHT_FIELDS = Object.freeze(TRANSLATED_HADITH_SEARCH_FIELDS.map(field => field.replace(/\^.*$/, '')));
const TRANSLATED_COMMENTARY_SEARCH_FIELDS = Object.freeze(CommentaryTranslationIndexFields.searchFields());
const TRANSLATED_COMMENTARY_HIGHLIGHT_FIELDS = Object.freeze(CommentaryTranslationIndexFields.highlightFields());
const TRANSLATED_COMMENTARY_TEXT_HIGHLIGHT_FIELDS = Object.freeze(TRANSLATED_COMMENTARY_HIGHLIGHT_FIELDS.filter(field => field.indexOf('text_') === 0));
const TRANSLATED_COMMENTARY_FOOTNOTE_HIGHLIGHT_FIELDS = Object.freeze(TRANSLATED_COMMENTARY_HIGHLIGHT_FIELDS.filter(field => field.indexOf('footnote_') === 0));
let quranAyahSearchCache = null;
let quranAyahSearchCachePromise = null;
const QURAN_AYAH_SEARCH_SOURCE_FIELDS = Object.freeze([
	'id',
	'hId',
	'ref',
	'path',
...
const SEARCH_FIELDS = Object.freeze({
	sirah: Object.freeze({
		ar: Object.freeze(['body^5', 'title_search_ar^4', 'title^4', 'h1_title^3', 'h2_title^3', 'h3_title^3', 'book_name_search_ar^2', 'book_shortName_search_ar^2', 'footnote']),
		en: Object.freeze(['body_en^5', 'title_en^4', 'book_name_search_en^3', 'book_shortName_search_en^3'])
	}),
	hadith: Object.freeze({
		ar: Object.freeze([
			...
			'footnote^2',
			'chain',
			'narrator_names_search',
			...TRANSLATED_HADITH_SEARCH_FIELDS
		]),
		en: Object.freeze([
			'body_en^5',
			'body_en_search^5',
	...
	commentary: Object.freeze({
		ar: Object.freeze([
			'text^5',
			'footnotes^2',
			...TRANSLATED_COMMENTARY_SEARCH_FIELDS
		]),
		en: Object.freeze([
			'text_en^5',
			'footnotes_en^2',
...
class Search {
	...
	static async a_searchText(qs, b, offset, options = {}) {
		...
		try {
			...
			if (options.redactLogs === true)
				debug.error('Unable to perform redacted text search.');
			else
	...
	static async a_autocomplete(qs, b, limit, options = {}) {
		...
		try {
			...
			generated = isSharhSearch(selectedFilters) ? [] : dedupeAutocompleteSuggestions(generated);
			if (allowsQuranAyahMemorySearch(selectedFilters, searchOptions))
				return await quranMemoryThenRestAutocompleteSuggestions(qs, lang, selectedFilters, generated, limit, searchOptions);
			...
			var docs = await Index.docsFromQuery('hadiths,toc,commentaries,sharhs', query, 0, remaining, SEARCH_ORDER_BY, true);
	...
	static async a_similarHadithAutocomplete(qs, limit, exclusions = {}) {
		qs = Search.truncateQuery(qs);
		limit = parseInt(limit, 10);
		if (!Number.isInteger(limit) || limit < 1)
			limit = 10;
		...
		if (!hasMinimumSearchCharacters(qs) || Search.isExpressionQuery(qs))
			return [];
		var excludedIds = Array.from(new Set([].concat(exclusions.ids || [])
			.map(id => parseInt(id, 10))
			.filter(id => Number.isInteger(id) && id > 0)));
		var excludedRefs = new Set([].concat(exclusions.refs || [])
			.map(ref => Utils.trimToEmpty(ref).replace(/^\//, ''))
			.filter(Boolean));
		var lang = Arabic.isArabic(qs) ? 'ar' : 'en';
		var suggestions = [];
		var reference = Books.findReference(qs, global.books);
		if (reference && reference.book && reference.book.alias !== 'quran' && !excludedRefs.has(reference.ref)) {
			var referenceDocs = await Index.docsFromKeyValue('hadiths', { ref: reference.ref }, 0, 1);
			var referenceDoc = referenceDocs.find(doc => !excludedIds.includes(Number(doc.hId || doc.id)));
			var referenceSuggestion = bookReferenceAutocompleteSuggestions(qs, lang, [], { excludeQuranAndTafsir: true })[0];
			if (referenceDoc && referenceSuggestion) {
				referenceSuggestion.id = Number(referenceDoc.hId || referenceDoc.id);
				suggestions.push(referenceSuggestion);
			}
		}
		var mustNot = [{ term: { book_alias: 'quran' } }];
		if (excludedIds.length)
			mustNot.push({ terms: { hId: excludedIds } });
		if (excludedRefs.size)
			mustNot.push({ terms: { ref: Array.from(excludedRefs) } });
		var query = {
			bool: {
				must: buildDoctypeSearchClause('hadith', qs, lang, qs.split(/\s+/).filter(Boolean).length),
				must_not: mustNot
			}
		};
		var docs = await Index.docsFromQuery('hadiths', query, 0, limit, SEARCH_ORDER_BY, true);
		docs.sort(compareSearchResults);
		suggestions = suggestions.concat(docs.map(doc => formatAutocompleteSuggestion(doc, lang, qs)).filter(suggestion => suggestion && suggestion.id));
		return dedupeAutocompleteSuggestions(suggestions).slice(0, limit);
	}

	static async a_withQuranMushafPages(suggestions) {
		return Promise.all((suggestions || []).map(async function (suggestion) {
			var ref = Utils.trimToEmpty(suggestion && suggestion.ref);
			var match = ref.match(/^quran:(\d+):(\d+)(?:-\d+)?$/);
			if (!suggestion || !suggestion.is_quran || !match)
				return suggestion;
			var page = await QuranMushaf.pageForRef(Number(match[1]), Number(match[2]));
			if (!Number.isInteger(page))
				return suggestion;
			return Object.assign({}, suggestion, { mushaf_page: page });
		}));
	}
	...
	static hadithContentFilters(filters) {
		const selected = normalizeBookFilters(filters);
		if (!selected.some(filter => ['hadith', 'sharh', 'toc'].includes(filter) || isSharhBookAlias(filter)))
			return ['hadith', 'sharh', ...selected];
		return selected;
	}

	static generalContentFilters(filters) {
		const selected = normalizeBookFilters(filters);
		if (!selected.length) return ['hadith', 'sharh', 'sirah', 'quran', 'commentaries'];
		if (selected.some(isSirahBookAlias) && !selected.includes('sirah')) return ['sirah', ...selected];
		if (selected.some(filter => ['hadith', 'sharh', 'sirah', 'quran', 'commentaries', 'translations', 'toc'].includes(filter) || isSharhBookAlias(filter))) return selected;
		return Search.hadithContentFilters(selected);
	}

	static supportsCanonicalQuranSort(filters, options = {}) {
		if (options.quranSearchProxy === true)
			return true;
		const selected = normalizeBookFilters(filters);
		return selected.length > 0
			&& selected.every(filter => ['quran', 'commentaries', 'translations'].includes(filter));
	}

	static describeBookFilters(b) {
		return normalizeBookFilters(b).map(filter => {
			if (filter === 'hadith') return 'Hadith';
			if (filter === 'sirah') return 'History';
			if (filter === 'sharh')
				return 'Sharh';
			if (filter === 'toc')
			...
			if (filter === 'translations')
				return 'Translation';
			var group = BookGroups.list('hadith').find(group => group.id === filter);
...
function quranAutocompleteSuggestions(qs, lang, selectedFilters, options = {}) {
	if (options.generalSearch && !selectedFilters.includes('quran')) return [];
...
function allowsQuranSuggestions(selectedFilters) {
	if (selectedFilters && isSharhSearch(selectedFilters)) return false;
	if (!selectedFilters || selectedFilters.length < 1)
		return true;
	var bookFilters = expandBookFilters(selectedFilters.filter(filter => filter !== 'hadith' && filter !== 'toc' && filter !== 'commentaries' && filter !== 'translations' && filter !== 'sharh' && !isSharhBookAlias(filter)));
	if (bookFilters.length > 0)
		return bookFilters.indexOf('quran') >= 0;
	return selectedFilters.indexOf('commentaries') < 0 && selectedFilters.indexOf('translations') < 0;
}

function bookReferenceAutocompleteSuggestions(qs, lang, selectedFilters, options = {}) {
	if (options.generalSearch) {
		if (!selectedFilters.includes('hadith') && !selectedFilters.includes('sirah')) return [];
		selectedFilters = selectedFilters.filter(filter => !['quran', 'commentaries', 'translations'].includes(filter));
		options = { ...options, excludeQuranAndTafsir: true };
	}
	var reference = Books.findReference(qs, global.books);
...
function allowsBookReferenceSuggestion(bookAlias, selectedFilters) {
	if (selectedFilters?.includes('hadith') && !selectedFilters.includes('sirah') && isSirahBookAlias(bookAlias)) return false;
	if (selectedFilters?.includes('sirah') && !selectedFilters.includes('hadith') && !isSirahBookAlias(bookAlias)) return false;
	if (!selectedFilters || selectedFilters.length < 1)
		return true;
	if (selectedFilters.indexOf('commentaries') >= 0 || selectedFilters.indexOf('translations') >= 0 || selectedFilters.indexOf('toc') >= 0)
		return false;
...
function tafsirAutocompleteSuggestions(qs, lang, selectedFilters, options = {}) {
	if (options.generalSearch && !selectedFilters.includes('commentaries')) return [];
	if (options.excludeQuranAndTafsir)
		return [];
...
function translationAutocompleteSuggestions(qs, lang, selectedFilters, options = {}) {
	if (options.generalSearch && !selectedFilters.includes('quran') && !selectedFilters.includes('translations') && !searchOptionTafsirAliases(options).length) return [];
	if (options.excludeQuranAndTafsir)
		return [];
	if (!allowsTranslationAutocompleteSuggestion(selectedFilters) && !searchOptionTafsirAliases(options).length)
		return [];
	var tafsirAliases = searchOptionTafsirAliases(options);
...
function allowsTranslationAutocompleteSuggestion(selectedFilters) {
	...
	if (!selectedFilters || selectedFilters.length < 1)
		return true;
	return selectedFilters.indexOf('quran') >= 0 || selectedFilters.indexOf('translations') >= 0;
}
...
function allowsBookAutocompleteSuggestion(bookAlias, selectedFilters) {
	...
	if (!selectedFilters || selectedFilters.length < 1)
		return true;
	if (selectedFilters.indexOf('commentaries') >= 0 || selectedFilters.indexOf('translations') >= 0)
		return false;
...
async function quranMemoryThenRestAutocompleteSuggestions(qs, lang, selectedFilters, generated, limit, options = {}) {
	var remaining = limit - generated.length;
	if (remaining < 1)
		return generated.slice(0, limit);
	var quranMemoryDocs = await inMemoryQuranAyahSearch(qs, lang);
	logInMemoryQuranHits(qs, lang, quranMemoryDocs, options);
	...
	if (remaining > 0) {
		var restIndexNames = quranFirstRestIndexNames(selectedFilters, options);
		if (!restIndexNames)
			return suggestions.slice(0, limit);
		var restQuery = buildQuranFirstRestSearchQuery(qs, lang, selectedFilters, options);
		var restDocs = await Index.docsFromQuery(restIndexNames, restQuery, 0, remaining, SEARCH_ORDER_BY, true);
...
async function a_search(qs, b, lang, offset, options = {}) {
	var selectedFilters = normalizeBookFilters(b);
	var searchOptions = normalizeSearchOptions(options);
	if (searchOptions.sort === 'canonical')
		return await executeCanonicalQuranSearch(qs, lang, offset, selectedFilters, searchOptions);
	if (allowsQuranAyahMemorySearch(selectedFilters, searchOptions))
...
async function a_searchExpression(qs, b, offset, options = {}) {
	...
	if (searchOptions.sort === 'canonical')
		return await executeCanonicalSearchQuery(qs, query, offset, selectedFilters, searchOptions);
	return await executeSearchQuery(qs, query, offset, undefined, selectedFilters, searchOptions);
}

async function executeCanonicalQuranSearch(qs, lang, offset, selectedFilters, options = {}) {
	var query = buildQuranFirstRestSearchQuery(qs, lang, selectedFilters, options);
	return await executeCanonicalSearchQuery(qs, query, offset, selectedFilters, options, lang);
}

async function executeCanonicalSearchQuery(qs, query, offset, selectedFilters, options = {}, lang) {
	offset = Number.isInteger(offset) ? offset : 0;
	var resultLimit = searchResultLimit(options);
	if (offset >= resultLimit)
		return searchResultsWithTotal([], resultLimit);
	var generatedDocs = lang ? generatedSearchSuggestionDocs(qs, lang, selectedFilters, options) : [];
	var seen = new Set();
	var suggestions = addUniqueSearchDocs([], generatedDocs, seen);
	var canonicalDocs = [];
	if (lang && allowsQuranAyahMemorySearch(selectedFilters, options)) {
		var quranDocs = await inMemoryQuranAyahSearch(qs, lang);
		logInMemoryQuranHits(qs, lang, quranDocs, options);
		canonicalDocs = addUniqueSearchDocs(canonicalDocs, quranDocs, seen);
	}
	var indexNames = lang ? quranFirstRestIndexNames(selectedFilters, options) : 'hadiths,toc,commentaries,sharhs';
	var indexedTotal = 0;
	if (indexNames) {
		var indexedDocs = await Index.docsFromQuery(indexNames, query, 0, resultLimit, SEARCH_ORDER_BY, true);
		indexedTotal = Number.isFinite(indexedDocs.total) ? indexedDocs.total : indexedDocs.length;
		canonicalDocs = addUniqueSearchDocs(canonicalDocs, indexedDocs, seen);
	}
	var allDocs = suggestions.concat(canonicalDocs);
	allDocs.sort(compareCanonicalQuranResults);
	var size = elasticSearchPageSize(offset, options.resultSize, resultLimit);
	var pageDocs = allDocs.slice(offset, offset + size);
	await hydrateCommentaryAyahs(pageDocs);
	var results = await formatSearchDocs(qs, pageDocs, options);
	results.total = cappedSearchTotal(suggestions.length + Math.max(canonicalDocs.length, indexedTotal), resultLimit);
	return results;
}
...
async function executeSearchQuery(qs, query, offset, lang, selectedFilters, options = {}) {
	...
	if (pageDocs.length < size) {
		...
		var indexedDocs = indexedSize > 0
			? await Index.docsFromQuery('hadiths,toc,commentaries,sharhs', query, indexedOffset, indexedSize, SEARCH_ORDER_BY, true)
			: [];
		indexedDocs.sort(compareSearchResults);
		indexedTotal = Number.isFinite(indexedDocs.total) ? indexedDocs.total : indexedDocs.length;
		pageDocs = pageDocs.concat(addUniqueSearchDocs([], indexedDocs, seen));
	} else {
		var countDocs = await Index.docsFromQuery('hadiths,toc,commentaries,sharhs', query, 0, 0, SEARCH_ORDER_BY, false);
		indexedTotal = Number.isFinite(countDocs.total) ? countDocs.total : 0;
	}
	await hydrateCommentaryAyahs(pageDocs);
	var results = await formatSearchDocs(qs, pageDocs, options);
	results.total = cappedSearchTotal(generatedTotal + indexedTotal, resultLimit);
	return results;
}

function generatedSearchSuggestionDocs(qs, lang, selectedFilters, options = {}) {
	if (isSharhSearch(selectedFilters)) return [];
...
async function executeQuranMemoryThenRestSearch(qs, lang, offset, selectedFilters, options = {}) {
	...
	if (pageDocs.length < size) {
		...
		var restSize = Math.min(size - pageDocs.length, resultLimit - offset - pageDocs.length);
		var restIndexNames = quranFirstRestIndexNames(selectedFilters, options);
		if (restIndexNames) {
			var restQuery = buildQuranFirstRestSearchQuery(qs, lang, selectedFilters, options);
	...
	await hydrateCommentaryAyahs(pageDocs);
	var results = await formatSearchDocs(qs, pageDocs, options);
	results.total = cappedSearchTotal(prefixTotal + restTotal, resultLimit);
	return results;
}
...
function elasticSearchPageSize(offset, requestedSize, resultLimit = SEARCH_ES_RESULT_LIMIT) {
	offset = Number.isInteger(offset) ? offset : 0;
	if (offset < 0)
		offset = 0;
	if (offset >= resultLimit)
	...
	if (!Number.isInteger(requested) || requested < 1)
		requested = Number(global.settings.search.itemsPerPage) + 1;
	if (!Number.isFinite(requested) || requested < 1)
		requested = resultLimit;
	return Math.min(requested, resultLimit - offset);
}

function searchResultLimit(options = {}) {
	var requested = Number(options.resultLimit);
	return Number.isInteger(requested) && requested > 0 ? Math.min(requested, 10000) : SEARCH_ES_RESULT_LIMIT;
}

function cappedSearchTotal(total, resultLimit = SEARCH_ES_RESULT_LIMIT) {
	total = Number(total);
	if (!Number.isFinite(total) || total < 0)
		return 0;
	return Math.min(total, resultLimit);
}
...
async function quranAyahRowsForSearch() {
	...
	if (!quranAyahSearchCachePromise) {
		quranAyahSearchCachePromise = loadQuranAyahRowsForSearch()
			.catch(err => {
				quranAyahSearchCachePromise = null;
				throw err;
			});
	}
	return await quranAyahSearchCachePromise;
}

function invalidateQuranAyahSearchCache() {
	quranAyahSearchCache = null;
	quranAyahSearchCachePromise = null;
}

async function rebuildInMemoryCaches() {
	invalidateQuranAyahSearchCache();
	await quranAyahRowsForSearch();
}

async function loadQuranAyahRowsForSearch() {
	var rows = await Index.docsFromQueryFields('hadiths', {
		bool: {
			filter: [
				{ term: { doctype: 'hadith' } },
	...
	}, QURAN_AYAH_SEARCH_SOURCE_FIELDS, 0, 7000, 'h1 ASC, numInChapter ASC', false);
...
function quranSearchNormalizeArabic(value) {
	return QuranReviewCore.quranSearchNormalizeArabic(value);
}

function logInMemoryQuranHits(qs, lang, docs, options = {}) {
	var count = Array.isArray(docs) ? docs.length : 0;
	if (options.redactLogs === true) {
		debug(`[quran-search] in-memory hits lang=${lang} count=${count}`);
		return;
	}
...
async function formatSearchDocs(qs, _results, options = {}) {
	...
	try {
		for (var i = 0; i < _results.length; i++) {
			...
			try {
				...
				if (hadith.doctype === 'sharh') {
					results.push(formatSharhSearchResult(hadith, lang));
					continue;
				}
				...
				if (options.redactLogs === true)
					debug.error(`Redacted search result formatting failed for Hadith ${hadith.bookId}:${hadith.num}`);
				else
					debug.error(`Search broke on Hadith ${hadith.bookId}:${hadith.num} for Query [${qs}]\n${err.stack || err.message}`);
			}
		}
	} catch (err) {
		debug.error(options.redactLogs === true ? 'Redacted search result formatting failed.' : err);
...
function buildSearchQuery(queryText, lang, selectedFilters, options = {}) {
	...
	if (queryText.length > 0) {
		...
		should.push(buildDoctypeSearchClause('sirah', queryText, lang, tokenCount));
		...
		should.push(buildDoctypeSearchClause('sharh', queryText, lang, tokenCount));
	}
...
function quranFirstRestIndexNames(selectedFilters, options = {}) {
	if (options.generalSearch) return 'hadiths,toc,commentaries,sharhs';
	if (isQuranScopedSearch(selectedFilters))
		return 'commentaries';
	return 'hadiths,toc,commentaries,sharhs';
}

function buildDoctypeSearchClause(doctype, queryText, lang, tokenCount) {
	var langFields = SEARCH_FIELDS[doctype === 'sharh' ? 'commentary' : doctype] || SEARCH_FIELDS.hadith;
...
function formatAutocompleteSuggestion(doc, lang, queryText) {
	if (!doc)
		return null;
	if (doc.doctype === 'sharh') {
		const item = formatSharhSearchResult(doc, lang);
		return { label: item.commentary_ref_en, value: item.commentary_ref_en, url: item.url, ref: item.ref, type: 'Sharh', is_quran: false, lang,
			metadata_en: item.commentary_ref_en, metadata_ar: item.commentary_ref_ar, fragment: lang === 'ar' ? item.commentary_fragment_html_ar : item.commentary_fragment_html_en };
	}
	if (doc.doctype === 'commentary')
	...
	var type = doc.doctype === 'toc' ? (isQuranSurahDoc(doc) ? 'Surah' : 'Heading') : (doc.doctype === 'sirah' ? 'History' : (doc.remark == 2 ? 'Ayah' : 'Hadith'));
	...
	return {
		id: Number(doc.hId || doc.id) || undefined,
		label: label,
...
function formatTranslationAutocompleteSuggestion(translation, lang) {
	...
	return {
		label: title,
		value: title,
		url: `/quran/${encodeURIComponent(translation.alias)}`,
...
function formatCommentaryAutocompleteSuggestion(doc, lang) {
	...
	var ref = Utils.trimToEmpty(doc.ref);
	var isTranslation = isTranslationSearchDoc(doc);
	var url = isTranslation ? translationAutocompleteUrl(doc) : commentaryUrl(doc);
	return {
		label: title,
		value: stripHtml(title),
		url: url,
		ref: ref,
		type: isTranslation ? 'Translation' : 'Tafsir',
		is_quran: true,
		lang: lang,
...
function translationAutocompleteUrl(doc) {
	var surah = Number(doc?.surah);
	var ayahFrom = Number(doc?.ayahFrom);
	var ayahTo = Number(doc?.ayahTo);
	var alias = Utils.trimToEmpty(doc?.commentary_alias);
	...
	return `/quran:${surah}:${range}?translation=${encodeURIComponent(alias)}`;
}
...
function firstHighlightedFragment(doc, lang) {
	...
	var fields = lang === 'ar'
				? (isQuranAyah
					...
					: ['text', 'footnotes', ...TRANSLATED_COMMENTARY_HIGHLIGHT_FIELDS, 'body', 'body_search_ar', 'body_start', ...TRANSLATED_HADITH_HIGHLIGHT_FIELDS, 'book_name_search_ar', 'book_shortName_search_ar', 'title_search_ar', 'title', 'footnote', 'chain', 'intro_search_ar', 'intro', 'h1_title', 'h2_title', 'h3_title', 'h1_intro', 'h2_intro', 'h3_intro'])
				: (isQuranAyah
...
function selectedSharhBooks(filters) {
	return (global.books || []).filter(book => book.type === 'sharh' && (filters || []).includes(book.alias));
}

function isSharhBookAlias(alias) {
	return selectedSharhBooks([alias]).length > 0;
}

function isSharhSearch(filters) {
	return !(filters || []).some(filter => ['hadith', 'sirah', 'quran', 'commentaries', 'translations'].includes(filter)) && ((filters || []).includes('sharh') || selectedSharhBooks(filters).length > 0);
}

function formatSharhSearchResult(item, lang) {
	const book = (global.books || []).find(book => book.type === 'sharh' && Number(book.id) === Number(item.bookId));
	if (book) {
		item.sharh_book = book;
		item.commentary_alias = book.alias;
		item.commentary_shortName_en = book.shortName_en || book.name_en || book.alias;
		item.commentary_shortName = book.shortName || book.title || book.alias;
	}
	item.url = '/' + item.ref;
	item.search_lang = lang;
	item.commentary_ref_en = `${item.commentary_shortName_en || item.commentary_alias} · ${item.ref}`;
	const referenceParts = String(item.ref || '').split(':');
	const hadithBook = (global.books || []).find(book => book.alias === referenceParts[0]);
	const arabicReference = [hadithBook && hadithBook.shortName || referenceParts[0], ...referenceParts.slice(1)].join(':');
	item.commentary_ref_ar = `${item.commentary_shortName || item.commentary_alias} · ${Arabic.toArabicDigits(arabicReference)}`;
	...
	item.commentary_is_bilingual = !!Utils.trimToEmpty(item.text) && !!Utils.trimToEmpty(item.text_en);
	return item;
}

function isSirahBookAlias(alias) {
	return (global.books || []).some(book => isHistoryBook(book) && book.alias === alias);
}

function isHistoryBook(book) {
	return !!book && ['sirah', 'history'].includes(book.type);
}

function buildSearchFilters(selectedFilters, options = {}) {
	if (options.generalSearch) {
		const selected = Search.generalContentFilters(selectedFilters);
		const branches = [];
		const hadithFilters = selected.filter(filter => !['quran', 'commentaries', 'translations', 'sirah'].includes(filter) && !isSirahBookAlias(filter));

		if (hadithFilters.some(filter => ['hadith', 'sharh', 'toc'].includes(filter) || isSharhBookAlias(filter)))
			branches.push({ bool: { filter: buildSearchFilters(hadithFilters, { excludeQuranAndTafsir: true }) } });
		if (selected.includes('quran'))
			branches.push({ bool: { filter: buildSearchFilters(['quran']) } });
		if (selected.includes('commentaries'))
			branches.push({ bool: { filter: buildSearchFilters(['commentaries'], { tafsirAliases: searchOptionTafsirAliases(options) }) } });
		if (selected.includes('translations'))
			branches.push({ bool: { filter: buildSearchFilters(['translations']) } });
		if (selected.includes('sirah')) {
			const sirahAliases = selected.filter(isSirahBookAlias);
			branches.push({ bool: { filter: [{ term: { doctype: 'sirah' } }, ...(sirahAliases.length ? [{ terms: { book_alias: sirahAliases } }] : [])] } });
		}
		// TOC documents have their own doctype; scope them by their owning books.
		const contentTypes = ['hadith', 'sharh', 'sirah', 'quran', 'commentaries', 'translations', 'toc'];
		const explicitBooks = expandBookFilters(selected.filter(filter => !contentTypes.includes(filter) && !isSharhBookAlias(filter) && !isSirahBookAlias(filter)));
		const sharhAliases = selectedSharhBooks(selected).map(book => book.alias);
		const sirahAliases = selected.filter(isSirahBookAlias);
		const tafsirAliases = searchOptionTafsirAliases(options);
		const tocAliases = (global.books || []).filter(book => {
			if (Number(book.hidden) === 1 && book.type !== 'sharh') return false;
			if (book.alias === 'quran') return selected.includes('quran');
			if (isHistoryBook(book)) return selected.includes('sirah') && (!sirahAliases.length || sirahAliases.includes(book.alias));
			if (book.type === 'sharh') return (selected.includes('sharh') || sharhAliases.length) && (!sharhAliases.length || sharhAliases.includes(book.alias));
			if (book.type === 'tafsir') return selected.includes('commentaries') && (!tafsirAliases.length || tafsirAliases.includes(book.alias));
			if (book.type === 'trans') return (selected.includes('quran') || selected.includes('translations') || tafsirAliases.includes(book.alias)) && (!tafsirAliases.length || tafsirAliases.includes(book.alias));
			return (selected.includes('hadith') || selected.includes('toc')) && (!explicitBooks.length || explicitBooks.includes(book.alias));
		}).map(book => book.alias);
		if (tocAliases.length) branches.push({ bool: { filter: [{ term: { doctype: 'toc' } }, { terms: { book_alias: tocAliases } }] } });
	...
	if (!selectedFilters || selectedFilters.length < 1)
		return filters;
	if (selectedFilters.every(filter => ['quran', 'commentaries', 'translations'].includes(filter))) {
		var quranBranches = [];
		if (selectedFilters.includes('quran')) {
			quranBranches.push({ bool: { filter: [{ term: { doctype: 'hadith' } }, { term: { book_alias: 'quran' } }] } });
			if (!tafsirAliases.length)
				quranBranches.push({ bool: { filter: [{ term: { doctype: 'commentary' } }, { term: { commentary_type: 'trans' } }] } });
		}
		if (selectedFilters.includes('commentaries')) {
			quranBranches.push({ bool: { filter: [{ term: { doctype: 'commentary' } }, ...(tafsirAliases.length ? [] : [{ term: { commentary_type: 'tafsir' } }])] } });
		}
		if (selectedFilters.includes('translations')) {
			quranBranches.push({ bool: { filter: [{ term: { doctype: 'commentary' } }, { term: { commentary_type: 'trans' } }] } });
		}
		return [...filters, { bool: { should: quranBranches, minimum_should_match: 1 } }];
	}
	var doctypes = [];
	const sharhBooks = selectedSharhBooks(selectedFilters);
	if (selectedFilters.includes('hadith')) doctypes.push('hadith');
	if (selectedFilters.includes('sirah')) doctypes.push('sirah');
	if (selectedFilters.includes('sharh') || sharhBooks.length) doctypes.push('sharh');
	if (sharhBooks.length) {
		const sourceFilter = { terms: { bookId: sharhBooks.map(book => Number(book.id)) } };
		filters.push(selectedFilters.includes('hadith')
			? { bool: { should: [{ term: { doctype: 'hadith' } }, sourceFilter], minimum_should_match: 1 } }
			: sourceFilter);
	}
	...
	if (selectedFilters.indexOf('translations') >= 0) {
		if (!doctypes.includes('commentary'))
			doctypes.push('commentary');
		filters.push({ term: { commentary_type: 'trans' } });
	}
	if (doctypes.length === 1)
		filters.push({ term: { doctype: doctypes[0] } });
	else if (doctypes.length > 1)
		filters.push({ terms: { doctype: doctypes } });
	var bookAliases = expandBookFilters(selectedFilters.filter(filter => filter !== 'hadith' && filter !== 'toc' && filter !== 'commentaries' && filter !== 'translations' && filter !== 'sharh' && !isSharhBookAlias(filter)));
	if (bookAliases.length > 0) {
		filters.push(buildBookAliasFilter(bookAliases));
...
function buildBookAliasFilter(bookAliases) {
	var primaryAliasFilter = { terms: { book_alias: bookAliases } };
	var virtualAliases = bookAliases.filter(function (alias) {
		var book = Array.isArray(global.books)
			? global.books.find(function (candidate) { return candidate && candidate.alias === alias; })
			: null;
		return book && Number(book.virtual) === 1;
	});
	if (virtualAliases.length < 1)
		return primaryAliasFilter;
	return {
		bool: {
			should: [
				primaryAliasFilter,
				{
					query_string: {
						query: virtualAliases.map(function (alias) {
							return `books:"{${escapeElasticQueryStringPhrase(alias)}}"`;
						}).join(' OR ')
					}
				}
			],
...
function escapeElasticQueryStringPhrase(value) {
...
function normalizeSearchOptions(options) {
	...
	return {
		...
		generalSearch: options.generalSearch === true,
		redactLogs: options.redactLogs === true,
		resultSize: Number.isInteger(options.resultSize) && options.resultSize > 0 ? options.resultSize : undefined,
		resultLimit: Number.isInteger(options.resultLimit) && options.resultLimit > 0 ? Math.min(options.resultLimit, 10000) : undefined,
		sort: options.sort === 'canonical' ? 'canonical' : 'relevance'
	};
}
...
function normalizeSearchOptionTafsirAliases(options) {
	...
	return BookGroups.expand(aliases.map(alias => Utils.trimToEmpty(alias)).filter(Boolean), 'tafsir');
}
...
function isQuranCommentarySearch(selectedFilters) {
	return Array.isArray(selectedFilters)
		...
		&& selectedFilters.every(filter => ['quran', 'commentaries'].includes(filter));
}

function isQuranScopedSearch(selectedFilters) {
	if (!Array.isArray(selectedFilters) || selectedFilters.length < 1)
		return false;
	var nonQuranFilters = selectedFilters.filter(filter => !['quran', 'commentaries', 'translations'].includes(filter));
	return nonQuranFilters.length < 1 && selectedFilters.some(filter => ['quran', 'commentaries', 'translations'].includes(filter));
}
...
function compareCanonicalQuranResults(a, b) {
	var refA = canonicalQuranPosition(a);
	var refB = canonicalQuranPosition(b);
	if (refA.surah !== refB.surah)
		return refA.surah - refB.surah;
	if (refA.ayah !== refB.ayah)
		return refA.ayah - refB.ayah;
	if (refA.kind !== refB.kind)
		return refA.kind - refB.kind;
	var aliasCompare = Utils.trimToEmpty(a?.commentary_alias).localeCompare(Utils.trimToEmpty(b?.commentary_alias));
	if (aliasCompare !== 0)
		return aliasCompare;
	return Number(a?.id || a?.hId || 0) - Number(b?.id || b?.hId || 0);
}

function canonicalQuranPosition(doc) {
	var match = Utils.trimToEmpty(doc?.ref || doc?.path).match(/^quran:(\d+):(\d+)/);
	var surah = Number(doc?.surah ?? doc?.h1 ?? (match && match[1]));
	var ayah = Number(doc?.ayahFrom ?? doc?.numInChapter ?? (match && match[2]));
	return {
		surah: Number.isFinite(surah) ? surah : Number.MAX_SAFE_INTEGER,
		ayah: Number.isFinite(ayah) ? ayah : Number.MAX_SAFE_INTEGER,
		kind: doc?.doctype === 'commentary' ? 1 : 0
	};
}
...
function applyElasticHighlights(item, lang) {
	...
	TRANSLATED_COMMENTARY_HIGHLIGHT_FIELDS.forEach(field => applyHighlightField(item, highlight, field));
...
function formatTranslationSearchResult(item, lang) {
	...
	return {
		...item,
		doctype: 'translation',
		url: `/quran/${encodeURIComponent(item.commentary_alias)}/quran:${range}`,
		translation_ref: range,
		translation_alias: Utils.trimToEmpty(item.commentary_alias),
		translation_book_ar: Utils.trimToEmpty(item.commentary_shortName || item.commentary_shortName_en || item.commentary_name || item.commentary_name_en || item.commentary_alias),
...
function highlightedCommentaryFootnoteFragment(item, lang) {
	...
	if (!highlight)
		return '';
	var fields = lang === 'ar'
		? ['footnotes', ...TRANSLATED_COMMENTARY_FOOTNOTE_HIGHLIGHT_FIELDS]
		: ['footnotes_en', ...TRANSLATED_COMMENTARY_FOOTNOTE_HIGHLIGHT_FIELDS];
	for (var i = 0; i < fields.length; i++) {
		var fragments = highlight[fields[i]];
		if (!Array.isArray(fragments))
			continue;
...
function highlightedCommentaryTextFragment(item, lang) {
	...
	if (!highlight)
		return '';
	var fields = lang === 'ar'
		? ['text', ...TRANSLATED_COMMENTARY_TEXT_HIGHLIGHT_FIELDS]
		: ['text_en', ...TRANSLATED_COMMENTARY_TEXT_HIGHLIGHT_FIELDS];
	for (var i = 0; i < fields.length; i++) {
		var fragments = highlight[fields[i]];
		if (!Array.isArray(fragments))
			continue;
...
function highlightedCommentaryFragment(item, lang) {
	...
	if (!highlight)
		return '';
	var fields = lang === 'ar'
		? ['text', 'footnotes', ...TRANSLATED_COMMENTARY_HIGHLIGHT_FIELDS]
		: ['text_en', 'footnotes_en', ...TRANSLATED_COMMENTARY_HIGHLIGHT_FIELDS];
	for (var i = 0; i < fields.length; i++) {
		var fragments = highlight[fields[i]];
		if (!Array.isArray(fragments))
			continue;
...
function commentaryUrl(item) {
	...
	if (!Number.isInteger(surah) || !Number.isInteger(ayahFrom) || !alias)
		return '';
	var range = Number.isInteger(ayahTo) && ayahTo > ayahFrom ? `${ayahFrom}-${ayahTo}` : `${ayahFrom}`;
	return `/quran/tafsir/${encodeURIComponent(commentarySlug(alias))}/quran:${surah}:${range}`;
}
...
function expandBookFilters(filters) {
	...
	for (var i = 0; i < filters.length; i++) {
		...
		if (filter === 'hadith' || filter === 'sharh' || filter === 'sirah' || filter === 'translations' || isSharhBookAlias(filter)) continue;
		expanded.push(...BookGroups.expand([filter], 'hadith'));
	}
...
Search.invalidateQuranAyahSearchCache = invalidateQuranAyahSearchCache;
Search.rebuildInMemoryCaches = rebuildInMemoryCaches;
Search.compareCanonicalQuranResults = compareCanonicalQuranResults;
