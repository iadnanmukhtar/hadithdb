...
function isSirahBookAlias(alias) {
	return (global.books || []).some(book => book.type === 'sirah' && book.alias === alias);
}
...
function buildSearchFilters(selectedFilters, options = {}) {
	if (options.generalSearch) {
		...
		const tocAliases = (global.books || []).filter(book => {
			...
			if (book.type === 'sirah') return selected.includes('sirah') && (!sirahAliases.length || sirahAliases.includes(book.alias));
