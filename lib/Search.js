...
class Search {
	...
	static async a_getRandom() {
		try {
			...
			var hadith = new Hadith(global.hadiths[i]);
	...
	static async a_lookupByRef(bookAlias, num) {
		try {
			...
			if (book) {
				var reNum = new RegExp('^' + num + '([^0-9]|$)', 'i');
...
function search(qs, lang) {
	...
	results = results.concat(searchQ(q, lang, 'exact'));
	...
	q = new RegExp('(' + q + ')', 'ig');
	if (prevq != q) {
		results = results.concat(searchQ(q, lang, 'prox').filter(function (value) {
			var add = true;
	...
	q = new RegExp('(' + q + ')', 'ig');
	if (prevq != q) {
		results = results.concat(searchQ(q, lang, 'any').filter(function (value) {
			var add = true;
			for (var i = 0; i < results.length; i++) {
				if (results[i].bookId == value.bookId && results[i].num == value.num) {
					add = false;
...
function searchQ(q, lang, matchType) {
	var results = [];
	for (var i = 0; i < global.hadiths.length; i++) {
