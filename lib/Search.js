...
const BOOK_FILTER_GROUPS = Object.freeze({
	...
	ninebooks: Object.freeze({
		label: 'Nine Books',
		aliases: Object.freeze(['bukhari', 'muslim', 'abudawud', 'tirmidhi', 'nasai', 'ibnmajah', 'malik', 'ahmad', 'darimi'])
	}),
	sihah: Object.freeze({
		label: 'Sihah',
		aliases: Object.freeze(['bukhari', 'muslim', 'malik', 'ibnhibban', 'ibnkhuzaymah', 'hakim'])
	}),
	sunan: Object.freeze({
		label: 'Sunan',
		aliases: Object.freeze(['abudawud', 'tirmidhi', 'nasai', 'ibnmajah', 'darimi', 'daraqutni', 'nasai-kubra', 'bayhaqi'])
	}),
	masanid: Object.freeze({
		label: 'Masanid',
		aliases: Object.freeze(['ahmad', 'bazzar'])
	}),
	musannaf: Object.freeze({
		label: 'Musannafat',
		aliases: Object.freeze(['malik', 'abdalrazzaq', 'ibnabishaybah'])
	}),
	maajim: Object.freeze({
		label: 'Maajim',
		aliases: Object.freeze(['tabarani-saghir', 'tabarani-awsat', 'tabarani'])
	})
});
...
function quranFirstRestIndexNames(selectedFilters, options = {}) {
	...
	if (isQuranScopedSearch(selectedFilters))
		return selectedFilters.indexOf('commentaries') >= 0 ? 'commentaries' : '';
...
function formatAutocompleteSuggestion(doc, lang, queryText) {
	...
	var type = doc.doctype === 'toc' ? (isQuranSurahDoc(doc) ? 'Surah' : 'Heading') : (doc.doctype === 'sirah' ? 'Sirah' : (doc.remark == 2 ? 'Ayah' : 'Hadith'));
...
function buildSearchFilters(selectedFilters, options = {}) {
	if (options.generalSearch) {
		...
		if (selected.includes('quran'))
			branches.push({ bool: { filter: [{ term: { doctype: 'hadith' } }, { term: { book_alias: 'quran' } }] } });
		...
		const tocAliases = (global.books || []).filter(book => {
			...
			if (['tafsir', 'trans'].includes(book.type)) return selected.includes('commentaries') && (!tafsirAliases.length || tafsirAliases.includes(book.alias));
