...
function commentaryUrl(item) {
	...
	return `/quran:${surah}:${range}#open-tafsir=${encodeURIComponent(item?.commentary_alias || '')}${commentaryLanguageHashPart(item)}`;
}
...
function commentarySectionUrl(item) {
	...
	return path ? `/${path.replace(/^\/+/, '')}#tafsir=${encodeURIComponent(item?.commentary_alias || '')}${commentaryLanguageHashPart(item)}` : '';
}

function commentaryLanguageHashPart(item) {
	return item?.lang === 'ar' || item?.lang === 'en' ? `&lang=${encodeURIComponent(item.lang)}` : '';
}
