...
class Search {
	...
	static async a_lookupQuranByRange(num, ayah1, ayah2) {
		var results = [{
			book: global.books[0]
		}];
		var toks = num.match(/^([a-z\-]+|[0-9]+)$/);
		if (!toks)
			throw createError(404, `Reference 'passage:${num}:${ayah1}-${ayah2}' does not exist`);
		var surah = toks[1];
		if (toks[1].match(/^[a-z]/)) {
			surah = global.surahs.find(function (value) {
				return (value.alias == toks[1]);
...
async function a_search(qs, b, lang, matchType) {
	var bookFilter = '';
	if (b && b.length > 0)
	...
	if (Arabic.isArabic(qs)) {
		...
		for (var n = 0; n < _hadiths.data.hits.hits.length; n++) {
			...
			_hadith.search_h1_title = Arabic.normalize(_hadith.h1_title);
			_hadith.search_h2_title = Arabic.normalize(_hadith.h2_title);
			_hadith.search_h3_title = Arabic.normalize(_hadith.h3_title);
			_hadith.search_h1_intro = Arabic.normalize(_hadith.h1_intro);
			_hadith.search_h2_intro = Arabic.normalize(_hadith.h2_intro);
			_hadith.search_h3_intro = Arabic.normalize(_hadith.h3_intro);
		...
		try {
			...
			if (lang == 'en') {
				...
				if (hadith.h1_title_en && hadith.h1_title_en.match(q))
					matches.push({ h1_title_en: hilite(q, hadith, 'h1_title_en', lang) });
				if (hadith.h2_title_en && hadith.h2_title_en.match(q))
					matches.push({ h2_title_en: hilite(q, hadith, 'h2_title_en', lang) });
				if (hadith.h3_title_en && hadith.h3_title_en.match(q))
					matches.push({ h3_title_en: hilite(q, hadith, 'h3_title_en', lang) });
				if (hadith.h1_intro_en && hadith.h1_intro_en.match(q))
					matches.push({ h1_intro_en: hilite(q, hadith, 'h1_intro_en', lang) });
				if (hadith.h2_intro_en && hadith.h2_intro_en.match(q))
					matches.push({ h2_intro_en: hilite(q, hadith, 'h2_intro_en', lang) });
				if (hadith.h3_intro_en && hadith.h3_intro_en.match(q))
					matches.push({ h3_intro_en: hilite(q, hadith, 'h3_intro_en', lang) });
			} else if (lang == 'ar') {
				...
				if (hadith.h1_title && hadith.h1_title.match(q))
					matches.push({ h1_title: hilite(q, hadith, 'h1_title', lang) });
				if (hadith.h2_title && hadith.h2_title.match(q))
					matches.push({ h2_title: hilite(q, hadith, 'h2_title', lang) });
				if (hadith.h3_title && hadith.h3_title.match(q))
					matches.push({ h3_title: hilite(q, hadith, 'h3_title', lang) });
				if (hadith.h1_intro && hadith.h1_intro.match(q))
					matches.push({ h1_intro: hilite(q, hadith, 'h1_intro', lang) });
				if (hadith.h2_intro && hadith.h2_intro.match(q))
					matches.push({ h2_intro: hilite(q, hadith, 'h2_intro', lang) });
				if (hadith.h3_intro && hadith.h3_intro.match(q))
					matches.push({ h3_intro: hilite(q, hadith, 'h3_intro', lang) });
			}
