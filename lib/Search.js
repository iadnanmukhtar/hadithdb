...
function tafsirAutocompleteSuggestions(qs, lang, selectedFilters) {
	...
	return Tafsir.visibleTafsirsSync()
		.filter(tafsir => Books.matchesQuery(tafsir, qs, lang))
		.slice(0, 3)
...
async function executeSearchQuery(qs, query, offset) {
	...
	var _results = size > 0
		? await Index.docsFromQuery('hadiths,toc,commentaries', query, offset, size, SEARCH_ORDER_BY, true)
		: [];
	var totalResults = cappedSearchTotal(Number.isFinite(_results.total) ? _results.total : _results.length);
...
async function executeQuranMemoryThenRestSearch(qs, lang, offset, selectedFilters) {
	...
	var docs = addUniqueSearchDocs([], quranMemoryDocs, seen);
	var quranTotal = docs.length;
	var restTotal = 0;
	...
	results.total = cappedSearchTotal(quranTotal + restTotal);
	return results;
}
...
function quranArabicHighlightTokenMatch(word, token) {
	...
	if (word.includes(token) || token.includes(word))
		return true;
	return token.length >= 3 && word.includes(token);
}

function quranSearchNormalizeArabic(value) {
	return Utils.trimToEmpty(Arabic.normalize(value, false))
		.replace(/يي/g, 'ي');
}
...
function logInMemoryQuranHits(qs, lang, docs) {
	...
	console.info(`[quran-search] in-memory hits query="${qs}" normalized="${normalized}" lang=${lang} count=${count}${refs.length ? ` refs=${refs.join(',')}` : ''}`);
}
...
function buildQuranOnlySearchQuery(queryText, lang, autocomplete) {
	queryText = (queryText || '').trim();
	var normalizedQueryText = lang === 'ar' ? quranSearchNormalizeArabic(queryText) : queryText;
	...
	var ayahClause = {
		bool: {
			...
			must: {
				bool: {
					should: buildQuranAyahMatchClauses(normalizedQueryText, fields, tokenCount, lang),
					minimum_should_match: 1
				}
			}
		}
...
function buildQuranAyahMatchClauses(queryText, fields, tokenCount, lang) {
	var should = buildAllTokenMatchClauses(queryText, fields, tokenCount);
	if (lang === 'ar') {
		var tokens = Arabic.tokenize(queryText).filter(Boolean);
		if (tokens.length > 0) {
			should.push({
				bool: {
					must: tokens.map(token => ({
						bool: {
							should: [
								buildQuranAyahWildcardClause('body_search_ar.raw', token, 2),
								buildQuranAyahWildcardClause('body_search_ar', token, 1)
							],
							minimum_should_match: 1
						}
					}))
...
function buildQuranAyahWildcardClause(field, token, boost) {
	return {
		wildcard: {
			[field]: {
				value: `*${escapeElasticWildcard(token)}*`,
				boost: boost
			}
		}
	};
...
function escapeElasticWildcard(value) {
	return Utils.trimToEmpty(value).replace(/([*?\\])/g, '\\$1');
}
...
function formatTafsirAutocompleteSuggestion(tafsir, lang) {
	var titleEn = Tafsir.displayShortName(tafsir, 'en');
	var titleAr = Tafsir.displayShortName(tafsir, 'ar');
	var title = lang === 'ar' ? titleAr : titleEn;
