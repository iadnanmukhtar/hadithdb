...
function formatCommentarySearchResult(item, lang) {
	...
	item.commentary_ref = commentaryRangeLabel(item);
	item.search_lang = lang;
	var fragment = firstHighlightedFragment(item, lang);
	if (!fragment)
		fragment = lang === 'ar' ? firstText(item.text, item.footnotes, item.text_en, item.footnotes_en) : firstText(item.text_en, item.footnotes_en, item.text, item.footnotes);
	item.commentary_fragment_html = highlightFragmentToHtml(Utils.truncate(cleanCommentaryFragment(fragment), 700, true));
	return item;
}
...
function commentaryRangeLabel(item) {
	var ayahFrom = Number(item?.ayahFrom);
	var ayahTo = Number(item?.ayahTo);
	return `Quran ${item?.surah}:${ayahFrom}${Number.isInteger(ayahTo) && ayahTo > ayahFrom ? `-${ayahTo}` : ''}`;
}
