...
class Search {
	...
	static generalContentFilters(filters) {
		...
		if (selected.some(filter => ['hadith', 'sharh', 'sirah', 'quran', 'commentaries', 'toc'].includes(filter) || isSharhBookAlias(filter))) return selected;
...
function allowsQuranSuggestions(selectedFilters) {
	...
	if (!selectedFilters || selectedFilters.length < 1)
		return true;
	var bookFilters = expandBookFilters(selectedFilters.filter(filter => filter !== 'hadith' && filter !== 'toc' && filter !== 'commentaries' && filter !== 'sharh' && !isSharhBookAlias(filter)));
...
function bookReferenceAutocompleteSuggestions(qs, lang, selectedFilters, options = {}) {
	if (options.generalSearch) {
		...
		selectedFilters = selectedFilters.filter(filter => !['quran', 'commentaries'].includes(filter));
...
function translationAutocompleteSuggestions(qs, lang, selectedFilters, options = {}) {
	if (options.generalSearch && !selectedFilters.includes('quran') && !searchOptionTafsirAliases(options).length) return [];
	if (options.excludeQuranAndTafsir)
		return [];
...
function allowsTranslationAutocompleteSuggestion(selectedFilters) {
	...
	if (!selectedFilters || selectedFilters.length < 1)
		return true;
	return selectedFilters.indexOf('quran') >= 0;
}
...
function isSharhSearch(filters) {
	return !(filters || []).some(filter => ['hadith', 'sirah', 'quran', 'commentaries'].includes(filter)) && ((filters || []).includes('sharh') || selectedSharhBooks(filters).length > 0);
}
...
function buildSearchFilters(selectedFilters, options = {}) {
	if (options.generalSearch) {
		...
		const branches = [];
		const hadithFilters = selected.filter(filter => !['quran', 'commentaries', 'sirah'].includes(filter) && !isSirahBookAlias(filter));
		...
		const contentTypes = ['hadith', 'sharh', 'sirah', 'quran', 'commentaries', 'toc'];
		...
		const tocAliases = (global.books || []).filter(book => {
			...
			if (book.type === 'trans') return (selected.includes('quran') || tafsirAliases.includes(book.alias)) && (!tafsirAliases.length || tafsirAliases.includes(book.alias));
	...
	if (!selectedFilters || selectedFilters.length < 1)
		return filters;
	if (selectedFilters.every(filter => ['quran', 'commentaries'].includes(filter))) {
		var quranBranches = [];
	...
	var bookAliases = expandBookFilters(selectedFilters.filter(filter => filter !== 'hadith' && filter !== 'toc' && filter !== 'commentaries' && filter !== 'sharh' && !isSharhBookAlias(filter)));
...
function isQuranScopedSearch(selectedFilters) {
	if (!Array.isArray(selectedFilters) || selectedFilters.length < 1)
		return false;
	var nonQuranFilters = selectedFilters.filter(filter => filter !== 'quran' && filter !== 'commentaries');
	return nonQuranFilters.length < 1 && selectedFilters.indexOf('quran') >= 0;
}
...
function expandBookFilters(filters) {
	...
	for (var i = 0; i < filters.length; i++) {
		...
		if (filter === 'hadith' || filter === 'sharh' || filter === 'sirah' || isSharhBookAlias(filter)) continue;
