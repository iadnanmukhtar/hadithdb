...
async function quranThenRestAutocompleteSuggestions(qs, lang, selectedFilters, generated, limit) {
	...
	var quranDocs = await Index.docsFromQuery('hadiths', quranQuery, 0, remaining, SEARCH_ORDER_BY, true);
...
async function quranOnlyAutocompleteSuggestions(qs, lang, generated, limit) {
	...
	if (remaining < 1)
		return generated.slice(0, limit);
	var quranDocs = await Index.docsFromQuery('hadiths', buildQuranOnlyAutocompleteQuery(qs, lang), 0, remaining, SEARCH_ORDER_BY, true);
	quranDocs.sort(compareSearchResults);
...
async function executeQuranOnlySearch(qs, lang, offset) {
	var _results = await Index.docsFromQuery('hadiths', buildQuranOnlySearchQuery(qs, lang), offset, undefined, SEARCH_ORDER_BY, true);
	var totalResults = Number.isFinite(_results.total) ? _results.total : _results.length;
	_results.sort(compareSearchResults);
	var results = await formatSearchDocs(qs, _results);
	results.total = totalResults;
...
function buildQuranOnlyAutocompleteQuery(queryText, lang) {
	queryText = (queryText || '').trim();
	return buildQuranOnlySearchQuery(queryText, lang);
}

function buildQuranOnlySearchQuery(queryText, lang) {
	queryText = (queryText || '').trim();
...
function firstHighlightedFragment(doc, lang) {
	...
	var fields = lang === 'ar'
			? (isQuranAyah
				? ['body_search_ar', 'body', 'footnote', 'text', 'footnotes', 'commentary_name', 'commentary_shortName', 'commentary_author', 'book_name_search_ar', 'book_shortName_search_ar', 'title_search_ar', 'title', 'chain', 'intro_search_ar', 'intro', 'h1_title', 'h2_title', 'h3_title', 'h1_intro', 'h2_intro', 'h3_intro']
