...
function installToastReporting() {
	'use strict';
	if (!window.toastr || window.toastr.__hadithdbErrorReporting)
		return false;
	...
	window.toastr.options.timeOut = 5200;
	window.toastr.options.extendedTimeOut = 1800;
	window.toastr.options.showMethod = 'show';
	var originalErrorToast = window.toastr.error.bind(window.toastr);
	var originalSuccessToast = window.toastr.success.bind(window.toastr);
	window.toastr.success = function reportedToastSuccess(message, title, optionsOverride) {
		var safeTitle = title === undefined || title === null || title === '' ? 'Success' : String(title);
		var safeMessage = message === undefined || message === null ? '' : String(message);
		console.log('[Toast success]', safeTitle + ':', safeMessage);
		return originalSuccessToast(safeMessage, safeTitle, optionsOverride);
	};
	window.toastr.error = function reportedToastError(message, title, optionsOverride) {
		var safeTitle = title === undefined || title === null || title === '' ? 'Something needs attention' : String(title);
		var safeMessage = message === undefined || message === null || message === '' ? 'An unexpected error occurred.' : String(message);
		console.error('[Toast error]', safeTitle + ':', safeMessage);
		try {
			fetch('/api/client-errors', {
				...
				console.warn('Unable to report toast error to the server:', error && error.message ? error.message : error);
			});
		} catch (error) {
			console.warn('Unable to prepare toast error report:', error && error.message ? error.message : error);
		}
		return originalErrorToast(safeMessage, safeTitle, optionsOverride);
	};
	Object.defineProperty(window.toastr, '__hadithdbErrorReporting', { value: true });
	return true;
}
...
$(function () {
	...
	initReaderLanguageColumnToggles(document);
...
var fixedHeaderResizeObserver = null;

function initFixedHeaderOffsetObserver() {
	var navbar = document.querySelector('.site-navbar.fixed-top');
	if (!navbar || !('ResizeObserver' in window))
		return;
	if (fixedHeaderResizeObserver)
		fixedHeaderResizeObserver.disconnect();
	fixedHeaderResizeObserver = new ResizeObserver(function () {
		updateFixedHeaderOffset();
	});
	fixedHeaderResizeObserver.observe(navbar);
	var navbarMain = navbar.querySelector('.site-navbar-main');
	if (navbarMain)
		fixedHeaderResizeObserver.observe(navbarMain);
}
...
$(document).on('click', '[data-reflection-disclosure-trigger]', function () {
	var disclosureId = this.getAttribute('data-reflection-disclosure-trigger') || '';
	var disclosure = disclosureId ? document.getElementById(disclosureId) : null;
	if (disclosure && disclosure.tagName === 'DETAILS')
...
function renderHadithAdminGear() {
	...
	if (desktopList && !desktopList.querySelector('.edit-gear')) {
		const authItem = desktopList.querySelector('#nav-auth-desktop');
...
function initCommandSearch() {
	var dialog = document.querySelector('[data-command-search-dialog]');
	if (!dialog || dialog.dataset.commandSearchBound === '1')
		return;
	dialog.dataset.commandSearchBound = '1';
	var form = dialog.querySelector('[data-command-search-form]');
	var input = dialog.querySelector('.command-search-input');
	...
	function refreshAutocomplete() {
		var $input = $(input);
		if (!$input.data('autocompleteBound') || !$.fn.autocomplete)
			return;
	...
	function applyContextualQuranFilters() {
		var context = dialog.dataset.quranSearchContext || 'quran';
		var alias = (dialog.dataset.quranCommentaryAlias || '').toString().trim();
		if (!alias && context === 'study')
			alias = quranSelectedTranslationAliasFromLocation() || storedQuranSelectedTranslationAlias();
		if (!alias)
			return;
		var checkbox = Array.from(dialog.querySelectorAll('[data-command-search-panel="quran"] input[name="tafsir"]')).find(function (candidate) {
			return candidate.value === alias;
		});
		if (!checkbox)
			return;
	...
	function renderSelectedFilters() {
		...
		var selected = dialog.querySelector('[data-command-search-selected]');
		var pills = dialog.querySelector('[data-command-search-pills]');
		if (!panel || !selected || !pills)
			return;
		...
		checked.forEach(function (checkbox) {
			...
			label.textContent = checkbox.dataset.commandFilterLabel || checkbox.value;
			var remove = document.createElement('button');
			remove.type = 'button';
			remove.className = 'btn-close';
			remove.setAttribute('aria-label', `Remove ${label.textContent} filter`);
	...
	document.querySelectorAll('[data-command-search-open]').forEach(function (trigger) {
		trigger.addEventListener('click', function () { openDialog(trigger); });
	});
	dialog.querySelector('[data-command-search-close]').addEventListener('click', function () {
		dialog.close();
	});
	...
	dialog.querySelectorAll('[data-command-search-mode]').forEach(function (button) {
		button.addEventListener('click', function () {
			setMode(button.dataset.commandSearchMode);
			input.focus();
		});
		button.addEventListener('keydown', function (event) {
			if (event.key !== 'ArrowLeft' && event.key !== 'ArrowRight')
				return;
			event.preventDefault();
			var nextMode = mode === 'hadith' ? 'quran' : 'hadith';
			setMode(nextMode);
			dialog.querySelector(`[data-command-search-mode="${nextMode}"]`).focus();
		});
	});
...
function quranTafsirLocalStorage() {
	try {
...
function normalizeQuranTafsirSlug(value) {
	try {
		value = decodeURIComponent((value || '').toString());
	} catch (_err) {
		return '';
	}
	value = tafsirBrowseSlug(value);
	return /^[A-Za-z0-9_-]+$/.test(value) ? value : '';
}

function getLastVisitedQuranTafsirSlug() {
	var storage = quranTafsirLocalStorage();
	return normalizeQuranTafsirSlug(storage && storage.getItem(quranTafsirStorageKeys.lastVisited)) || defaultQuranTafsirAlias;
}

function storeLastVisitedQuranTafsir(alias) {
	var storage = quranTafsirLocalStorage();
	var slug = normalizeQuranTafsirSlug(alias);
	if (storage && slug)
		storage.setItem(quranTafsirStorageKeys.lastVisited, slug);
}

function rememberCurrentQuranTafsir() {
	var match = window.location.pathname.match(/^\/quran\/tafsir\/([^/]+)(?:\/|$)/);
	if (match)
		storeLastVisitedQuranTafsir(match[1]);
}
...
function storeQuranTafsirAlias(alias) {
	...
	storeLastVisitedQuranTafsir(alias);
}
...
function initQuranTafsirTabs(root) {
	var scope = root || document;
		...
		var ayahs = (container.attr('data-ayahs') || '').split(',').filter(Boolean).map(Number).filter(function (ayah) {
			...
			var selectedTafsirCarouselLanguage = normalizeQuranTafsirLanguage(container.attr('data-selected-tafsir-carousel-language'));
			var activeLanguage = selectedTafsirCarouselLanguage || selectedTafsirLanguage || getStoredQuranTafsirLanguage() || 'en';
		...
		var scrollTafsirTabIntoView = function (tab) {
			...
			var renderTafsirSizeIndicator = function (book) {
				...
				if (!label)
					return null;
				return $('<span>').addClass(`tafsir-size-indicator tafsir-size-indicator-${size}`).attr({
					'aria-label': label,
					role: 'img'
				});
			};
			var catalogBookIsBilingual = function (book) {
				var languages = (book && (book.languages || book.lang) || '').toString().toLowerCase().split('-');
				var format = (book && book.format || '').toString().toLowerCase().split(',').map(function (value) {
					return value.trim();
				});
				return languages.includes('en') && languages.includes('ar')
					|| format.some(function (value) { return value.startsWith('en:'); }) && format.some(function (value) { return value.startsWith('ar:'); });
			};
			...
			var setTafsirBookHoverHighlight = function (target, highlighted) {
				...
				if (!panel.length && rendersTafsirPassagePage)
					panel = container.find('.quran-tafsir-panel.active').first();
			...
			var setTafsirBookDisabled = function (alias, disabled) {
				...
				container.find(`.quran-tafsir-source[data-tafsir-src="${cssEscape(alias)}"]`).each(function () {
					$(this).attr('data-tafsir-disabled', disabled ? '1' : '0')
						.find('[data-tafsir-enable-toggle]').prop('checked', !disabled);
				});
			...
			var appendTafsirEnableToggle = function (target, book) {
				...
				}).prop('checked', book.disabled !== true).appendTo(toggleLabel);
				bindTafsirEnableToggle(toggleLabel, book);
			};
			var appendSourceHeader = function (panel, book) {
				var header = $('<header>').addClass('quran-tafsir-source row').attr({
					'data-tafsir-src': book.alias,
					'data-tafsir-lang': book.lang || '',
					'data-tafsir-disabled': book.disabled === true ? '1' : '0'
				});
				if (rendersTafsirPassagePage) {
					var isLazyLoadedPage = container.closest('[data-reader-infinite-page="1"]').length > 0;
					var isSelectedBook = book.alias === preferredTafsirAlias
						&& (!selectedTafsirLanguage || book.lang === selectedTafsirLanguage);
					if (isLazyLoadedPage || !isSelectedBook || container.children('.quran-tafsir-source').length)
						return;
					header.addClass('text-muted').insertBefore(container.children('.quran-tafsir-heading').first());
				} else {
			...
			var addCatalogTab = function (book) {
				...
				var tab = $(dedicatedHref ? '<a>' : '<button>').addClass('btn btn-outline-primary text-nowrap').attr({
					id: tabId,
		...
		var mergeLocalEnglishTranslationPayloads = function (payloads, englishPayloads) {
			...
			return payloads.map(function (payload) {
				...
				return Object.assign({}, payload, {
					...
					html: `<div class="row quran-tafsir-local-pair"><section class="col-md-6 col-sm-12" lang="en" data-reader-language-column="english">${english.html}</section><section class="col-md-6 col-sm-12" lang="ar" dir="rtl" data-reader-language-column="arabic">${payload.html}</section></div>`,
		...
		var loadPanel = async function (panel) {
			...
			if (!src || panel.data('tafsirLoading'))
				return;
			if (panel.data('tafsirLoaded')) {
				initReaderLanguageColumnToggles(container[0]);
				return;
			}
			...
			try {
				...
				entries.forEach(function (entry) {
					...
					if (entry.payload.bilingual) {
						generatedBody.find('.quran-tafsir-local-pair > [lang]').each(function () {
							var language = (this.getAttribute('lang') || '').toLowerCase();
							this.setAttribute('data-reader-language-column', language === 'ar' ? 'arabic' : 'english');
						});
					} else {
						generatedBody.attr('data-reader-language-column', panelLanguage === 'ar' ? 'arabic' : 'english');
					}
					if (hasTafsirTranslationItem) {
						generatedBody.attr({
		...
		var showLanguage = function (language, preferredAlias) {
			...
			updateTafsirCarouselLanguageSwitch(container.find('[data-tafsir-carousel-language-switch]').first(), language);
			...
			container.find('[data-tafsir-carousel-language-switch]').on('click', function () {
				var language = $(this).attr('data-tafsir-carousel-language') === 'ar' ? 'en' : 'ar';
				storeQuranTafsirLanguage(language);
			...
			var visibleBooks = (Array.isArray(books) ? books : []).filter(function (book) {
				return !disabledAliases.has(book.alias) && !(book.lang === 'ar' && catalogBookIsBilingual(book));
			...
			var initialAlias = preferredTafsirAlias || (isDedicatedTafsirPassage ? '' : getStoredQuranTafsirAlias() || defaultQuranTafsirAlias);
			var selectedCatalogBook = initialAlias ? visibleBooks.find(function (book) {
				return book.alias === initialAlias;
			}) : null;
			var initialLanguage = selectedCatalogBook
				? normalizeQuranTafsirLanguage(selectedCatalogBook.lang)
				: selectedTafsirCarouselLanguage || selectedTafsirLanguage || getStoredQuranTafsirLanguage();
...
function quranSelectedTranslationAliasFromLocation() {
	try {
		...
		if (queryAlias)
			return queryAlias;
		var renderedAlias = validQuranTranslationAlias(document.body && document.body.dataset.quranSelectedTranslationAlias || '');
		if (renderedAlias)
			return renderedAlias;
		var pathMatch = window.location.pathname.match(/^\/quran\/([^/]+)\/(?:\d+\/\d+|quran:\d+:\d+)(?:\/|$)/);
...
function quranCurrentPassagePartsFromLocation() {
	...
	var scopedRefMatch = path.match(/^\/quran\/[^/]+\/quran:(\d+):(\d+(?:-\d+)?)(?:\/|$)/);
	var plainRefMatch = path.match(/^\/quran:(\d+):(\d+(?:-\d+)?)(?:\/|$)/);
	var refMatch = scopedRefMatch || plainRefMatch;
	if (refMatch)
		return { surah: refMatch[1], ref: `${refMatch[1]}:${refMatch[2]}` };
...
function quranTranslationPassageUrl(alias) {
	...
	if (!parts)
		return '';
	var path = parts.ref
		? (alias ? `/quran/${encodeURIComponent(alias)}/quran:${parts.ref}` : `/quran:${parts.ref}`)
		: (alias ? `/quran/${encodeURIComponent(alias)}/${parts.surah}/${parts.passage}` : `/quran/${parts.surah}/${parts.passage}`);
...
function quranAyahStudyUrl(ref) {
	var parts = (ref || '').toString().replace(/^quran:/, '').match(/^(\d+):(\d+)$/);
	if (!parts)
		return quranUrl('/quran');
	var alias = quranSelectedTranslationAliasFromLocation();
	if (!alias) {
		var selector = document.querySelector('[data-quran-passage-translation-select="1"]')
			|| document.querySelector('[data-quran-audio-translation-select="1"]');
		alias = selector && selector.options.length
			? validQuranTranslationAlias(selector.value || '')
			: storedQuranSelectedTranslationAlias();
	}
	var path = alias
		? `/quran/${encodeURIComponent(alias)}/quran:${parts[1]}:${parts[2]}`
		: `/quran:${parts[1]}:${parts[2]}`;
...
function applyQuranPassageDisplay(state) {
	...
	document.querySelectorAll('[data-quran-passage-display-toggle]').forEach(function (button) {
		...
		var otherActive = target === 'arabic' ? state.translation : state.arabic;
		var mustRemainVisible = active && !otherActive;
		var label = target === 'arabic' ? 'Quran Arabic' : 'Translation';
		button.classList.toggle('is-active', active);
		button.setAttribute('aria-pressed', active ? 'true' : 'false');
		button.disabled = mustRemainVisible;
		button.setAttribute('title', mustRemainVisible
...
function readerLanguageColumnStorageKey() {
	return 'hadithdb.readerLanguageColumns';
}

function storedReaderLanguageColumns() {
	var fallback = { english: true, arabic: true };
	try {
		var stored = JSON.parse(window.localStorage.getItem(readerLanguageColumnStorageKey()) || '{}');
		var state = {
			english: stored.english !== false,
			arabic: stored.arabic !== false
		};
		return state.english || state.arabic ? state : fallback;
	} catch (err) {
		return fallback;
	}
}

function storeReaderLanguageColumns(state) {
	try {
		window.localStorage.setItem(readerLanguageColumnStorageKey(), JSON.stringify({
			english: state.english !== false,
			arabic: state.arabic !== false
		}));
	} catch (err) {}
}

function readerLanguageColumnRoots(scope) {
	var roots = new Set();
	var target = scope || document;
	var query = function (selector) {
		return Array.from(target.querySelectorAll(selector)).concat(target.matches && target.matches(selector) ? [target] : []);
	};
	query('[data-reader-infinite-page="1"]').forEach(function (root) {
		var mode = root.getAttribute('data-reader-mode') || '';
		if (mode === 'tafsir' || mode.indexOf('hadith-') === 0) {
			root.setAttribute('data-reader-language-root', mode === 'tafsir' ? 'tafsir' : 'hadith');
			roots.add(root);
		}
	});
	...
	query('[data-reader-language-item="hadith"]').forEach(function (item) {
		...
		if (!root)
			return;
		root.setAttribute('data-reader-language-root', 'hadith');
		roots.add(root);
	});
	query('.tafsir-only-page:not(.translation-only-page)').forEach(function (root) {
		root.setAttribute('data-reader-language-root', 'tafsir');
		roots.add(root);
	});
	query('.quran-tafsirs').forEach(function (tafsir) {
		var pageRoot = tafsir.closest('[data-reader-infinite-page="1"]') || tafsir.closest('.tafsir-only-page:not(.translation-only-page)');
		if (pageRoot) {
			pageRoot.setAttribute('data-reader-language-root', 'tafsir');
			roots.add(pageRoot);
			return;
		}
		tafsir.setAttribute('data-reader-language-root', 'tafsir');
		roots.add(tafsir);
	});
	return Array.from(roots);
}

function readerLanguageColumnToolbar() {
	var toolbar = document.createElement('aside');
	toolbar.className = 'reader-language-toolbar';
	toolbar.setAttribute('aria-label', 'Text columns');
	toolbar.setAttribute('role', 'group');
	[
		{ target: 'english', label: 'English', lang: 'en', dir: 'ltr' },
		{ target: 'arabic', label: 'Arabic', lang: 'ar', dir: 'rtl' }
	].forEach(function (choice) {
		var button = document.createElement('button');
		button.type = 'button';
		button.className = 'btn btn-outline-secondary quran-ayah-select-btn quran-passage-display-btn quran-passage-view-btn reader-language-toggle is-active';
		button.setAttribute('data-reader-language-toggle', choice.target);
		button.setAttribute('aria-pressed', 'true');
		button.setAttribute('lang', choice.lang);
		button.setAttribute('dir', choice.dir);
		var icon = document.createElement('span');
		icon.className = 'bi bi-eye';
		icon.setAttribute('aria-hidden', 'true');
		button.appendChild(icon);
		button.setAttribute('title', `Hide ${choice.label}`);
		button.setAttribute('aria-label', button.getAttribute('title'));
...
function ensureReaderLanguageColumnToolbar(root) {
	if (!root)
		return;
	...
	if (kind === 'tafsir') {
		...
		var tafsirEntry = activePanel
			? activePanel.querySelector('.quran-tafsir-entry')
			: root.querySelector('.quran-tafsir-entry');
...
function applyReaderLanguageColumns(state) {
	state = {
		english: state && state.english !== false,
		arabic: state && state.arabic !== false
	};
	if (!state.english && !state.arabic)
		state.english = true;
	readerLanguageColumnRoots(document).forEach(function (root) {
		root.classList.toggle('reader-hide-english', !state.english);
		root.classList.toggle('reader-hide-arabic', !state.arabic);
	});
	document.querySelectorAll('[data-reader-language-toggle]').forEach(function (button) {
		var target = button.getAttribute('data-reader-language-toggle');
		var active = target === 'arabic' ? state.arabic : state.english;
		var otherActive = target === 'arabic' ? state.english : state.arabic;
		var mustRemainVisible = active && !otherActive;
		var label = target === 'arabic' ? 'Arabic' : 'English';
		button.classList.toggle('is-active', active);
		button.setAttribute('aria-pressed', active ? 'true' : 'false');
		button.disabled = mustRemainVisible;
		button.setAttribute('title', mustRemainVisible ? `${label} must remain visible` : `${active ? 'Hide' : 'Show'} ${label}`);
		button.setAttribute('aria-label', button.getAttribute('title'));
		var icon = button.querySelector('.bi');
		if (icon) {
			icon.classList.toggle('bi-eye', active);
...
function initReaderLanguageColumnToggles(root) {
	readerLanguageColumnRoots(root).forEach(ensureReaderLanguageColumnToolbar);
	applyReaderLanguageColumns(storedReaderLanguageColumns());
	document.querySelectorAll('[data-reader-language-toggle]').forEach(function (button) {
		if (button.dataset.readerLanguageToggleBound === 'true')
			return;
		button.dataset.readerLanguageToggleBound = 'true';
		button.addEventListener('click', function () {
			var state = storedReaderLanguageColumns();
			var target = button.getAttribute('data-reader-language-toggle');
			if (target === 'arabic')
			...
			if (!state.english && !state.arabic)
				return;
			state = applyReaderLanguageColumns(state);
			storeReaderLanguageColumns(state);
		});
	});
}
	...
	Promise.all([quranTranslationBooks(), getQuranTafsirSettings().catch(function () { return {}; })]).then(function (results) {
		...
		selectors.forEach(function (selector) {
			...
			selector.addEventListener('change', function () {
				...
				if (!targetUrl) {
					if (window.toastr)
						toastr.error('Unable to build the selected translation URL.', 'Settings');
					return;
				}
...
function syncQuranAudioTranslationSelection(alias) {
	...
	applyQuranHeadingTranslationScope(alias);
}
...
function initQuranAudioTranslationSelects(controls) {
	...
	Promise.all([quranTranslationBooks(), getQuranTafsirSettings().catch(function () { return {}; })]).then(function (results) {
		...
		selectors.forEach(function (selector) {
			...
			syncQuranAudioTranslationMenu(selector);
			bindQuranDynamicTranslationMenu(selector);
			applyQuranHeadingTranslationScope(selector.value);
		});
	}).catch(function () {});
	selectors.forEach(function (selector) {
		selector.addEventListener('change', function () {
...
function getQuranTafsirSettings() {
	...
	function compareTafsirAlphabeticalEntries(a, b) {
		var aBook = a && a.book || {};
		var bBook = b && b.book || {};
		var aIsDefault = aBook.alias === defaultQuranTafsirAlias;
		var bIsDefault = bBook.alias === defaultQuranTafsirAlias;
...
function initPageHelpTips(root) {
	...
	study: [
		...
		},
		{
			...
			selector: '.quran-ayah-modal.show .tafsir-carousel-language-switch, .quran-tafsirs .tafsir-carousel-language-switch',
			requiresHero: true,
	...
	tafsir: [
		...
		},
		{
			...
			copy: 'Use the sticky horizontal tafsir list to open another commentary at this Quran passage. Each choice opens its dedicated reading page. **Swipe** the list on a phone when more tafsirs are available off screen.',
			selector: '.tafsir-only-page .quran-commentary-passage-carousel'
		},
		{
			title: 'Compare translations',
...
function initQuranMushafAyahMarkerActions(root) {
	...
	var markerTafsirBook = function () {
		...
		markerTafsirBookPromise = getQuranTafsirBooks().then(function (books) {
			...
			if (storedBook)
				return storedBook;
			var defaultBook = books.find(function (book) {
				return book.alias === defaultQuranTafsirAlias && book.lang === 'en';
			}) || books.find(function (book) {
				return book.alias === defaultQuranTafsirAlias;
			});
			if (defaultBook)
				return defaultBook;
	...
	var showMenu = function (marker, focusAction) {
		...
		var note = menu.querySelector('[data-quran-ayah-note]');
		note.dataset.ayahNoteRef = 'quran:' + (marker.getAttribute('data-quran-ref') || '').replace(/^quran:/, '');
		document.dispatchEvent(new CustomEvent('quranAyahNoteTarget', { detail: { button: note } }));
		var view = menu.querySelector('[data-quran-mushaf-ayah-view]');
		view.setAttribute('href', quranAyahStudyUrl(marker.getAttribute('data-quran-ref') || ''));
...
function copyQuranReaderModeHrefs(source, target) {
	if (!source || !target)
		return;
	['passage', 'ayat', 'tafsir', 'mushaf', 'memorize', 'review'].forEach(function (mode) {
	...
	var firstRef = quranReaderModeFirstRef(source);
	if (firstRef)
		target.setAttribute('data-quran-reader-first-ref', firstRef);
}

function quranReaderModeFirstRef(source) {
	if (!source)
		return '';
	var storedRef = source.getAttribute && (source.getAttribute('data-reader-quran-ref') || source.getAttribute('data-quran-reader-first-ref'));
	var commentary = source.querySelector && source.querySelector('[data-reader-quran-ref]');
	storedRef = storedRef || commentary && commentary.getAttribute('data-reader-quran-ref');
	if (/^\d+:\d+$/.test(storedRef || ''))
		return storedRef;
	var mushafPage = source.matches && source.matches('[data-quran-mushaf-page]')
		? source
		: source.querySelector && source.querySelector('[data-quran-mushaf-page]');
	var refElement = mushafPage && (mushafPage.querySelector('.quran-mushaf-search-ayah[data-quran-ref]') || mushafPage.querySelector('.quran-mushaf-sheet:not(.quran-review-page-continuity) [data-quran-ref]'));
	if (!refElement && source.querySelector)
		refElement = source.querySelector('.quran-passage-section .ayah.ayah-selected[data-quran-ref]');
	if (!refElement && source.querySelector)
		refElement = source.querySelector('.quran-passage-section .ayah[data-quran-ref]');
	var ref = refElement && refElement.getAttribute('data-quran-ref') || '';
	return /^\d+:\d+$/.test(ref) ? ref : '';
}

function quranReaderTafsirHref(firstRef) {
	if (!/^\d+:\d+$/.test(firstRef || ''))
		return '';
	var parts = firstRef.split(':');
	var slug = getLastVisitedQuranTafsirSlug();
	return quranUrl(`/quran/tafsir/${encodeURIComponent(slug)}/quran:${encodeURIComponent(parts[0])}:${encodeURIComponent(parts[1])}`);
}
...
function updateQuranReaderModeHrefs(marker) {
	...
	var firstRef = quranReaderModeFirstRef(marker);
	if (firstRef) {
		var mushafHref = marker.getAttribute('data-quran-reader-mushaf-href');
		if (mushafHref) {
			var mushafUrl = new URL(mushafHref, window.location.origin);
			mushafUrl.searchParams.set('ayah', firstRef);
			document.querySelectorAll('[data-quran-reader-mode-link="mushaf"]').forEach(function (link) {
				link.setAttribute('href', mushafUrl.pathname + mushafUrl.search);
			});
		}
		if (marker.matches('[data-quran-mushaf-page]') || marker.getAttribute('data-reader-mode') === 'tafsir') {
			document.querySelectorAll('[data-quran-reader-mode-link="passage"]').forEach(function (link) {
				link.setAttribute('href', quranUrl(`/quran:${firstRef}`));
			});
		}
	}
	var tafsirHref = quranReaderTafsirHref(quranReaderModeFirstRef(marker));
	if (tafsirHref) {
		document.querySelectorAll('[data-quran-reader-mode-link="tafsir"]:not(.active)').forEach(function (link) {
			link.setAttribute('href', tafsirHref);
		});
	}
}
...
function applyQuranHeadingTranslationScope(alias, force) {
	var toc = quranHeadingTocState.toc || document.querySelector('[data-quran-heading-toc]');
	if (!toc || toc.getAttribute('data-quran-heading-tafsir-base'))
		return;
	alias = validQuranTranslationAlias(alias || '');
	var currentAlias = validQuranTranslationAlias(toc.getAttribute('data-quran-heading-translation-alias') || '');
	if (!force && alias === currentAlias)
		return;
	if (alias)
		toc.setAttribute('data-quran-heading-translation-alias', alias);
	else
		toc.removeAttribute('data-quran-heading-translation-alias');
	var bookLink = toc.querySelector('[data-quran-heading-toc-book]');
	if (bookLink)
		bookLink.href = quranUrl(alias ? `/quran/${encodeURIComponent(alias)}` : '/quran');
	if (quranHeadingTocState.toc !== toc)
		return;
	var surah = quranHeadingTocState.surah;
	var activeKey = quranHeadingTocState.activeKey;
	quranHeadingTocState.activeKey = '';
	if (surah && renderQuranHeadingToc(surah) && activeKey)
		setActiveQuranHeading(surah, activeKey);
}
...
function renderQuranHeadingToc(surah) {
	...
	toc.querySelectorAll('.quran-commentary-introduction-rail-link').forEach(function (link) {
		link.hidden = Number(outline.surah) !== 1;
	});
...
function initQuranHeadingToc(root) {
	...
	applyQuranHeadingTranslationScope(quranSelectedTranslationAliasFromLocation() || (function () {
		var selector = document.querySelector('[data-quran-passage-translation-select="1"]')
			|| document.querySelector('[data-quran-audio-translation-select="1"]');
		return selector && selector.options.length ? selector.value : storedQuranSelectedTranslationAlias();
	})(), true);
...
function quranMushafAyahSelection(value) {
	var match = String(value || '').match(/^(\d+):(\d+)(?:-(\d+))?$/);
	if (!match) return null;
	var surah = Number(match[1]);
	var start = Number(match[2]);
	var end = match[3] === undefined ? start : Number(match[3]);
	if (surah < 1 || surah > 114 || start < 1 || end < start || end > 286) return null;
	return { surah: surah, start: start, end: end };
}

function highlightQuranMushafSelection(root, ref) {
	var selection = quranMushafAyahSelection(ref === undefined ? new URLSearchParams(window.location.search).get('ayah') : ref);
	root.querySelectorAll('[data-quran-ref]').forEach(function (element) {
		if (element.closest('.quran-review-page-continuity')) return;
		var ref = element.getAttribute('data-quran-ref').split(':').map(Number);
		var selected = !!selection && ref[0] === selection.surah && ref[1] >= selection.start && ref[1] <= selection.end;
		if (!element.matches('.quran-corpus-word, .quran-mushaf-ayah-marker')) return;
		if (element.classList.contains('quran-mushaf-search-ayah') || selected)
			element.classList.toggle('quran-audio-active-ayah', selected);
		element.classList.toggle('quran-mushaf-search-ayah', selected);
	});
}

function initQuranMushafSearchAyahScroll(root) {
	if (!/^\/quran\/page\/\d+$/.test(window.location.pathname))
		return;
	var ref = new URLSearchParams(window.location.search).get('ayah') || '';
	if (!quranMushafAyahSelection(ref))
		return;
	var scope = root || document;
	var selector = '.quran-mushaf-search-ayah';
	var target = scope.querySelector(`.quran-corpus-word${selector}`) || scope.querySelector(selector);
	if (!target)
		return;
	var line = target.closest('.quran-mushaf-line') || target;
	var scrollToLine = function () {
		var navbar = document.querySelector('.site-navbar.fixed-top');
		var navbarOffset = navbar ? navbar.getBoundingClientRect().height : 0;
		window.scrollTo({
			top: Math.max(0, window.pageYOffset + line.getBoundingClientRect().top - navbarOffset - 12),
			behavior: 'auto'
		});
	};
	window.requestAnimationFrame(function () {
		window.requestAnimationFrame(function () {
			scrollToLine();
			window.setTimeout(scrollToLine, 320);
		});
	});
}
...
function initQuranMushafInfinite(root) {
	...
	var schedulePageUrlUpdate = function (pageNumber) {
		...
		pageUrlTimer = window.setTimeout(function () {
			...
			if (!querySuffix && quranMushafAyahSelection(selectedRef) && pendingPage && pendingPage.querySelector('.quran-mushaf-search-ayah'))
	...
	var loadNextPage = function () {
		...
		return fetch(quranApiPath(nextUrl), { credentials: 'same-origin' }).then(function (response) {
			...
			highlightQuranMushafSelection(importedPage);
...
function initQuranDynamicPassageHero(root) {
	...
	var selectedRefFromPath = function () {
		...
		if (/\/quran\/page\/\d+$/.test(window.location.pathname)) {
			...
			return quranMushafAyahSelection(queryRef) ? queryRef : '';
		}
		return '';
	};
	...
	var setMushafSearchAyahHighlight = function (ref) {
		if (!/^\/quran\/page\/\d+$/.test(window.location.pathname))
			return false;
		document.querySelectorAll('[data-quran-mushaf-page]').forEach(function (page) {
			highlightQuranMushafSelection(page, ref);
		});
		refreshQuranMushafAyahBackgrounds([document]);
		return true;
	};
...
function initQuranInfinitePassageNavigation(root) {
	...
	var appendPassage = function (html, url) {
		...
		var chunk = $('<section class="quran-infinite-page" data-quran-infinite-page="1"></section>').attr({
			// The fetched document lives under /quran/api, but browser history must
			// retain the public passage URL that initiated the request.
			'data-quran-url': url,
			'data-page-title': parsed.title || '',
			'data-quran-surah': remoteSurah,
			'data-quran-prev-url': publicQuranInfiniteUrl(remoteMain.getAttribute('data-quran-prev-url') || ''),
			'data-quran-prev-title': remoteMain.getAttribute('data-quran-prev-title') || 'Previous',
			'data-quran-next-url': publicQuranInfiniteUrl(remoteMain.getAttribute('data-quran-next-url') || ''),
		...
		var reflectionSection = remoteChildren.find(function (node) {
			return node.matches && node.matches('.reflection-section[data-reflection-section]');
		});
		...
		nodes.forEach(function (node) {
			...
			if (imported.matches && imported.matches('.reflection-section[data-reflection-section]'))
		...
		if (window.refreshHadithActions)
			window.refreshHadithActions();
		scheduleQuranHeadingTocUpdate();
		nextUrl = publicQuranInfiniteUrl(remoteMain.getAttribute('data-quran-next-url') || '');
		if (nextUrl)
...
function updateTafsirPassageCarouselRef(ref) {
	var match = (ref || '').toString().match(/^(\d+):(\d+)/);
	if (!match)
		return;
	var surah = match[1];
	var ayah = match[2];
	document.querySelectorAll('.quran-commentary-passage-carousel [data-commentary-slug]').forEach(function (link) {
		var slug = link.getAttribute('data-commentary-slug') || '';
		if (!slug)
			return;
		var href = quranUrl(`/quran/tafsir/${encodeURIComponent(slug)}/quran:${encodeURIComponent(surah)}:${encodeURIComponent(ayah)}`);
		link.setAttribute('href', href);
		link.setAttribute('data-commentary-href', href);
	});
	document.querySelectorAll('.quran-commentary-passage-carousel').forEach(function (carousel) {
		carousel.setAttribute('data-commentary-quran-ref', `${surah}:${ayah}`);
	});
}
...
function initReaderInfiniteNavigation(root) {
	...
	$(scope).find('[data-reader-infinite]').addBack('[data-reader-infinite]').each(function () {
		...
		var updateInfiniteNavigation = function () {
			...
			if (mode === 'tafsir') {
				updateQuranReaderModeHrefs(marker);
				var introductionContext = marker.getAttribute('data-reader-context-key') === '0';
				var headingRail = document.querySelector('[data-quran-heading-toc]');
				var headingRailSurah = headingRail && headingRail.querySelector('[data-quran-heading-toc-surah]');
				document.querySelectorAll('.quran-commentary-introduction-rail-link').forEach(function (link) {
					link.hidden = !introductionContext && marker.getAttribute('data-reader-context-key') !== '1';
				});
				if (introductionContext) {
					quranHeadingTocState.surah = 0;
					if (headingRailSurah)
						headingRailSurah.textContent = 'Introduction';
					var focus = Math.max(96, window.innerHeight * .28);
					var articles = Array.from(document.querySelectorAll('[data-quran-introduction-target]'));
					var activeArticle = articles.filter(function (article) {
						return article.getBoundingClientRect().top <= focus;
					}).pop() || articles[0];
					var activeIntroduction = activeArticle && activeArticle.getAttribute('data-quran-introduction-target');
					document.querySelectorAll('[data-quran-introduction-heading]').forEach(function (link) {
						var active = link.getAttribute('data-quran-introduction-heading') === activeIntroduction;
						link.classList.toggle('active', active);
						if (active)
							link.setAttribute('aria-current', 'location');
						else
				...
				if (!introductionContext)
					updateTafsirPassageCarouselRef(quranRef);
				if (quranRefMatch) {
					var headingSurah = Number(quranRefMatch[1]);
					if (!introductionContext && headingSurah > 0 && quranHeadingTocState.surah !== headingSurah)
						renderQuranHeadingToc(headingSurah);
		...
		var appendTafsirPage = function (remoteMain, chunk) {
			...
			var surahIntroduction = remoteChildren.find(function (node) {
				return node.matches && node.matches('.quran-commentary-heading-intro');
			});
			...
			var introductionArticles = remoteChildren.filter(function (node) {
				return node.matches && node.matches('.quran-commentary-introduction-article');
			});
			...
			if (breadcrumbs)
				nodes.push(breadcrumbs);
			if (surahIntroduction)
				nodes.push(surahIntroduction);
			if (tafsirArticle)
				nodes.push(tafsirArticle);
			if (reflectionSection)
				nodes.push(reflectionSection);
			if (introductionArticles.length)
				nodes.push.apply(nodes, introductionArticles);
			if (nodes.length < 1)
...
function publicQuranInfiniteUrl(url) {
	if (!url)
		return '';
	try {
		var parsed = new URL(url, window.location.origin);
		parsed.pathname = parsed.pathname.replace(/^\/quran\/api(?=\/|$)/, '/quran');
		return parsed.origin === window.location.origin
			? `${parsed.pathname}${parsed.search}${parsed.hash}`
			: parsed.toString();
	} catch (_err) {
		return url.toString().replace(/^\/quran\/api(?=\/|$)/, '/quran');
	}
}
...
function quranScriptChoiceShortLabel(script) {
	return { uthmani: 'Uth', 'indo-pak': 'Ind', warsh: 'War' }[script] || 'Scr';
}
...
function updateQuranScriptSwitcher(switcher, script) {
	...
	var shortLabel = switcher.querySelector('[data-quran-script-short-label]');
	if (shortLabel)
		shortLabel.textContent = quranScriptChoiceShortLabel(script);
	...
	getQuranTafsirSettings().then(function (settings) {
		...
		switchers.forEach(function (switcher) {
			...
			switcher.addEventListener('click', function (event) {
				...
				var shortLabels = Array.from(document.querySelectorAll('[data-quran-script-short-label]'));
				shortLabels.forEach(function (label) { label.textContent = '…'; });
...
function initQuranAyahSelector(root) {
	...
	function applyBookCarouselFilter(filter) {
		...
		var availableCount = items.filter(function () {
			return !$(this).hasClass('d-none') && !$(this).hasClass('tafsir-carousel-language-hidden') && !$(this).hasClass('book-carousel-filter-hidden');
		}).length;
	...
	function updateTafsirCarouselLanguageSwitch(languageSwitch, language) {
		language = normalizeQuranTafsirLanguage(language) || 'en';
		var isArabic = language === 'ar';
		languageSwitch.attr({
			'data-tafsir-carousel-language': language,
			'aria-label': isArabic
				? 'Showing Arabic-only tafsirs; switch to English and bilingual tafsirs'
				: 'Showing English and bilingual tafsirs; switch to Arabic-only tafsirs'
		}).text(isArabic ? 'Ar' : 'En');
	}

	function applyTafsirCarouselLanguage(menu, language) {
		language = normalizeQuranTafsirLanguage(language) || 'en';
		menu.children('[data-tafsir-carousel-language]:not([data-tafsir-carousel-language-switch])').each(function () {
			$(this).toggleClass('tafsir-carousel-language-hidden', $(this).attr('data-tafsir-carousel-language') !== language);
		});
		updateTafsirCarouselLanguageSwitch(menu.children('[data-tafsir-carousel-language-switch]').first(), language);
		applyBookCarouselFilter(menu.children('[data-book-carousel-filter]').first());
	}

	function initTafsirCarouselLanguageSwitches(root) {
		var scope = root || document;
		$(scope).find('[data-tafsir-carousel-language-switch]').each(function () {
			var languageSwitch = $(this);
			if (languageSwitch.closest('.quran-tafsirs').length || languageSwitch.data('tafsirCarouselLanguageSwitchBound'))
				return;
			languageSwitch.data('tafsirCarouselLanguageSwitchBound', true);
			var menu = languageSwitch.closest('.h-menu');
			var currentItem = menu.children('[data-tafsir-carousel-language].active, [data-tafsir-carousel-language][aria-current="page"]').first();
			var selectedLanguage = normalizeQuranTafsirLanguage(menu.closest('[data-current-commentary-carousel-language]').attr('data-current-commentary-carousel-language'));
			var initialLanguage = selectedLanguage || currentItem.attr('data-tafsir-carousel-language') || getStoredQuranTafsirLanguage() || 'en';
			applyTafsirCarouselLanguage(menu, initialLanguage);
			languageSwitch.on('click', function () {
				var language = languageSwitch.attr('data-tafsir-carousel-language') === 'ar' ? 'en' : 'ar';
				storeQuranTafsirLanguage(language);
				applyTafsirCarouselLanguage(menu, language);
			});
		});
	}
	...
	function refreshTafsirBookCarousels(root) {
		...
		function scrollQuranCommentaryCarouselToCurrent(carousel) {
			...
			var current = carousel.find('[data-quran-commentary-book-nav-item].active:not(.d-none):not(.tafsir-carousel-language-hidden):not(.book-carousel-filter-hidden), [data-quran-commentary-book-nav-item][aria-current="page"]:not(.d-none):not(.tafsir-carousel-language-hidden):not(.book-carousel-filter-hidden)').get(0);
			if (!menu || !current)
				return;
			window.requestAnimationFrame(function () {
				var targetLeft = current.offsetLeft - ((menu.clientWidth - current.offsetWidth) / 2);
		...
		function applyQuranCommentaryTocNavigation(root) {
			...
			getQuranTafsirSettings().then(function (settings) {
				$(scope).find('[data-quran-commentary-carousel]').each(function () {
					...
					var isPassageCarousel = carousel.attr('data-commentary-passage-carousel') === '1';
	...
	$('input[role=search][name=q], .quran-passage-search').each(function () {
		...
		$input.on('keydown', function (event) {
			...
			if (activeItem && activeItem.url) {
				event.preventDefault();
				var activeLink = $widget.find('.ui-state-active').closest('a[href]').get(0);
				if (activeLink)
					activeLink.click();
				else
					window.location.assign(searchAutocompleteDestination($input, activeItem));
				return;
			}
			event.preventDefault();
			if (suggestionOnly) {
		...
		$input.autocomplete({
			...
			select: function (event, ui) {
				if (ui.item.url) {
					var nativeLink = event.originalEvent && event.originalEvent.target && event.originalEvent.target.closest
						? event.originalEvent.target.closest('a[href]')
						: null;
					if (!nativeLink) {
						event.preventDefault();
						window.location.assign(searchAutocompleteDestination($input, ui.item));
					}
			...
			var $item = $('<li>');
			var $row = item.url
				? $('<a>').attr('href', searchAutocompleteDestination($input, item))
				: $('<div>');
			$row.addClass('search-autocomplete-item');
...
function searchAutocompleteDestination($input, item) {
	...
	var returnMode = $form.find('input[name="mode"]:not(:disabled)').val() || '';
	if (returnMode === 'mushaf') {
		var mushafPage = Number(item.mushaf_page);
		if (Number.isInteger(mushafPage) && mushafPage >= 1 && mushafPage <= 604) {
			var mushafRef = (item.ref || '').toString().match(/^quran:(\d+):(\d+(?:-\d+)?)/);
			var ayahQuery = mushafRef ? `?ayah=${mushafRef[1]}:${mushafRef[2]}` : '';
			return quranUrl(`/quran/page/${mushafPage}${ayahQuery}`);
		}
	}
	else if (returnMode === 'tafsir' && item.type === 'Ayah') {
		var tafsirBase = ($form.attr('data-quran-tafsir-base') || '').replace(/\/$/, '');
		if (!tafsirBase) {
			var tafsirMode = ($form.find('input[name="tafsirMode"]:not(:disabled)').val() || '').toString().trim();
			if (tafsirMode)
				tafsirBase = `/quran/tafsir/${encodeURIComponent(tafsirMode)}`;
		}
		var ref = (item.ref || '').toString().replace(/^\/+/, '');
		if (tafsirBase && /^quran:\d+:\d+(?:-\d+)?$/.test(ref))
			return quranUrl(`${tafsirBase}/${ref}`);
	}
	return quranUrl(url);
}
...
function buildSearchAutocompleteParams($input, term) {
	...
	$input.closest('form').find('input[name=tafsir]:checked:not(:disabled)').each(function () {
...
function initQuranSearchReturnMode() {
	...
	var requestedMode = searchParams.get('mode');
	var mode = requestedMode === 'mushaf' || requestedMode === 'study' || requestedMode === 'tafsir' ? requestedMode : '';
	var tafsirMode = requestedMode === 'tafsir' ? (searchParams.get('tafsirMode') || '') : '';
	if (/^\/quran\/page(?:\/\d+)?$/.test(path) || path === '/quran/review')
		mode = 'mushaf';
	else if (/^\/quran\/\d+(?:\/|$)/.test(path))
		mode = 'study';
	else {
		var tafsirPathMatch = path.match(/^\/quran\/tafsir\/([^/]+)/);
		if (tafsirPathMatch) {
			mode = 'tafsir';
			try {
				tafsirMode = decodeURIComponent(tafsirPathMatch[1]);
			} catch (err) {
				tafsirMode = tafsirPathMatch[1];
			}
		}
	}
	if (!mode)
	...
	document.querySelectorAll('form').forEach(function (form) {
		...
		input.value = mode;
		if (mode === 'tafsir' && tafsirMode) {
			var tafsirInput = form.querySelector('input[name="tafsirMode"]');
			if (!tafsirInput) {
				tafsirInput = document.createElement('input');
				tafsirInput.type = 'hidden';
				tafsirInput.name = 'tafsirMode';
				form.appendChild(tafsirInput);
			}
			tafsirInput.value = tafsirMode;
			form.dataset.quranTafsirBase = `/quran/tafsir/${encodeURIComponent(tafsirMode)}`;
		}
	});
}
...
function quranSearchBookFilterParams($form, useQuranDefaults) {
	...
	var $filters = $form.find('input[name=b]').filter(function () {
		return !this.disabled && (this.type === 'hidden' || this.checked);
	});
...
function hadithSharhGoogleUrl(link) {
	...
	var query = body.toString()
		...
		.slice(0, 25)
		.join(' ');
	return query ? `https://www.google.com/search?igu=1&q=site%3Adorar.net+${encodeURIComponent(query)}` : '';
}

function hadithSharhModal() {
	var modal = document.getElementById('hadith-sharh-modal');
	if (modal)
		return modal;
	modal = document.createElement('aside');
	modal.id = 'hadith-sharh-modal';
	modal.className = 'modal fade';
	modal.tabIndex = -1;
	modal.setAttribute('aria-labelledby', 'hadith-sharh-modal-title');
	modal.setAttribute('aria-hidden', 'true');
	modal.innerHTML = `
		<div class="modal-dialog modal-xl modal-dialog-scrollable">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="btn btn-sm btn-outline-secondary me-3 hadith-sharh-back" disabled aria-label="Go back in Sharh search results" title="Back">
						<span class="bi bi-arrow-left" aria-hidden="true"></span> Back
					</button>
					<h2 class="modal-title fs-5" id="hadith-sharh-modal-title">Sharh</h2>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<div class="modal-body"><div class="text-center py-4"><span class="spinner-border" role="status" aria-hidden="true"></span><span class="visually-hidden">Loading Sharh…</span></div></div>
				<div class="modal-footer justify-content-start d-none"></div>
			</div>
		</div>`;
...
function openHadithSharhModal(link) {
	var modalElement = hadithSharhModal();
	var modalBody = modalElement.querySelector('.modal-body');
	var modalFooter = modalElement.querySelector('.modal-footer');
	var backButton = modalElement.querySelector('.hadith-sharh-back');
	var googleUrl = hadithSharhGoogleUrl(link);
	...
	modal.show();
	if (!googleUrl) {
		modalBody.innerHTML = '<div class="alert alert-warning mb-0">No Arabic Hadith text is available to search.</div>';
		return;
	}
	var iframe = document.createElement('iframe');
	iframe.className = 'border-0 w-100';
	iframe.style.height = '70vh';
	iframe.title = 'Google search results for this Hadith';
	...
	iframe.addEventListener('load', function () {
		...
		backButton.disabled = navigationDepth <= 1;
	});
	...
	external.href = googleUrl;
	external.target = '_blank';
	external.rel = 'noopener noreferrer';
	external.textContent = 'Open Google results in a new tab';
	modalFooter.replaceChildren(external);
