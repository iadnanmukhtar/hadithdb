...
* Signed-in readers make one initial self-assessment for an untouched Later āyah: Learning, Weak, Memorized, or Core. After enrollment, review grades manage movement among those stages; the manual menu exposes only Paused and Later. The same rule is enforced by the server for individual, page, surah, and progress-group updates. A complete page or surah can receive an initial bulk assessment only while all of its āyāt are Later; mixed groups can only be moved to Later. Signed-out readers are not shown a control that could imply local persistence.
...
* An initial **Weak** assessment initializes the ayah like a Hard result, with lower stability, higher difficulty, and a short first interval. Weak āyāt remain eligible for regular Review. One Easy or two consecutive Good reviews promote Weak to Memorized; Hard or Again keeps it Weak.
...
* **Weak** combines deliberately fragile memorization with scheduler-driven recovery after Again on a Memorized āyah. Recovery remains distinct internally for scheduling, but it uses the Weak label, color, count, and progress filter. A successful recovery returns it to Memorized.
