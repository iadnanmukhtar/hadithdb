...
Regular review sessions are persisted and bounded. They admit eligible Learning, Weak, and Memorized āyāt, including automatic Weak recovery, under separate internal category caps, allocate the overall limit fairly across nonempty categories, and present Learning first. Newly enrolled, ungraded Learning cards are immediately included in Due now, but enter each session gradually under the Learning cap. Any one-time Again retries follow the original queue; Core āyāt are not admitted to regular sessions. An āyah cannot reappear after a grade or Skip unless it entered that retry slot. **My Settings → Quran Memorization** and the **Review settings** modal configure the maximum total āyāt per session (default 10), optional time budget, and a collapsed Advanced session limits group containing Learning, Weak recovery, Weak, and Memorized caps plus queue order.
...
**All āyāt** is the first scope choice and reviews the complete selected surah. For surahs longer than three Mushaf pages, the user may instead choose the first 1, 2, 3, 4, 5, 10, or 20 pages; choices longer than the selected surah are hidden. Shorter surahs use all āyāt automatically. When a Page or Surah Review starts, every Later āyah in the selected scope becomes a Learning card, enters the schedule, and is included in the persisted Quran-order session snapshot. Existing Learning, Weak, Memorized, Core, and Paused states and their review history are preserved.
