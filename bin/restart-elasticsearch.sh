...
ES_USERNAME="${ELASTICSEARCH_USERNAME:-${ES_USERNAME:-}}"
ES_PASSWORD="${ELASTICSEARCH_PASSWORD:-${ES_PASSWORD:-}}"
SETTINGS_PATH="${HADITHDB_SETTINGS_PATH:-/home/adnanmukhtar/.hadithdb/settings.json}"
...
if [[ -z "${NODE_BIN}" && -x /home/linuxbrew/.linuxbrew/bin/node ]]; then
 NODE_BIN="/home/linuxbrew/.linuxbrew/bin/node"
fi

if [[ (-z "${ES_USERNAME}" || -z "${ES_PASSWORD}") && -r "${SETTINGS_PATH}" && -n "${NODE_BIN}" ]]; then
 settings_auth=$("${NODE_BIN}" -e 'const fs=require("fs"); const p=process.argv[1]; const s=JSON.parse(fs.readFileSync(p,"utf8")); console.log(`${s.search?.username||s.search?.user||""}\n${s.search?.password||""}`);' "${SETTINGS_PATH}" 2>/dev/null || true)
 if [[ -z "${ES_USERNAME}" ]]; then
   ES_USERNAME="$(printf '%s\n' "${settings_auth}" | sed -n '1p')"
 fi
 if [[ -z "${ES_PASSWORD}" ]]; then
   ES_PASSWORD="$(printf '%s\n' "${settings_auth}" | sed -n '2p')"
 fi
fi

get_health_status() {
 local response
 local curl_args=(-fsSL --max-time "${CURL_MAX_TIME}")
 if [[ -n "${ES_USERNAME}" && -n "${ES_PASSWORD}" ]]; then
   curl_args+=(-u "${ES_USERNAME}:${ES_PASSWORD}")
 fi
 response=$(curl "${curl_args[@]}" "${ES_URL}/_cluster/health" 2>/dev/null) || return 1
